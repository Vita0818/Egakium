# Web renderer 与 Egakium 原生渲染层对比

日期：2026-07-26

## 结论

把这个实验的**行为合同**作为 Egakium 的参考是可行的；把 React、KaTeX、
CodeMirror 和会话驻留控制器整套塞进生产 App 的 `WKWebView` 则技术上可行、
工程上不划算。当前直接接 App 的结论是 **NO-GO**。

推荐保留现有 `EgakiumMessageContentView`、plain-safe 熔断、64 KiB admission、
...
| 维度 | 独立 Web 实验 | 当前 Egakium 原生实现 | 判断 |
|---|---|---|---|
...
`EgakiumMessageContentView` 完成 role、plain-safe、revision、appearance 和
...
```text
EgakiumMessageContentView
  → EgakiumMicrosoftMarkdownPipeline
...
合同，但**不能**直接证明 ChatGPT 私有 thread tree、Egakium runtime 或 WebContent
进程会使用相同内存。真实生产方案还需要：
...
- Web 实验与 Egakium native fixture 的逐语料视觉/辅助功能一致性；
