---
name: egakium-skill-creator
description: Create, adapt, validate, and iterate on Egakium or Codex-compatible Agent Skills. Use when a user asks to add a Skill, improve an existing SKILL.md, organize Skill scripts or references, validate discovery compatibility, or design a reusable workflow for Code or Cowork.
---

<!-- Modified by Egakium from the OpenAI Codex skill-creator sample.
See ThirdPartyNotices/OpenAICodexSkillCreator.md. -->

# Egakium Skill Creator
...
- Remember that Egakium freezes a fresh Skill snapshot for each Code send or Cowork
...
Egakium currently freezes text resources. Do not make a Skill depend on an unreadable binary
...
```sh
python3 .agents/skills/egakium-skill-creator/scripts/init_skill.py my-skill
```
...
dependencies that the Egakium host can attest in the same request-owned MCP snapshot.
...
```sh
python3 .agents/skills/egakium-skill-creator/scripts/quick_validate.py \
...
`agents/openai.yaml` limit. It does not reimplement Egakium' strict Swift MCP metadata parser;
when MCP dependencies are present, the actual Egakium loader is authoritative for their schema
