...
/// Unified error type across all Egakium modules.
public enum EgakiumError: Error, Sendable, Equatable, LocalizedError {
    case config(String)
...
/// `EgakiumError.cancelled`; callers must therefore check both the error shape
/// and the current task's cancellation bit before treating a turn as
/// user-interrupted.
public enum EgakiumCancellation {
    public static func isCancellationSignal(_ error: Error) -> Bool {
        if error is CancellationError {
            return true
        }
        guard let egakiumError = error as? EgakiumError else {
            return false
        }
