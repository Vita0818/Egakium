...
/// Stable identity of one independently editable HTML document that can be
/// represented as a card on an Egakium Session Canvas.
///
/// This is deliberately not an ownership relationship with ``AgentID``. A
/// spawn operation may provision an initial element for a new agent, while
/// later tasks remain free to reassign either the agent or the document.
public struct CanvasElementID: TypedID {
    public let rawValue: String
    public init(rawValue: String) { self.rawValue = rawValue }
    public static func new() -> CanvasElementID {
        CanvasElementID(rawValue: IDGen.random(prefix: "element", length: 12))
    }
}
