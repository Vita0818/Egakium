...
public enum EgakiumDiagnosticConstants {
    public static let subsystem = "com.Vita0818.Egakium"
...
public enum EgakiumPerformanceIntervalKind: String, Codable, CaseIterable, Sendable {
...
public enum EgakiumDiagnosticDurationMetric:
    String,
    Codable,
    CaseIterable,
    Sendable
...
public enum EgakiumDiagnosticDurationBucket:
    String,
    Codable,
    CaseIterable,
    Sendable
...
public struct EgakiumDiagnosticDurationHistogram:
    Codable,
    Equatable,
    Sendable
{
    ...
    public func value(
        for bucket: EgakiumDiagnosticDurationBucket
    ) -> UInt64 {
        switch bucket {
        case .under1Millisecond:
            return under1Millisecond
    ...
    fileprivate mutating func record(
        _ bucket: EgakiumDiagnosticDurationBucket
    ) {
        switch bucket {
        case .under1Millisecond:
            under1Millisecond =
                egakiumSaturatingAdd(under1Millisecond, 1)
        case .oneToUnder4Milliseconds:
            oneToUnder4Milliseconds =
                egakiumSaturatingAdd(oneToUnder4Milliseconds, 1)
        case .fourToUnder16Milliseconds:
            fourToUnder16Milliseconds =
                egakiumSaturatingAdd(fourToUnder16Milliseconds, 1)
        case .sixteenToUnder50Milliseconds:
            sixteenToUnder50Milliseconds =
                egakiumSaturatingAdd(sixteenToUnder50Milliseconds, 1)
        case .fiftyToUnder100Milliseconds:
            fiftyToUnder100Milliseconds =
                egakiumSaturatingAdd(fiftyToUnder100Milliseconds, 1)
        case .oneHundredToUnder500Milliseconds:
            oneHundredToUnder500Milliseconds =
                egakiumSaturatingAdd(oneHundredToUnder500Milliseconds, 1)
        case .fiveHundredToUnder2000Milliseconds:
            fiveHundredToUnder2000Milliseconds =
                egakiumSaturatingAdd(fiveHundredToUnder2000Milliseconds, 1)
        case .atLeast2000Milliseconds:
            atLeast2000Milliseconds =
                egakiumSaturatingAdd(atLeast2000Milliseconds, 1)
        }
    }
}

public struct EgakiumDiagnosticDurationAggregate:
    Codable,
    Equatable,
    Sendable
{
    ...
    public private(set) var histogram: EgakiumDiagnosticDurationHistogram

    public init(
        count: UInt64 = 0,
        totalNanoseconds: UInt64 = 0,
        maximumNanoseconds: UInt64 = 0,
        histogram: EgakiumDiagnosticDurationHistogram = .init()
    ) {
        self.count = count
    ...
    fileprivate mutating func record(durationNanoseconds: UInt64) {
        count = egakiumSaturatingAdd(count, 1)
        totalNanoseconds = egakiumSaturatingAdd(
            totalNanoseconds,
            durationNanoseconds)
        maximumNanoseconds = max(maximumNanoseconds, durationNanoseconds)
        histogram.record(
            EgakiumDiagnosticDurationBucket.bucket(
                forNanoseconds: durationNanoseconds))
    }
}

/// Fixed-shape duration section embedded in a metrics snapshot.
public struct EgakiumDiagnosticDurationSummaries:
    Codable,
    Equatable,
    Sendable
{
    public let projectionBatch: EgakiumDiagnosticDurationAggregate
    public let projectionFold: EgakiumDiagnosticDurationAggregate
    public let projectionCommit: EgakiumDiagnosticDurationAggregate
    public let markdownQueueWait: EgakiumDiagnosticDurationAggregate
    public let markdownParse: EgakiumDiagnosticDurationAggregate
    public let markdownPublish: EgakiumDiagnosticDurationAggregate

    public init(
        projectionBatch: EgakiumDiagnosticDurationAggregate = .init(),
        projectionFold: EgakiumDiagnosticDurationAggregate = .init(),
        projectionCommit: EgakiumDiagnosticDurationAggregate = .init(),
        markdownQueueWait: EgakiumDiagnosticDurationAggregate = .init(),
        markdownParse: EgakiumDiagnosticDurationAggregate = .init(),
        markdownPublish: EgakiumDiagnosticDurationAggregate = .init()
    ) {
    ...
    fileprivate init(
        values: [EgakiumDiagnosticDurationMetric:
            EgakiumDiagnosticDurationAggregate]
    ) {
        self.init(
    ...
    public func value(
        for metric: EgakiumDiagnosticDurationMetric
    ) -> EgakiumDiagnosticDurationAggregate {
        switch metric {
        case .projectionBatch:
...
private func egakiumSaturatingAdd(
    _ lhs: UInt64,
    _ rhs: UInt64
) -> UInt64 {
...
public struct EgakiumPerformanceIntervalFields: Codable, Equatable, Sendable {
...
public enum EgakiumScrollDiagnosticReason: Int, Codable, Sendable {
    case unknown = 0
    case liveContent = 1
    case completion = 2
    case terminal = 3
...
public enum EgakiumScrollDiagnosticOutcome: Int, Codable, Sendable {
    case requested = 1
    case executed = 2
    case cancelled = 3
    case stale = 4
...
public enum EgakiumDiagnosticCounter: String, Codable, CaseIterable, Sendable {
...
public enum EgakiumCoworkAgentSwitchDiagnosticOutcome: Int, Codable, Sendable {
    case requested = 1
    case committed = 2
    case stale = 3
}

public struct EgakiumDiagnosticMetricsSnapshot: Codable, Equatable, Sendable {
    public let counters: [String: UInt64]
    public let durations: EgakiumDiagnosticDurationSummaries

    public init(
        counters: [String: UInt64],
        durations: EgakiumDiagnosticDurationSummaries = .init()
    ) {
        self.counters = counters
        self.durations = durations
    }

    public func value(for counter: EgakiumDiagnosticCounter) -> UInt64 {
    ...
    public init(from decoder: Decoder) throws {
        ...
        durations = try container.decodeIfPresent(
            EgakiumDiagnosticDurationSummaries.self,
...
public struct EgakiumPerformanceIntervalToken: @unchecked Sendable {
    public let kind: EgakiumPerformanceIntervalKind
    fileprivate let completion: EgakiumPerformanceIntervalCompletion
    ...
    #if canImport(os)
    fileprivate init(
        kind: EgakiumPerformanceIntervalKind,
        state: OSSignpostIntervalState?,
        startedAtNanoseconds: UInt64
    ) {
        self.kind = kind
        self.state = state
        self.completion = EgakiumPerformanceIntervalCompletion(
...
private final class EgakiumPerformanceIntervalCompletion:
    @unchecked Sendable
{
...
public struct EgakiumProjectionBatchMetrics:
    Codable,
    Equatable,
    Sendable
{
...
public struct EgakiumProjectionBatchPublication:
    @unchecked Sendable,
    Equatable
{
    public let metrics: EgakiumProjectionBatchMetrics
    private let interval:
        EgakiumProjectionBatchInterval

    fileprivate init(
        metrics: EgakiumProjectionBatchMetrics,
        interval: EgakiumProjectionBatchInterval
    ) {
    ...
    public static func == (
        lhs: EgakiumProjectionBatchPublication,
        rhs: EgakiumProjectionBatchPublication
    ) -> Bool {
...
public final class EgakiumProjectionBatchInterval:
    @unchecked Sendable
{
    private let diagnostics:
        EgakiumPerformanceDiagnostics
    private let token:
        EgakiumPerformanceIntervalToken
    private let lock = NSLock()
    private var isFinished = false
    private var sealedMetrics:
        EgakiumProjectionBatchMetrics?
    ...
    public func seal(
        metrics: EgakiumProjectionBatchMetrics
    ) -> EgakiumProjectionBatchPublication {
        lock.lock()
        ...
        let canonicalMetrics =
            sealedMetrics ?? metrics
        lock.unlock()
        return EgakiumProjectionBatchPublication(
...
public final class EgakiumPerformanceDiagnostics: @unchecked Sendable {
    public static let shared = EgakiumPerformanceDiagnostics()

    private let lock = NSLock()
    private var counters: [EgakiumDiagnosticCounter: UInt64] = [:]
    private var durations:
        [EgakiumDiagnosticDurationMetric: EgakiumDiagnosticDurationAggregate] =
            [:]

    #if canImport(os)
    private let signposter = OSSignposter(
        subsystem: EgakiumDiagnosticConstants.subsystem,
    ...
    public func increment(
        _ counter: EgakiumDiagnosticCounter,
        by amount: UInt64 = 1
    ) {
    ...
    public func snapshot() -> EgakiumDiagnosticMetricsSnapshot {
        lock.lock()
        let values = counters
        let durationValues = durations
        lock.unlock()
        return EgakiumDiagnosticMetricsSnapshot(
            counters: Dictionary(
                uniqueKeysWithValues: values.map { ($0.key.rawValue, $0.value) }),
            durations: EgakiumDiagnosticDurationSummaries(
                values: durationValues))
    }

    public func beginInterval(
        _ kind: EgakiumPerformanceIntervalKind,
        fields: EgakiumPerformanceIntervalFields = .init()
    ) -> EgakiumPerformanceIntervalToken {
        let startedAtNanoseconds = DispatchTime.now().uptimeNanoseconds
        #if canImport(os)
        guard signposter.isEnabled else {
            return EgakiumPerformanceIntervalToken(
                kind: kind,
                state: nil,
                startedAtNanoseconds: startedAtNanoseconds)
        }
    ...
    public func endInterval(
        _ token: EgakiumPerformanceIntervalToken,
        fields: EgakiumPerformanceIntervalFields = .init()
    ) {
    ...
    func recordDuration(
        _ metric: EgakiumDiagnosticDurationMetric,
        nanoseconds: UInt64
    ) {
        lock.lock()
    ...
    private func durationMetric(
        for interval: EgakiumPerformanceIntervalKind
    ) -> EgakiumDiagnosticDurationMetric {
        switch interval {
        case .projectionBatch:
            return .projectionBatch
        case .markdownQueueWait:
    ...
    public func recordScrollRequest(
        reason: EgakiumScrollDiagnosticReason,
        outcome: EgakiumScrollDiagnosticOutcome,
        pendingCount: Int = 0
    ) {
        let counter: EgakiumDiagnosticCounter
        switch outcome {
        case .requested:
    ...
    public func recordCoworkAgentSwitch(
        outcome: EgakiumCoworkAgentSwitchDiagnosticOutcome,
...
public struct EgakiumMainThreadHeartbeatConfiguration: Equatable, Sendable {
    ...
    public static let production = Self(
        warningAfterNanoseconds:
            EgakiumDiagnosticConstants.heartbeatWarningMilliseconds * 1_000_000,
        incidentAfterNanoseconds:
            EgakiumDiagnosticConstants.heartbeatIncidentMilliseconds * 1_000_000,
        incidentCooldownNanoseconds:
            EgakiumDiagnosticConstants.heartbeatIncidentCooldownMilliseconds * 1_000_000)
}

public enum EgakiumMainThreadHeartbeatAction: Equatable, Sendable {
...
public struct EgakiumMainThreadHeartbeatStateMachine: Sendable {
    ...
    public let configuration: EgakiumMainThreadHeartbeatConfiguration
    ...
    public init(
        configuration: EgakiumMainThreadHeartbeatConfiguration = .production
    ) {
    ...
    public mutating func tick(
    ...
    ) -> [EgakiumMainThreadHeartbeatAction] {
        ...
        let delay = now >= pending.enqueuedAtNanoseconds
            ...
            : 0
        var actions: [EgakiumMainThreadHeartbeatAction] = []
...
public enum EgakiumHangDiagnosticSource: String, Codable, Sendable {
    case mainThreadHeartbeat
    case externalCapture
}

public struct EgakiumHangCaptureResult: Codable, Equatable, Sendable {
...
public struct EgakiumHangDiagnosticManifest: Codable, Equatable, Sendable {
    public let schemaVersion: Int
    public let source: EgakiumHangDiagnosticSource
    public let recordedAt: Date
    public let processIdentifier: Int32
    public let mainThreadDelayMilliseconds: UInt64?
    public let applicationVersion: String?
    public let buildVersion: String?
    public let metrics: EgakiumDiagnosticMetricsSnapshot?
    public let capture: EgakiumHangCaptureResult?

    public init(
        source: EgakiumHangDiagnosticSource,
        recordedAt: Date,
        processIdentifier: Int32,
        mainThreadDelayMilliseconds: UInt64? = nil,
        applicationVersion: String? = nil,
        buildVersion: String? = nil,
        metrics: EgakiumDiagnosticMetricsSnapshot? = nil,
        capture: EgakiumHangCaptureResult? = nil
    ) {
...
public enum EgakiumHangDiagnosticAttachmentKind: String, Codable, Sendable {
    case sample
    case unifiedLog
    case captureErrors
...
public struct EgakiumHangDiagnosticAttachment: Sendable {
    public let kind: EgakiumHangDiagnosticAttachmentKind
    fileprivate let data: Data

    private init(
        kind: EgakiumHangDiagnosticAttachmentKind,
        data: Data
    ) {
        self.kind = kind
        self.data = data
...
public enum EgakiumHangDiagnosticTextSanitizer {
...
public struct EgakiumHangDiagnosticRetentionPolicy: Equatable, Sendable {
...
public struct EgakiumHangDiagnosticBundleLocation: Equatable, Sendable {
...
public enum EgakiumHangDiagnosticStoreError:
...
public actor EgakiumHangDiagnosticBundleStore {
    ...
    public let retentionPolicy: EgakiumHangDiagnosticRetentionPolicy

    public init(
        rootURL: URL,
        retentionPolicy: EgakiumHangDiagnosticRetentionPolicy = .production
    ) {
    ...
    public func writeBundle(
        manifest: EgakiumHangDiagnosticManifest,
        attachments: [EgakiumHangDiagnosticAttachment] = []
    ) throws -> EgakiumHangDiagnosticBundleLocation {
        guard manifest.schemaVersion == 1,
              manifest.processIdentifier > 0 else {
            throw EgakiumHangDiagnosticStoreError.invalidManifest
        }
        ...
        for attachment in attachments {
            ...
            guard !result.overflow else {
                throw EgakiumHangDiagnosticStoreError.bundleTooLarge
            }
        ...
        return try DurableOwnerOnlyFile.withExclusiveLock(
            ...
            do {
                ...
                return EgakiumHangDiagnosticBundleLocation(
                    directoryURL: directory)
            } catch {
                try? Self.removeNewBundleIfSafe(directory)
                if error is EgakiumHangDiagnosticStoreError {
                    throw error
                }
                throw EgakiumHangDiagnosticStoreError.writeFailed
            }
        }
    }
    ...
    public func latestManifest(
        processIdentifier: Int32,
        recordedAfter: Date
    ) throws -> EgakiumHangDiagnosticManifest? {
            ...
            for descriptor in descriptors {
                ...
                guard let data = try DurableOwnerOnlyFile.read(from: url),
                      let manifest = try? Self.manifestDecoder()
                        .decode(EgakiumHangDiagnosticManifest.self, from: data),
    ...
    private func enforceRetentionLocked() throws {
        ...
        if descriptors.count > retentionPolicy.maximumBundleCount
            || totalBytes > retentionPolicy.maximumTotalBytes {
            throw EgakiumHangDiagnosticStoreError.retentionFailed
        }
    }
    ...
    private static func ensureOwnerOnlyDirectory(_ url: URL) throws {
        ...
        if existed {
            guard isDirectory.boolValue else {
                throw EgakiumHangDiagnosticStoreError.unsafeDirectory
            }
        } else {
    ...
    private static func safeBundleByteCount(_ directory: URL) -> Int? {
        ...
        let allowed = Set([
            "incident.json",
            EgakiumHangDiagnosticAttachmentKind.sample.fileName,
            EgakiumHangDiagnosticAttachmentKind.unifiedLog.fileName,
            EgakiumHangDiagnosticAttachmentKind.captureErrors.fileName,
        ])
        for child in children {
            guard child.deletingLastPathComponent().standardizedFileURL
