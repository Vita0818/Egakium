...
public struct EgakiumDiagnosticFileSnapshot: Equatable, Sendable {
...
public enum EgakiumDiagnosticSnapshotError:
...
public enum EgakiumDiagnosticSnapshotReader {
    ...
    public static func readTail(
        ...
        guard descriptor >= 0 else {
            if errno == ELOOP {
                throw EgakiumDiagnosticSnapshotError.unsafeFile
            }
            throw EgakiumDiagnosticSnapshotError.readFailed
        }
        ...
        return EgakiumDiagnosticFileSnapshot(
            data: result,
...
public struct EgakiumDiagnosticRedactedEventLog: Equatable, Sendable {
    public let data: Data
...
public enum EgakiumDiagnosticEventLogRedactor {
    ...
    public static func redact(
        _ snapshot: EgakiumDiagnosticFileSnapshot,
        maximumOutputBytes: Int = 8 * 1_024 * 1_024
    ) -> EgakiumDiagnosticRedactedEventLog {
        let outputLimit = max(
            1_024,
        ...
        return EgakiumDiagnosticRedactedEventLog(
            data: output,
    ...
    private static func redactEnvelope(
        ...
        for key in ["ts", "session", "type"] {
            ...
            output[key] = EgakiumHangDiagnosticTextSanitizer.sanitize(
                value,
                maximumBytes: 1_024)
        }
