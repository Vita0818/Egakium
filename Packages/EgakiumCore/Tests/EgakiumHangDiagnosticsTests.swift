...
final class EgakiumHangDiagnosticsTests: XCTestCase {
    ...
    func testIntervalDurationRecordsOnceEvenWhenTokenIsCopied() {
        ...
        XCTAssertEqual(
            EgakiumDiagnosticDurationBucket.allCases.reduce(0) {
    ...
    func testMetricsSnapshotDecodesLegacyCountersWithoutDurations() throws {
        ...
        let snapshot = try JSONDecoder().decode(
            EgakiumDiagnosticMetricsSnapshot.self,
            from: legacy)

        XCTAssertEqual(snapshot.value(for: .projectionBatches), 2)
        for metric in EgakiumDiagnosticDurationMetric.allCases {
            XCTAssertEqual(
    ...
    func testSanitizerRemovesPathsURLsAndCredentialShapes() {
        ...
        """
        let sanitized = EgakiumHangDiagnosticTextSanitizer.sanitize(raw)
        ...
        for index in 0..<3 {
            let manifest = EgakiumHangDiagnosticManifest(
                ...
                metrics: .init(counters: [
                    EgakiumDiagnosticCounter.mainThreadIncidents.rawValue:
                        UInt64(index + 1),
                ]))
    ...
    func testProductionRetentionIsFiveBundlesAndTwentyMegabytes() {
        XCTAssertEqual(
            EgakiumHangDiagnosticRetentionPolicy.production.maximumBundleCount,
            5)
        XCTAssertEqual(
            EgakiumHangDiagnosticRetentionPolicy.production.maximumTotalBytes,
            20 * 1_024 * 1_024)
    }
    ...
    func testHangBundleRetentionAlsoPrunesByTotalBytes() async throws {
        ...
        for index in 0..<2 {
            let attachment = EgakiumHangDiagnosticAttachment.sanitizedText(
                kind: .sample,
                rawData: Data(
    ...
    func testHangBundleSanitizesExternalTextAndLimitsAttachments() async throws {
        ...
        let store = EgakiumHangDiagnosticBundleStore(rootURL: root)
    ...
    func testHangBundleRejectsSymlinkRoot() async throws {
        ...
        let store = EgakiumHangDiagnosticBundleStore(rootURL: link)
        ...
        } catch let error as EgakiumHangDiagnosticStoreError {
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "egakium-hang-diagnostics-\(UUID().uuidString)",
            isDirectory: true)
        try FileManager.default.createDirectory(
            at: url,
            withIntermediateDirectories: false)
