#ifndef EGAKIUM_MCP_STDIO_GUARD_H
#define EGAKIUM_MCP_STDIO_GUARD_H
...
typedef struct egakium_mcp_stdio_guard egakium_mcp_stdio_guard_t;
...
typedef struct {
    ...
    uint64_t file_id;
    uint64_t byte_count;
} egakium_mcp_stdio_exec_identity_t;

typedef struct {
    int mode;
    uint32_t gateway_ipv4_network_order;
    uint16_t gateway_port_host_order;
} egakium_mcp_stdio_network_policy_t;

enum {
    EGAKIUM_MCP_STDIO_NETWORK_DENIED = 0,
    EGAKIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY = 1
};
...
} egakium_mcp_stdio_guard_spawn_result_t;
...
typedef struct {
    ...
    int error_number;
} egakium_mcp_stdio_guard_poll_result_t;

enum {
    EGAKIUM_MCP_STDIO_GUARD_OK = 0,
    EGAKIUM_MCP_STDIO_GUARD_UNAVAILABLE = 1,
    EGAKIUM_MCP_STDIO_GUARD_PIPE_FAILED = 2,
    EGAKIUM_MCP_STDIO_GUARD_FORK_FAILED = 3,
    EGAKIUM_MCP_STDIO_GUARD_TRACE_FAILED = 4,
    EGAKIUM_MCP_STDIO_GUARD_IDENTITY_FAILED = 5,
    EGAKIUM_MCP_STDIO_GUARD_POLICY_VIOLATION = 6
};
