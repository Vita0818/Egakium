...
#include "EgakiumMCPStdioGuard.h"
...
#define EGAKIUM_PTRACE_SYSCALL_INFO_NONE 0U
#define EGAKIUM_PTRACE_SYSCALL_INFO_ENTRY 1U
#define EGAKIUM_PTRACE_SYSCALL_INFO_EXIT 2U
#define EGAKIUM_PTRACE_SYSCALL_INFO_SECCOMP 3U
...
#define EGAKIUM_AUDIT_ARCH AUDIT_ARCH_X86_64
#elif defined(__aarch64__)
#define EGAKIUM_AUDIT_ARCH AUDIT_ARCH_AARCH64
#else
#define EGAKIUM_AUDIT_ARCH 0
#endif

#define EGAKIUM_SECCOMP_ALLOW SECCOMP_RET_ALLOW
#define EGAKIUM_SECCOMP_TRACE SECCOMP_RET_TRACE
#define EGAKIUM_SECCOMP_DENY \
    (SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA))
#define EGAKIUM_SECCOMP_NOT_IMPLEMENTED \
...
typedef struct {
    ...
    uint64_t file_id;
    uint64_t byte_count;
} egakium_owned_identity_t;

struct egakium_mcp_stdio_guard {
    pid_t group_id;
    pid_t initial_pid;
    pid_t target_pid;
    int wrapper_seen;
    ...
    int fatal_errno;
    egakium_mcp_stdio_network_policy_t network_policy;
    egakium_owned_identity_t wrapper;
    egakium_owned_identity_t primary;
    egakium_owned_identity_t *helpers;
    size_t helper_count;
    egakium_tracee_t *tracees;
    size_t tracee_count;
    size_t tracee_capacity;
};

#define EGAKIUM_MAXIMUM_TRACEE_COUNT 4096U
#define EGAKIUM_FREEZE_TIMEOUT_MILLISECONDS 2000U
#define EGAKIUM_NETWORK_SYSCALL_TIMEOUT_MILLISECONDS 5000U

typedef struct {
    struct sock_filter *items;
    size_t count;
    size_t capacity;
} egakium_filter_builder_t;

static void egakium_close_fd(int *fd) {
    if (*fd >= 0) {
        (void)close(*fd);
        *fd = -1;
    }
...
static int egakium_set_cloexec(int fd) {
...
static int egakium_set_nonblocking(int fd) {
    int flags = fcntl(fd, F_GETFL);
    if (flags < 0 || fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0) {
        return -1;
    }
    return egakium_set_cloexec(fd);
}

static int egakium_pipe_cloexec(int descriptors[2]) {
    ...
    if (egakium_set_cloexec(descriptors[0]) < 0 ||
        egakium_set_cloexec(descriptors[1]) < 0) {
        int saved = errno;
        close(descriptors[0]);
        close(descriptors[1]);
        errno = saved;
...
static int egakium_append_filter(
    egakium_filter_builder_t *builder,
...
#define EGAKIUM_STMT(code_value, k_value) \
    ((struct sock_filter)BPF_STMT((code_value), (k_value)))
#define EGAKIUM_JUMP(code_value, k_value, true_jump, false_jump) \
    ((struct sock_filter)BPF_JUMP( \
        (code_value), (k_value), (true_jump), (false_jump)))

static int egakium_append_deny_syscall(
    egakium_filter_builder_t *builder,
    int syscall_number,
    uint32_t denial) {
    return egakium_append_filter(
               builder,
               EGAKIUM_JUMP(
                   ...
                   0,
                   1)) ||
           egakium_append_filter(
               builder,
               EGAKIUM_STMT(BPF_RET | BPF_K, denial));
}

static int egakium_append_trace_syscall(
    egakium_filter_builder_t *builder,
    int syscall_number) {
    return egakium_append_deny_syscall(
        builder, syscall_number, EGAKIUM_SECCOMP_TRACE);
}

static int egakium_build_seccomp_filter(
    int helpers_enabled,
    struct sock_filter **items,
    size_t *count) {
    if (EGAKIUM_AUDIT_ARCH == 0) {
        errno = ENOTSUP;
        return -1;
    }
    egakium_filter_builder_t builder = {0};
    if (egakium_append_filter(
            &builder,
            EGAKIUM_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct egakium_seccomp_data, arch))) ||
        egakium_append_filter(
            &builder,
            EGAKIUM_JUMP(
                BPF_JMP | BPF_JEQ | BPF_K,
        ...
        egakium_append_filter(
            &builder,
            EGAKIUM_STMT(BPF_RET | BPF_K, SECCOMP_RET_KILL_PROCESS)) ||
        egakium_append_filter(
            &builder,
            EGAKIUM_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct egakium_seccomp_data, nr)))) {
        free(builder.items);
        return -1;
    }

#ifdef __NR_setsid
    if (egakium_append_deny_syscall(
            &builder, __NR_setsid, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_setpgid
    if (egakium_append_deny_syscall(
            &builder, __NR_setpgid, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_unshare
    if (egakium_append_deny_syscall(
            &builder, __NR_unshare, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_setns
    if (egakium_append_deny_syscall(
            &builder, __NR_setns, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_ptrace
    if (egakium_append_deny_syscall(
            &builder, __NR_ptrace, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_clone3
    ...
    if (egakium_append_deny_syscall(
            &builder,
            __NR_clone3,
            EGAKIUM_SECCOMP_NOT_IMPLEMENTED)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_fork
    if (!helpers_enabled &&
        egakium_append_deny_syscall(
            &builder, __NR_fork, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_vfork
    if (!helpers_enabled &&
        egakium_append_deny_syscall(
            &builder, __NR_vfork, EGAKIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
    ...
    if (egakium_append_filter(
        ...
        egakium_append_filter(
            ...
            EGAKIUM_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct egakium_seccomp_data, args[0]))) ||
        egakium_append_filter(
            &builder,
            EGAKIUM_JUMP(
        ...
        egakium_append_filter(
            &builder,
            EGAKIUM_STMT(
                BPF_RET | BPF_K, EGAKIUM_SECCOMP_DENY)) ||
        egakium_append_filter(
            &builder,
            EGAKIUM_STMT(
                BPF_RET | BPF_K, EGAKIUM_SECCOMP_ALLOW))) {
        goto allocation_failure;
    }
    if (helpers_enabled) {
     ...
     */
#ifdef __NR_socket
    if (egakium_append_trace_syscall(&builder, __NR_socket)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_connect
    if (egakium_append_trace_syscall(&builder, __NR_connect)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_bind
    if (egakium_append_trace_syscall(&builder, __NR_bind)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_listen
    if (egakium_append_trace_syscall(&builder, __NR_listen)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_accept
    if (egakium_append_trace_syscall(&builder, __NR_accept)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_accept4
    if (egakium_append_trace_syscall(&builder, __NR_accept4)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendto
    if (egakium_append_trace_syscall(&builder, __NR_sendto)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendmsg
    if (egakium_append_trace_syscall(&builder, __NR_sendmsg)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendmmsg
    if (egakium_append_trace_syscall(&builder, __NR_sendmmsg)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_setup
    if (egakium_append_trace_syscall(&builder, __NR_io_uring_setup)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_enter
    if (egakium_append_trace_syscall(&builder, __NR_io_uring_enter)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_register
    if (egakium_append_trace_syscall(&builder, __NR_io_uring_register)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_bpf
    if (egakium_append_trace_syscall(&builder, __NR_bpf)) {
        goto allocation_failure;
    }
#endif
    ...
    if (egakium_append_filter(
            &builder,
            EGAKIUM_STMT(BPF_RET | BPF_K, EGAKIUM_SECCOMP_ALLOW))) {
        goto allocation_failure;
    }
    *items = builder.items;
    *count = builder.count;
...
static int egakium_make_seccomp_fd(
    ...
    size_t count = 0;
    if (egakium_build_seccomp_filter(
...
static int egakium_close_from_four(void) {
...
static int egakium_write_handshake_byte(int fd) {
...
static int egakium_read_handshake_byte(int fd) {
...
static void egakium_guarded_child(
     ...
     */
    if (egakium_write_handshake_byte(startup_child) < 0 ||
        egakium_read_handshake_byte(startup_child) < 0) {
        _exit(126);
    }
    (void)close(startup_child);
    if (egakium_close_from_four() < 0 ||
...
static int egakium_capture_owned_identity(
    const char *path,
    egakium_owned_identity_t *identity) {
...
static int egakium_copy_identity(
    const egakium_mcp_stdio_exec_identity_t *source,
    egakium_owned_identity_t *destination) {
...
static int egakium_add_tracee(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid,
    int is_wrapper_root) {
    ...
    if (guard->tracee_count >= EGAKIUM_MAXIMUM_TRACEE_COUNT) {
        errno = E2BIG;
        return -1;
    }
    if (guard->tracee_count == guard->tracee_capacity) {
        ...
        egakium_tracee_t *tracees = (egakium_tracee_t *)realloc(
            guard->tracees, next * sizeof(egakium_tracee_t));
        if (tracees == NULL) {
            return -1;
        }
...
static egakium_tracee_t *egakium_find_tracee(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].pid == pid) {
...
static int egakium_identity_matches_pid(
    pid_t pid,
    const egakium_owned_identity_t *expected) {
    char proc_path[64];
    char resolved[4096];
    struct stat metadata;
    int length = snprintf(
...
static int egakium_match_helper(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    for (size_t index = 0; index < guard->helper_count; index++) {
        if (egakium_identity_matches_pid(pid, &guard->helpers[index])) {
            return 1;
        }
    }
    return 0;
...
typedef struct {
    ...
    uint64_t instruction_pointer;
    uint64_t stack_pointer;
} egakium_ptrace_syscall_info_prefix_t;

static int egakium_ptrace_syscall_operation(
    pid_t pid,
    uint8_t *operation) {
    egakium_ptrace_syscall_info_prefix_t information;
    ...
    if (count < (long)sizeof(information) ||
        information.operation
            == EGAKIUM_PTRACE_SYSCALL_INFO_NONE) {
        errno = ENOTSUP;
        return -1;
    }
...
static int egakium_read_syscall_snapshot(
    pid_t pid,
    egakium_syscall_snapshot_t *snapshot) {
#if defined(__x86_64__) || defined(__aarch64__)
    egakium_native_registers_t registers;
...
static int egakium_rewrite_syscall_number(
    pid_t pid,
    long syscall_number) {
#if defined(__x86_64__) || defined(__aarch64__)
    egakium_native_registers_t registers;
...
static int egakium_rewrite_syscall_result(
    pid_t pid,
    long result) {
#if defined(__x86_64__) || defined(__aarch64__)
    egakium_native_registers_t registers;
...
static int egakium_read_tracee_memory(
    pid_t pid,
...
static int egakium_validate_network_syscall(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    egakium_syscall_snapshot_t snapshot;
    memset(&snapshot, 0, sizeof(snapshot));
    if (egakium_read_syscall_snapshot(pid, &snapshot) < 0) {
        return -1;
    }
    ...
    if (snapshot.syscall_number == __NR_socket) {
        ...
        return guard->network_policy.mode
                    == EGAKIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY &&
               family == AF_INET &&
...
static uint64_t egakium_monotonic_milliseconds(void);

static int egakium_thread_group_leader(
    pid_t thread_id,
    pid_t *leader_id) {
 ...
 */
static int egakium_emulate_exact_connect(
    const egakium_mcp_stdio_guard_t *guard,
    pid_t pid,
    const egakium_syscall_snapshot_t *snapshot,
     ...
     */
    pid_t leader_id = 0;
    if (egakium_thread_group_leader(pid, &leader_id) < 0) {
        return -1;
    }
    int pid_descriptor =
    ...
    if (connect_result < 0 &&
        ...
        uint64_t start = egakium_monotonic_milliseconds();
        if (start == 0) {
            connect_error = EIO;
            goto connect_failure;
        }
        for (;;) {
            uint64_t now = egakium_monotonic_milliseconds();
            if (now == 0 ||
                now - start >=
                    EGAKIUM_NETWORK_SYSCALL_TIMEOUT_MILLISECONDS) {
...
static void egakium_mark_violation(
    egakium_mcp_stdio_guard_t *guard,
    int error_number) {
    if (!guard->violation) {
        guard->violation = 1;
...
static void egakium_mark_network_violation(
    egakium_mcp_stdio_guard_t *guard,
    int error_number) {
    guard->network_violation = 1;
    egakium_mark_violation(guard, error_number);
}

static int egakium_decode_exit_status(int status);
static int egakium_handle_exec_event(
    egakium_mcp_stdio_guard_t *guard,
    egakium_tracee_t *tracee);
...
static void egakium_poll_pause(void) {
    struct timespec value;
    value.tv_sec = 0;
...
static void egakium_record_tracee_exit(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid,
    int status) {
    egakium_tracee_t *tracee =
        egakium_find_tracee(guard, pid);
    if (tracee == NULL) {
        return;
    }
    tracee->alive = 0;
    ...
    if (pid == guard->target_pid) {
        ...
        guard->target_exit_status =
            egakium_decode_exit_status(status);
    }
}

static int egakium_all_live_tracees_frozen(
    const egakium_mcp_stdio_guard_t *guard) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].alive &&
...
static int egakium_capture_frozen_stop(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid,
    int status) {
    if (WIFEXITED(status) || WIFSIGNALED(status)) {
        egakium_record_tracee_exit(guard, pid, status);
        return 0;
    }
 ...
 */
static int egakium_execute_frozen_network_syscall(
    egakium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    egakium_tracee_t *tracee =
        egakium_find_tracee(guard, pid);
    ...
    egakium_syscall_snapshot_t expected;
    memset(&expected, 0, sizeof(expected));
    if (egakium_read_syscall_snapshot(
            pid, &expected) < 0) {
        return -1;
    }
    ...
    if (expected.syscall_number == __NR_connect) {
        if (egakium_emulate_exact_connect(
                guard,
                pid,
                &expected,
                &emulated_result) < 0 ||
            egakium_rewrite_syscall_number(pid, -1) < 0) {
            return -1;
        }
    ...
    for (;;) {
        ...
        if (waited == pid) {
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
                egakium_record_tracee_exit(
                    guard, pid, status);
                errno = EIO;
                return -1;
            }
            tracee = egakium_find_tracee(guard, pid);
            ...
            uint8_t operation = 0;
            egakium_syscall_snapshot_t observed;
            memset(&observed, 0, sizeof(observed));
            if (egakium_ptrace_syscall_operation(
                    pid, &operation) < 0 ||
                egakium_read_syscall_snapshot(
                    pid, &observed) < 0 ||
            ...
            if (operation
                    == EGAKIUM_PTRACE_SYSCALL_INFO_EXIT) {
                if (has_emulated_result &&
                    egakium_rewrite_syscall_result(
            ...
            if ((operation
                    != EGAKIUM_PTRACE_SYSCALL_INFO_ENTRY &&
                 operation
                    != EGAKIUM_PTRACE_SYSCALL_INFO_SECCOMP) ||
                ++syscall_stops > 2) {
                errno = EIO;
                return -1;
            }
 ...
 */
static int egakium_freeze_validate_and_execute_network(
    egakium_mcp_stdio_guard_t *guard,
    pid_t triggering_pid,
    int triggering_status) {
    egakium_tracee_t *trigger =
        egakium_find_tracee(guard, triggering_pid);
    if (trigger == NULL || !trigger->alive) {
        egakium_mark_network_violation(guard, ECHILD);
        return -1;
    }
    trigger->frozen = 1;
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        egakium_tracee_t *tracee = &guard->tracees[index];
        ...
        if (ptrace(
            ...
            egakium_mark_network_violation(guard, errno);
            return -1;
        }
    }
    ...
    uint64_t start = egakium_monotonic_milliseconds();
    if (start == 0) {
        egakium_mark_network_violation(guard, EIO);
        return -1;
    }
    while (!egakium_all_live_tracees_frozen(guard)) {
        int made_progress = 0;
        ...
        for (size_t index = 0; index < observed_count; index++) {
            ...
            if (waited < 0 ||
                egakium_capture_frozen_stop(
                    guard, pid, status) < 0) {
                int saved = errno;
                egakium_mark_network_violation(
        ...
        if (!made_progress) {
            uint64_t now = egakium_monotonic_milliseconds();
            if (now == 0 || now - start
                    >= EGAKIUM_FREEZE_TIMEOUT_MILLISECONDS) {
                egakium_mark_network_violation(
                    guard, ETIMEDOUT);
                return -1;
            }
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        ...
        if (event == PTRACE_EVENT_EXEC &&
            egakium_handle_exec_event(guard, tracee) < 0) {
            egakium_mark_violation(guard, EACCES);
            return -1;
        }
    }
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        ...
        if (egakium_ptrace_syscall_operation(
            ...
            operation
                != EGAKIUM_PTRACE_SYSCALL_INFO_SECCOMP ||
            egakium_validate_network_syscall(
                guard, pid) < 0 ||
            egakium_execute_frozen_network_syscall(
                guard, pid) < 0) {
            int saved = errno;
            egakium_mark_network_violation(
...
static int egakium_all_tracees_exited(
    const egakium_mcp_stdio_guard_t *guard) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].alive) {
            return 0;
...
static int egakium_handle_exec_event(
    ...
    if (!guard->wrapper_seen) {
        if (!tracee->is_wrapper_root ||
            !egakium_identity_matches_pid(tracee->pid, &guard->wrapper)) {
            return -1;
        }
    ...
    if (!guard->target_seen) {
        if (!egakium_identity_matches_pid(tracee->pid, &guard->primary)) {
            return -1;
        }
        guard->target_seen = 1;
    ...
    if (!tracee->may_exec_helper ||
        ...
        !egakium_match_helper(guard, tracee->pid)) {
        return -1;
    }
...
static void egakium_execution_guard_probe_child(void) {
    struct sock_filter filter[] = {
        BPF_STMT(
            BPF_LD | BPF_W | BPF_ABS,
            offsetof(struct egakium_seccomp_data, nr)),
...
int egakium_mcp_stdio_execution_guard_probe(void) {
    ...
    pid_t child = fork();
    if (child < 0) {
        egakium_close_fd(&startup_pair[0]);
        egakium_close_fd(&startup_pair[1]);
        return 0;
    }
    if (child == 0) {
        egakium_close_fd(&startup_pair[0]);
        if (egakium_read_handshake_byte(startup_pair[1]) < 0) {
            _exit(111);
        }
        egakium_close_fd(&startup_pair[1]);
        egakium_execution_guard_probe_child();
    }
    ...
    if (ptrace(
        ...
        egakium_write_handshake_byte(startup_pair[0]) < 0 ||
        waitpid(child, &status, __WALL) != child ||
        !WIFSTOPPED(status) ||
        (((unsigned int)status) >> 16)
            != PTRACE_EVENT_SECCOMP ||
        egakium_ptrace_syscall_operation(
            child, &operation) < 0 ||
        operation
            != EGAKIUM_PTRACE_SYSCALL_INFO_SECCOMP ||
        egakium_rewrite_syscall_number(child, -1) < 0 ||
        ...
        operation
            != EGAKIUM_PTRACE_SYSCALL_INFO_EXIT ||
        egakium_rewrite_syscall_result(child, 4242) < 0 ||
        ptrace(PTRACE_CONT, child, NULL, NULL) < 0 ||
        waitpid(child, &status, __WALL) != child ||
        !WIFEXITED(status) ||
            WEXITSTATUS(status) != 0) {
        egakium_close_fd(&duplicated_descriptor);
        egakium_close_fd(&pid_descriptor);
        egakium_close_fd(&startup_pair[0]);
...
int egakium_mcp_stdio_guard_spawn(
    const char *wrapper_path,
    char *const argv[],
    char *const envp[],
    const char *working_directory,
    const char *runtime_directory,
    const egakium_mcp_stdio_exec_identity_t *primary,
    const egakium_mcp_stdio_exec_identity_t *helpers,
    size_t helper_count,
    const egakium_mcp_stdio_network_policy_t *network_policy,
    egakium_mcp_stdio_guard_spawn_result_t *result) {
    if (result == NULL) {
        return EGAKIUM_MCP_STDIO_GUARD_UNAVAILABLE;
    }
    ...
    if (wrapper_path == NULL || argv == NULL || envp == NULL ||
        ...
        (network_policy->mode != EGAKIUM_MCP_STDIO_NETWORK_DENIED &&
         network_policy->mode
            != EGAKIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY) ||
        (network_policy->mode
            == EGAKIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY &&
        ...
        (network_policy->mode
            == EGAKIUM_MCP_STDIO_NETWORK_DENIED &&
         (network_policy->gateway_ipv4_network_order != 0 ||
          network_policy->gateway_port_host_order != 0)) ||
        EGAKIUM_AUDIT_ARCH == 0) {
    ...
    if (egakium_pipe_cloexec(stdin_pipe) < 0 ||
        egakium_pipe_cloexec(stdout_pipe) < 0 ||
        egakium_pipe_cloexec(stderr_pipe) < 0) {
        result->error_number = errno;
        egakium_close_fd(&stdin_pipe[0]);
        egakium_close_fd(&stdin_pipe[1]);
        egakium_close_fd(&stdout_pipe[0]);
        egakium_close_fd(&stdout_pipe[1]);
        egakium_close_fd(&stderr_pipe[0]);
        egakium_close_fd(&stderr_pipe[1]);
        return EGAKIUM_MCP_STDIO_GUARD_PIPE_FAILED;
    }
    int seccomp_fd =
        egakium_make_seccomp_fd(runtime_directory, helper_count > 0);
    if (seccomp_fd < 0) {
        result->error_number = errno;
        egakium_close_fd(&stdin_pipe[0]);
        egakium_close_fd(&stdin_pipe[1]);
    ...
    egakium_mcp_stdio_guard_t *guard =
        (egakium_mcp_stdio_guard_t *)calloc(1, sizeof(*guard));
    if (guard == NULL ||
        egakium_capture_owned_identity(
            wrapper_path, &guard->wrapper) < 0 ||
        egakium_copy_identity(primary, &guard->primary) < 0) {
        result->error_number = errno;
        egakium_close_fd(&seccomp_fd);
        egakium_close_fd(&stdin_pipe[0]);
        egakium_close_fd(&stdin_pipe[1]);
        egakium_close_fd(&stdout_pipe[0]);
        egakium_close_fd(&stdout_pipe[1]);
        egakium_close_fd(&stderr_pipe[0]);
        egakium_close_fd(&stderr_pipe[1]);
        egakium_mcp_stdio_guard_destroy(guard);
        return EGAKIUM_MCP_STDIO_GUARD_IDENTITY_FAILED;
    }
    guard->network_policy = *network_policy;
    if (helper_count > 0) {
        guard->helpers = (egakium_owned_identity_t *)calloc(
            helper_count, sizeof(egakium_owned_identity_t));
    ...
    if (child < 0) {
        ...
        egakium_close_fd(&stderr_pipe[1]);
        egakium_mcp_stdio_guard_destroy(guard);
        return EGAKIUM_MCP_STDIO_GUARD_FORK_FAILED;
    }
    if (child == 0) {
        egakium_guarded_child(
            wrapper_path,
            argv,
            envp,
            working_directory,
    ...
    if (egakium_add_tracee(guard, child, 1) < 0) {
        result->error_number = ENOMEM;
        (void)kill(-child, SIGKILL);
        (void)kill(child, SIGKILL);
        (void)waitpid(child, NULL, 0);
        ...
        egakium_close_fd(&stderr_pipe[0]);
        egakium_mcp_stdio_guard_destroy(guard);
        return EGAKIUM_MCP_STDIO_GUARD_TRACE_FAILED;
    }
    ...
    if (egakium_read_handshake_byte(startup_pair[0]) < 0 ||
        getpgid(child) != child ||
        ptrace(
            PTRACE_SEIZE,
            child,
        ...
        egakium_set_nonblocking(stdin_pipe[1]) < 0 ||
        egakium_set_nonblocking(stdout_pipe[0]) < 0 ||
        egakium_set_nonblocking(stderr_pipe[0]) < 0 ||
        egakium_write_handshake_byte(startup_pair[0]) < 0) {
        result->error_number = errno;
        (void)kill(-child, SIGKILL);
        (void)kill(child, SIGKILL);
    ...
    result->guard = guard;
    return EGAKIUM_MCP_STDIO_GUARD_OK;
}

int egakium_mcp_stdio_guard_poll(
    egakium_mcp_stdio_guard_t *guard,
    egakium_mcp_stdio_guard_poll_result_t *result) {
    ...
    do {
        ...
        for (size_t index = 0; index < observed_count; index++) {
            ...
            if (waited < 0) {
                ...
                if (errno == ECHILD) {
                    egakium_mark_violation(guard, ECHILD);
                    continue;
                }
                egakium_mark_violation(guard, errno);
                continue;
            }
            made_progress = 1;
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
            ...
            if (event == PTRACE_EVENT_EXEC) {
                if (egakium_handle_exec_event(guard, tracee) < 0) {
                    egakium_mark_violation(guard, EACCES);
                }
            } else if (event == PTRACE_EVENT_SECCOMP) {
                if (egakium_freeze_validate_and_execute_network(
    ...
    if (!guard->target_seen &&
        egakium_all_tracees_exited(guard)) {
        guard->target_exited = 1;
    ...
    result->all_tracees_exited = egakium_all_tracees_exited(guard);
    ...
    return guard->violation
        ? EGAKIUM_MCP_STDIO_GUARD_POLICY_VIOLATION
        : EGAKIUM_MCP_STDIO_GUARD_OK;
}

void egakium_mcp_stdio_guard_destroy(
    egakium_mcp_stdio_guard_t *guard) {
    if (guard == NULL) {
        return;
    }
