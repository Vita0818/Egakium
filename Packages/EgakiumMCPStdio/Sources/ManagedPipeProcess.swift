...
public actor ManagedPipeProcess: Transport {
    ...
    private init(
        ...
        self.logger = Logger(
            label: "com.vitemis.egakium.mcp.stdio.\(ticket.request.authority.server.serverID.rawValue)")
    ...
    @unchecked Sendable
{
    ...
    func poll() -> MCPStdioLinuxGuardObservation {
        ...
        var result = egakium_mcp_stdio_guard_poll_result_t()
        let status = egakium_mcp_stdio_guard_poll(
            pointer,
            &result)
        return MCPStdioLinuxGuardObservation(
    ...
    deinit {
        if let pointer {
            egakium_mcp_stdio_guard_destroy(pointer)
            self.pointer = nil
        }
    }
}
...
private func spawnLinuxGuardedPipeProcess(
    ...
    var helperIdentities = helpers.map { helper in
        var value = egakium_mcp_stdio_exec_identity_t()
    ...
    guard status == EGAKIUM_MCP_STDIO_GUARD_OK,
          result.process_id > 0,
        ...
        if let guardPointer = result.guard {
            egakium_mcp_stdio_guard_destroy(guardPointer)
        }
        if status == EGAKIUM_MCP_STDIO_GUARD_UNAVAILABLE
            || status
                == EGAKIUM_MCP_STDIO_GUARD_TRACE_FAILED {
            throw MCPManagedPipeError
                .descendantExecutionGuardUnavailable
        }
