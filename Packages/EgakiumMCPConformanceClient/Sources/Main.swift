...
@main
private enum EgakiumMCPConformanceClient {
    static func main() async {
        do {
            ...
            FileHandle.standardError.write(
                Data("Egakium MCP conformance client failed: \(summary)\n".utf8))
            exit(EXIT_FAILURE)
        }
    }
}
...
private struct Runner {
    ...
    func run() async throws {
        ...
        case "egakium/task-complete":
            try await runTaskAugmented(.complete)
        case "egakium/task-timeout":
            try await runTaskAugmented(.timeout)
        case "egakium/task-cancel":
    ...
    private func makeSession(
        ...
        return MCPClientSession(
            configuration: MCPClientSessionConfiguration(
                ...
                clientName: "egakium-conformance-client",
                clientVersion: "1.0.0",
    ...
    static func make() async throws -> ConformanceResources {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "egakium-mcp-conformance-\(UUID().uuidString)",
                isDirectory: true)
        ...
        try await store.initialize(
            passphrase: Data(
                "egakium-conformance-disposable-key".utf8))
...
private struct OAuthResources {
    ...
    static func make(
        ...
        try await store.initialize(
            passphrase: Data(
                "egakium-oauth-conformance-key".utf8))
