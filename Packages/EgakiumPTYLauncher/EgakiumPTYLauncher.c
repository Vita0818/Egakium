...
} EgakiumPTYChildError;

static int egakium_set_cloexec(int descriptor) {
...
static int egakium_maximum_descriptor(void) {
    struct rlimit limit;
...
static void egakium_report_child_error(
    int descriptor,
    int32_t stage,
    int32_t error_number,
    int exit_code) {
    EgakiumPTYChildError payload = {
        .stage = stage,
...
static void egakium_reset_child_signals(void) {
    sigset_t empty_mask;
...
static void egakium_kill_and_reap(pid_t pid) {
...
int32_t egakium_spawn_managed_pty(
    ...
    *result = (EgakiumPTYSpawnResult){
        .pid = -1,
        .master_fd = -1,
        .error_stage = EGAKIUM_PTY_STAGE_PREPARE,
        .error_number = 0
    };
    ...
    int pipe_error = egakium_set_cloexec(error_pipe[0]);
    if (pipe_error == 0) {
        pipe_error = egakium_set_cloexec(error_pipe[1]);
    }
    if (pipe_error != 0) {
    ...
    int maximum_descriptor = egakium_maximum_descriptor();
    ...
    if (pid == 0) {
        ...
        if (error_descriptor != 3) {
            if (dup2(error_descriptor, 3) == -1) {
                egakium_report_child_error(
                    error_descriptor,
                    EGAKIUM_PTY_STAGE_PREPARE,
                    errno,
                    126);
            }
        ...
        int cloexec_error = egakium_set_cloexec(error_descriptor);
        if (cloexec_error != 0) {
            egakium_report_child_error(
                error_descriptor,
                EGAKIUM_PTY_STAGE_PREPARE,
        ...
        egakium_report_child_error(
            error_descriptor,
            EGAKIUM_PTY_STAGE_EXEC,
            errno,
            127);
    }

    (void)close(error_pipe[1]);
    EgakiumPTYChildError child_error = {
        .stage = EGAKIUM_PTY_STAGE_NONE,
        .error_number = 0
    };
    ...
    int master_error = egakium_set_cloexec(master_descriptor);
    if (master_error != 0) {
        (void)close(master_descriptor);
        egakium_kill_and_reap(pid);
        return master_error;
    ...
    result->error_stage = EGAKIUM_PTY_STAGE_NONE;
