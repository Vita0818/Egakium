...
@testable import EgakiumAgentKernel
...
final class ContextProjectionTests: XCTestCase {
    ...
    func testCoworkSystemPromptDoesNotEmbedDynamicIdentityOrWorkspaceData() {
        ...
        XCTAssertTrue(prompt.contains("You are operating in an Egakium Cowork session."))
    ...
    func testCoordinatorPromptRequiresExactBundledOrchestrationSkill() {
        ...
        XCTAssertTrue(coordinator.contains(
            "`\(EgakiumBundledSkills.coworkAgentOrchestrationName)`"))
        ...
        XCTAssertTrue(coordinator.contains("Inspect the bounded EGAKIUM_SKILL_CATALOG"))
    ...
    func testExactMainCanvasPromptNamesHostCreatedEditableIndex() throws {
        let session = SessionID(rawValue: "cowork_canvas_prompt")
        let prompt = ContextBuilder.coworkSystemPrompt(
            name: "main",
            folder: "/workspace",
            coordinationDepth: 1,
            canCoordinate: true,
            editableCanvasSessionID: session)

        XCTAssertTrue(prompt.contains(
            "`.egakium/canvas/cowork_canvas_prompt/index.html`"))
        XCTAssertTrue(prompt.contains("host-created, directly editable HTML Canvas"))
        XCTAssertTrue(prompt.contains("one direct `.egakium-element` card"))
        XCTAssertTrue(prompt.contains("`data-element-id`"))
        XCTAssertTrue(prompt.contains("`data-x`"))
        XCTAssertTrue(prompt.contains("`data-y`"))
        XCTAssertTrue(prompt.contains("`data-width`"))
        XCTAssertTrue(prompt.contains("`data-height`"))
        XCTAssertTrue(prompt.contains("`elements/<ElementID>/index.html`"))
        XCTAssertTrue(prompt.contains("`sandbox=\"allow-scripts\"`"))
        XCTAssertTrue(prompt.contains("selection, drag, resize"))
        XCTAssertTrue(prompt.contains(
            "source attributes in `index.html` remain authoritative"))
        XCTAssertTrue(prompt.contains(
            "multiple ordinary sub-agents to edit it concurrently"))
        XCTAssertTrue(prompt.contains(
            "Every successful spawn_agent admission automatically provisions"))
        XCTAssertTrue(prompt.contains("The host chooses its"))
        XCTAssertTrue(prompt.contains("canvas_element_id"))
        XCTAssertTrue(prompt.contains("canvas_element_path"))
        XCTAssertTrue(prompt.contains("canvas_template_version"))
        XCTAssertTrue(prompt.contains("a read_only agent still"))
        XCTAssertTrue(prompt.contains(
            "agent recycling does not delete the document"))
        XCTAssertTrue(prompt.contains(
            "not permanent Agent-to-Element ownership"))
        XCTAssertFalse(prompt.contains(
            "EGAKIUM_GENERIC_ELEMENT_TEMPLATE"))

        let provisioned = SessionCanvasElementDescriptor(
            elementID: CanvasElementID(rawValue: "element_worker_001"),
            relativeIndexPath:
                ".egakium/canvas/cowork_canvas_prompt/elements/element_worker_001/index.html")
        let provisionedWorker = ContextBuilder.coworkSystemPrompt(
            name: "worker",
            folder: "/workspace",
            coordinationDepth: 0,
            canCoordinate: false,
            provisionedCanvasElement: provisioned,
            includesGenericCanvasElementTemplate: true)
        XCTAssertTrue(provisionedWorker.contains(
            "one real, host-provisioned generic Canvas element"))
        XCTAssertTrue(provisionedWorker.contains("`element_worker_001`"))
        XCTAssertTrue(provisionedWorker.contains(
            "`.egakium/canvas/cowork_canvas_prompt/elements/element_worker_001/index.html`"))
        XCTAssertTrue(provisionedWorker.contains("This is a workspace file"))
        XCTAssertTrue(provisionedWorker.contains("provenance, not"))
        XCTAssertFalse(provisionedWorker.contains(
            "EGAKIUM_GENERIC_ELEMENT_TEMPLATE"))
        ...
        let worker = ContextBuilder.coworkSystemPrompt(
            ...
            editableCanvasSessionID: session,
            includesGenericCanvasElementTemplate: true)
        XCTAssertFalse(worker.contains(".egakium/canvas/"))
        XCTAssertFalse(worker.contains("directly editable HTML Canvas"))
        XCTAssertTrue(worker.contains(
            "<EGAKIUM_GENERIC_ELEMENT_TEMPLATE version=\"1\">"))
        XCTAssertTrue(worker.contains(SessionCanvasElementTemplate.html))
        XCTAssertTrue(worker.contains(
            "This prompt copy is not a workspace file"))
        XCTAssertTrue(worker.contains(
            "If that destination already exists"))
        XCTAssertFalse(worker.contains("data-element-id"))

        let workerWithoutTemplate = ContextBuilder.coworkSystemPrompt(
            name: "worker",
            folder: "/workspace",
            coordinationDepth: 0,
            canCoordinate: false)
        XCTAssertFalse(workerWithoutTemplate.contains(
            "EGAKIUM_GENERIC_ELEMENT_TEMPLATE"))
    }
    ...
    func testFirstAgentRequestDeclaresEgakiumRuntimeAndToolProtocol() {
        for (runtime, modeName) in [
            ...
            XCTAssertTrue(systemPrompt.contains("running inside Egakium"), modeName)
    ...
    func testRuntimeProjectsSkillCatalogAsDeveloperAndExplicitBodyAsUser() async throws {
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "egakium-skill-context-\(UUID().uuidString)",
                isDirectory: true)
        ...
        XCTAssertTrue(system.contains("EGAKIUM_SKILL_CATALOG"))
        ...
        async throws
    {
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "egakium-skill-rejection-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: workspace) }
    ...
    func testWorkerPromptDoesNotReplayOriginalSpawnInstructionAsFreshUserMessage() async throws {
        let ws = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-context-\(UUID().uuidString)", isDirectory: true)
