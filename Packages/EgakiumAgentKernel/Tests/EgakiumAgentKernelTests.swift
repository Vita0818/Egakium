...
private struct PartialThenFailingToolProvider: ToolCallingProvider {
    func stream(_ request: AgentRequest) -> AsyncThrowingStream<AgentChunk, Error> {
        AsyncThrowingStream { continuation in
            ...
            continuation.finish(throwing: EgakiumError.decoding(
...
private struct SecretEchoFailingProvider: ToolCallingProvider {
    ...
    func stream(_ request: AgentRequest) -> AsyncThrowingStream<AgentChunk, Error> {
        AsyncThrowingStream { continuation in
            continuation.finish(throwing: EgakiumError.provider(
...
final class EgakiumAgentKernelTests: XCTestCase {
    ...
    private func workspaceAndLog() throws -> (URL, EventLog) {
        let ws = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-kernel-\(UUID().uuidString)", isDirectory: true)
    ...
    func testAgentLoopRunsBrowserSearchThroughPermissionFlow() async throws {
        ...
        let searchArgs = """
        {"query":"Egakium browser tools","engine":"duckduckgo","profile":"agent-web","channel":"msedge","waitMillis":0,"maxCharacters":2000}
        """
        let provider = ScriptedProvider([
        ...
        let browserStdout = #"{"action":"search","profile":"agent-web","backend":"cdp","backendDetail":"edge","url":"https://duckduckgo.com/?q=Egakium%20browser%20tools","title":"Search Results","text":"Search results for Egakium browser tools","links":[{"text":"Egakium browser tool docs","href":"https://example.com/egakium-browser"}],"elements":[{"role":"textbox","name":"Search","selector":"input[name=q]","tag":"input","type":"search"}]}"#
        ...
        let answer = try await loop.send("search the web for Egakium browser tools")
        ...
        XCTAssertTrue(result.observation.contains("Egakium browser tool docs - https://example.com/egakium-browser"), result.observation)
        XCTAssertTrue(result.observation.contains(#"textbox "Search" selector=input[name=q] type=search"#), result.observation)

        let stateURL = ws.appendingPathComponent(".egakium/browser/state/agent-web.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
        XCTAssertEqual(state["url"] as? String, "https://duckduckgo.com/?q=Egakium%20browser%20tools")
    ...
    func testAgentLoopCompletesBrowserFormTaskThroughPermissionFlow() async throws {
        ...
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/agent-task.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
    ...
    func testAgentLoopBrowsesDynamicFeedThroughPermissionFlow() async throws {
        ...
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/social-feed.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
    ...
    func testAgentLoopRunsBrowserProfileDeleteThroughDestructivePermissionFlow() async throws {
        ...
        XCTAssertTrue(result.observation.contains("removed profile data: yes (.egakium/browser/profiles/work)"), result.observation)
        XCTAssertTrue(result.observation.contains("removed state file: yes (.egakium/browser/state/work.json)"), result.observation)
        XCTAssertTrue(result.observation.contains("removed downloads: yes (.egakium/browser/downloads/work)"), result.observation)
    ...
    func testUnknownToolArgumentsDoNotRequestPermissionOrExecuteTool() async throws {
        ...
        XCTAssertEqual(durableCall.args, #"{"_egakium":"arguments_redacted"}"#)
    ...
    func testUnknownToolNameAndArgumentsAreRedactedBeforeEventLog() async throws {
        ...
        XCTAssertEqual(call.args, #"{"_egakium":"arguments_redacted"}"#)
