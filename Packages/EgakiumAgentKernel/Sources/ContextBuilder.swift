...
public struct RuntimeEnvironmentManifest: Equatable, Sendable {
    ...
    fileprivate var systemPrompt: String {
        var prompt = """
        You are running inside Egakium, an Apple-first local AI workbench, in \(mode.rawValue) mode.
        Egakium gives you model-visible tools for workspace, network, browser, document, Git, goal, task, message, and agent operations when the current lease allows them.
        ...
        Treat a tool action as completed only after receiving its ToolResult. Permission, scheduling, persistence, recovery, WorkTask readiness, and terminal state are owned by Egakium.
...
public struct ContextBuilder: Sendable {
    ...
    You are an Egakium coding agent working inside a single local workspace.
    ...
    public static func coworkSystemPrompt(name _: String,
                                          ...
                                          canCoordinate: Bool? = nil,
                                          editableCanvasSessionID: SessionID? = nil,
                                          provisionedCanvasElement:
                                            SessionCanvasElementDescriptor? = nil,
                                          includesGenericCanvasElementTemplate:
                                            Bool = false) -> String {
        var prompt = defaultSystemPrompt + "\n\nYou are operating in an Egakium Cowork session."
        if canCoordinate ?? (coordinationDepth > 0) {
            ...
            Inspect the bounded EGAKIUM_SKILL_CATALOG before planning or acting. Proactively
            ...
            `\(EgakiumBundledSkills.coworkAgentOrchestrationName)` within the current system,
            ...
            contains its non-rejected EGAKIUM_ACTIVATED_SKILLS block, do not activate it again.
            ...
            Prefer delegate_task for each concrete WorkTask: Egakium can select an existing idle
        ...
        if (canCoordinate ?? (coordinationDepth > 0)),
           let editableCanvasSessionID,
           let relativePath = try? SessionCanvasStore.relativeIndexPath(
               for: editableCanvasSessionID) {
            prompt += """


            You are the exact @main coordinator for an Egakium Session that has a
            host-created, directly editable HTML Canvas. Its workspace-relative entry
            file is `\(relativePath)`. The host creates this file deterministically when
            the Session is initialized; do not ask the user or another model to create
            the initial file.

            For user requests that affect the Canvas, inspect this exact `index.html`
            before editing it, then use only the advertised workspace tools and normal
            permission flow to update it. You may directly change its complete HTML,
            CSS, and JavaScript. Keep it a valid standalone HTML document and preserve the
            direct `#canvas` element-container boundary.

            Represent each child HTML document as one direct `.egakium-element` card under
            `#canvas`. Every card must have one stable, safe `data-element-id`, a bounded
            human-readable `data-element-title`, and numeric `data-x`, `data-y`,
            `data-width`, and `data-height` source-layout attributes. Put the child document
            in a relative iframe URL beneath this same Session Canvas directory, normally
            `elements/<ElementID>/index.html`, and keep `sandbox="allow-scripts"`. The host
            presentation runtime adds card chrome, selection, drag, resize, keyboard
            movement, and a non-persistent user layout override. Do not implement a second
            competing drag framework or treat Web storage/runtime DOM changes as durable
            layout truth; the source attributes in `index.html` remain authoritative until
            a versioned host layout projection is introduced.

            Every successful spawn_agent admission automatically provisions one fresh generic
            element document in the new agent's exact workspace root. The host chooses its
            ElementID and creates the file with no-overwrite semantics; do not invent either
            identity before the call succeeds. Wait for the successful ToolResult, which returns
            `canvas_element_id`, `canvas_element_path`, and `canvas_template_version`, before
            creating a card or referring to that resource in a later tool-call round. Request
            read_write only when the new agent must edit its element; a read_only agent still
            receives the host-created document but cannot modify it. When the new agent shares
            this Canvas workspace root, convert its workspace-relative element path to the iframe
            URL beneath this Session Canvas directory. A distinct workspace remains bounded by
            its own lease and must not be treated as silently readable here. The provisioned
            document is a fresh task resource, not permanent Agent-to-Element ownership; later
            tasks may reassign either side, and agent recycling does not delete the document.

            This shared `index.html` is coordinated by exact @main. Do not arrange for
            multiple ordinary sub-agents to edit it concurrently. If delegation helps,
            give sub-agents distinct supporting files or element-document paths, wait for
            their successful ToolResults, and integrate or reference those results from
            `index.html` yourself. Never infer authority from the Canvas path: all file,
            tool, lease, permission, task, and causal ToolResult rules still apply.
            """
        }
        if let provisionedCanvasElement {
            prompt += """


            This agent was created with one real, host-provisioned generic Canvas element
            document already present in its exact workspace root:

            - ElementID: `\(provisionedCanvasElement.elementID.rawValue)`
            - Workspace-relative document: `\(provisionedCanvasElement.relativeIndexPath)`
            - Template version: `\(provisionedCanvasElement.templateVersion)`

            This is a workspace file, not a prompt-only example. Inspect it before element work
            and preserve any existing progress; do not recreate, reset, move, or rename it merely
            because a new task begins. Its initial generic structure uses `#element` and `#content`
            as integration boundaries, while the outer Session Canvas owns card chrome, position,
            size, selection, and drag/resize. Your current WorkspaceLease and advertised tools
            remain the only authority to edit it. The spawn-time association is provenance, not
            permanent ownership, and does not prevent later reassignment.
            """
        } else if includesGenericCanvasElementTemplate {
            prompt += """


            The host provides the following immutable generic starting template for a fresh
            Canvas element document. This prompt copy is not a workspace file, does not name
            an ElementID or destination, and grants no write authority. Use it only when the
            current task establishes one exact element-document path and the advertised tools
            plus WorkspaceLease allow the required write. If that destination already exists,
            inspect and preserve its current work instead of resetting it to this template.
            The outer Session Canvas owns card chrome, position, size, selection, and
            drag/resize; this child document owns only its internal content. Preserve
            `#element` and `#content` as integration boundaries while adapting the rest to the
            assigned deliverable.

            <EGAKIUM_GENERIC_ELEMENT_TEMPLATE version="\(SessionCanvasElementTemplate.templateVersion)">
            \(SessionCanvasElementTemplate.html)
            </EGAKIUM_GENERIC_ELEMENT_TEMPLATE>
            """
        }
        return prompt
    }
    ...
    A later developer-role EGAKIUM_SKILL_CATALOG block describes the bounded
    Skills visible to this invocation. A user-role EGAKIUM_ACTIVATED_SKILLS
