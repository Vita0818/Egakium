...
public struct CoworkProjection: Equatable, Sendable {
    ...
    /// Active spawn-scoped generic Canvas resources. This map intentionally
    /// describes provisioning only; it is not an Element ownership registry.
    public private(set) var provisionedCanvasElements:
    ...
    public mutating func apply(_ envelope: Envelope) {
        ...
        case .agentSpawned(let payload):
            ...
            if let element = payload.provisionedCanvasElement,
               (try? SessionCanvasStore.validateElementDescriptor(
                    element,
                    sessionID: envelope.session)) != nil {
                provisionedCanvasElements[payload.agent] = element
            }
        case .agentDetached(let payload):
            ...
            provisionedCanvasElements.removeValue(forKey: payload.agent)
