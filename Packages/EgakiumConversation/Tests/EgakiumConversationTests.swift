...
final class EgakiumConversationTests: XCTestCase {

    private func tmpFile() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-conv-\(UUID().uuidString)", isDirectory: true)
        ...
        let loop = ChatLoop(
            ...
            attachmentResolver: { ids in
                guard ids == [firstID] else {
                    throw EgakiumError.notFound("unexpected attachment set")
                }
                return [firstImage]
            })
