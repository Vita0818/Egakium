...
@testable import EgakiumConversation
...
final class CoworkProjectionRegressionTests: XCTestCase {
    ...
    func testSpawnProjectionCarriesValidatedElementUntilAgentDetaches() throws {
        let descriptor = SessionCanvasElementDescriptor(
            elementID: CanvasElementID(rawValue: "element_worker_001"),
            relativeIndexPath:
                ".egakium/canvas/sess_cowork_projection/elements/element_worker_001/index.html")
        let spawned = AgentSpawnedPayload(
            requestedBy: main,
            agent: worker,
            path: "/workspace/worker",
            model: ModelID(rawValue: "model"),
            provisionedCanvasElement: descriptor)

        let active = CoworkProjection.build(from: [
            envelope(0, .agentSpawned(spawned)),
        ])
        XCTAssertEqual(active.provisionedCanvasElements[worker], descriptor)

        let detached = CoworkProjection.build(from: [
            envelope(0, .agentSpawned(spawned)),
            envelope(1, .agentDetached(.init(agent: worker))),
        ])
        XCTAssertNil(detached.provisionedCanvasElements[worker])
    }

    func testSpawnProjectionRejectsNonCanonicalElementDescriptor() {
        let invalid = SessionCanvasElementDescriptor(
            elementID: CanvasElementID(rawValue: "element_worker_001"),
            relativeIndexPath: "../outside/index.html")
        let projection = CoworkProjection.build(from: [
            envelope(0, .agentSpawned(.init(
                requestedBy: main,
                agent: worker,
                path: "/workspace/worker",
                model: ModelID(rawValue: "model"),
                provisionedCanvasElement: invalid))),
        ])

        XCTAssertNil(projection.provisionedCanvasElements[worker])
    }
