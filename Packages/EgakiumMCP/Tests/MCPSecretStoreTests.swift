...
final class MCPSecretStoreTests: XCTestCase {
        ...
        async throws
    {
        ...
        let service =
            "com.vitemis.egakium.mcp.tests.\(UUID().uuidString)"
    ...
    func testMacOSProductionKeychainQueryUsesDataProtectionAndAccessClass() {
        let query = MCPKeychainSecretStore.makeAddQuery(
            service: "com.vitemis.egakium.mcp.tests.query",
            account: "mcp:test",
            accessGroup: nil,
    ...
    private func requireHostKeychainIntegration() throws {
        guard ProcessInfo.processInfo.environment[
            "EGAKIUM_RUN_HOST_KEYCHAIN_TESTS"
        ] == "1" else {
            throw XCTSkip(
                "Set EGAKIUM_RUN_HOST_KEYCHAIN_TESTS=1 in an unsandboxed, "
    ...
    private func makeFixture() throws -> (root: URL, store: URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "egakium-mcp-secret-store-\(UUID().uuidString)",
                isDirectory: true)
