...
final class ThreadScrollCoordinatorTests: XCTestCase {
    private let codeA = EgakiumThreadPresentationScope(
        kind: "code",
        sessionID: "code-a")
    private let codeB = EgakiumThreadPresentationScope(
        kind: "code",
        sessionID: "code-b")
    private let coworkA = EgakiumThreadPresentationScope(
        kind: "cowork",
        sessionID: "cowork-a")
    ...
    func testScopeAndBottomAnchorAreSessionSpecific() {
        ...
        XCTAssertNotEqual(
            EgakiumThreadBottomAnchorID(scope: codeA),
            EgakiumThreadBottomAnchorID(scope: codeB))
    }
    ...
    func testBottomVisibilityDoesNotUseGeometryPreferences() throws {
        ...
        let surfaces = try String(
            ...
            encoding: .utf8)
        XCTAssertFalse(
            surfaces.contains("EgakiumThreadViewportFramesPreferenceKey"))
    ...
    func testScopeChangeCancelsPendingRequest() async {
        ...
        var executed: [EgakiumThreadScrollRequest] = []
    ...
    func testLazyEntrySuspendsRichUntilRawBottomAnchorIsVisible() async {
        ...
        var admissionInsideExecutor: EgakiumMessageViewportAdmission?
    ...
    func testCompletionCanReplaceInitialRestoreWithoutLeavingRichSuspended() async {
        let coordinator = EgakiumThreadScrollCoordinator()
        var executed: [EgakiumThreadScrollReason] = []
        coordinator.activate(
            scope: codeA,
            defersRichUntilInitialRestore: true)
    ...
    func testRapidABARestoreOnlyFinalScopeCanReleaseRichAdmission() async {
        let coordinator = EgakiumThreadScrollCoordinator()
        var executedScopes: [EgakiumThreadPresentationScope] = []
    ...
    func testLateRichCommitCannotCrossScopeOrABAActivation() async throws {
        let coordinator = EgakiumThreadScrollCoordinator()
        var executions: [EgakiumThreadPresentationScope] = []
    ...
    func testFiveHundredUpdatesUseFixedWindowLeadingTrailingCadence() {
        var model = EgakiumThreadScrollCadenceModel()
        var executed: [EgakiumThreadScrollRequest] = []
    ...
    func testCadenceDeadlineIsNotResetByEveryUpdate() {
        ...
        XCTAssertEqual(
            ...
            EgakiumThreadScrollCadenceModel.intervalNanoseconds)
        XCTAssertEqual(
    ...
    func testWindowLocalCoordinatorsDoNotCancelEachOther() async {
        let firstWindow = EgakiumThreadScrollCoordinator()
        let secondWindow = EgakiumThreadScrollCoordinator()
    ...
    func testGeometryUsesToleranceAndIgnoresSubpixelHeightJitter() {
        let atBottom = EgakiumThreadScrollGeometry.measure(
            contentOffsetY: 476,
            containerHeight: 500,
            bottomInset: 0,
            contentHeight: 1_000)
        ...
        let previous = EgakiumThreadScrollGeometry(
            isAtBottom: true,
            contentHeight: 1_000)
        let jitter = EgakiumThreadScrollGeometry(
            isAtBottom: false,
            contentHeight: 1_000.5)
        let materialGrowth = EgakiumThreadScrollGeometry(
            isAtBottom: false,
            contentHeight: 1_002)
        ...
        XCTAssertEqual(previous, EgakiumThreadScrollGeometry(
            isAtBottom: true,
            contentHeight: 9_999))
    ...
    func testMacRichTranscriptSurfacesUseBoundedEagerWindows() throws {
        ...
        for sourceURL in sources {
            ...
            XCTAssertTrue(
                ...
                sourceURL.lastPathComponent)
            XCTAssertTrue(
                transcript.contains("EgakiumThreadHistoryPager("),
                sourceURL.lastPathComponent)
            ...
            XCTAssertFalse(
                transcript.contains("EgakiumAdaptiveThreadStack("),
                sourceURL.lastPathComponent)
            XCTAssertFalse(
    ...
    func testRichSettleClosesBeforeReturningSingleSettlement() {
        var model = EgakiumThreadRichSettleModel()
        let token = EgakiumThreadRichSettleToken.finalDocument(
            messageID: "message",
    ...
    func testRichSettleWaitsForQuietButHonorsHardCap() {
        var model = EgakiumThreadRichSettleModel()
        let token = EgakiumThreadRichSettleToken.width(900_000)
        _ = model.open(
            token: token,
            contentHeight: 1_000,
            nowNanoseconds: 0)
    ...
    func testScrollDiagnosticReasonsPreserveSemanticSource() {
        XCTAssertEqual(
            EgakiumThreadScrollReason.initialRestore.diagnosticReason.rawValue,
            EgakiumScrollDiagnosticReason.initialRestore.rawValue)
        XCTAssertEqual(
            EgakiumThreadScrollReason.liveUpdate.diagnosticReason.rawValue,
            EgakiumScrollDiagnosticReason.liveContent.rawValue)
        XCTAssertEqual(
            EgakiumThreadScrollReason.completion.diagnosticReason.rawValue,
            EgakiumScrollDiagnosticReason.completion.rawValue)
        XCTAssertEqual(
            EgakiumThreadScrollReason.richHeightCorrection
                .diagnosticReason.rawValue,
            EgakiumScrollDiagnosticReason.richSettle.rawValue)
        XCTAssertEqual(
            EgakiumThreadScrollReason.jumpToLatest.diagnosticReason.rawValue,
            EgakiumScrollDiagnosticReason.manualJump.rawValue)
    }
    ...
    private func fireDueRequests(
        model: inout EgakiumThreadScrollCadenceModel,
        through now: UInt64,
        executed: inout [EgakiumThreadScrollRequest]
    ) {
