...
private func waitForMarkdownSchedulerSnapshot<Key: Hashable & Sendable>(
    _ scheduler: EgakiumLatestOnlyPermitScheduler<Key>,
    attempts: Int = 2_000,
    matching predicate: @Sendable (EgakiumLatestOnlyPermitSchedulerSnapshot<Key>) -> Bool
) async throws -> EgakiumLatestOnlyPermitSchedulerSnapshot<Key> {
    for _ in 0..<attempts {
...
final class MarkdownSchedulerTests: XCTestCase {
    func testRejectsNonPositiveBounds() {
        XCTAssertThrowsError(try EgakiumLatestOnlyPermitScheduler<String>(
            maxConcurrentPermits: 0,
            maxPendingAcquires: 1
        )) { error in
            XCTAssertEqual(
                error as? EgakiumLatestOnlyPermitSchedulerConfigurationError,
    ...
    func testGlobalConcurrencyAndPendingCapacityStayBounded() async throws {
        let scheduler = try EgakiumLatestOnlyPermitScheduler<Int>(
    ...
    func testSameKeyKeepsOneRunningAndOneReplaceablePending() async throws {
        let scheduler = try EgakiumLatestOnlyPermitScheduler<String>(
            maxConcurrentPermits: 1,
            maxPendingAcquires: 1)
