...
final class ThreadLayoutTests: XCTestCase {
    func testIOSComposerGlassMergeThresholdStaysBelowRowGap() {
        XCTAssertEqual(EgakiumComposerControlMetrics.rowSpacing, 8)
        XCTAssertEqual(
        ...
        XCTAssertLessThan(
            EgakiumComposerControlMetrics.glassEffectSpacing(for: .iOS),
            EgakiumComposerControlMetrics.rowSpacing)
    }

    func testComposerUsesPlatformControlSizeWithinSharedFortyPointGeometry() {
        XCTAssertEqual(EgakiumComposerControlMetrics.controlHeight, 40)
        XCTAssertEqual(
            EgakiumComposerControlMetrics.iconControlSize(for: .iOS),
            .small)
        XCTAssertEqual(
            EgakiumComposerControlMetrics.iconControlSize(for: .macOS),
            .regular)
    }

    func testSidebarOpenGestureRequiresLeadingEdgeHorizontalIntent() {
        XCTAssertTrue(EgakiumSidebarGesturePolicy.shouldOpen(
            startX: 8,
            translation: CGSize(width: 72, height: 8)))
        XCTAssertFalse(EgakiumSidebarGesturePolicy.shouldOpen(
            startX: 40,
            translation: CGSize(width: 72, height: 8)))
        XCTAssertFalse(EgakiumSidebarGesturePolicy.shouldOpen(
            startX: 8,
    ...
    func testSidebarCloseGestureRequiresHorizontalLeftIntent() {
        XCTAssertTrue(EgakiumSidebarGesturePolicy.shouldClose(
            translation: CGSize(width: -60, height: 8)))
        XCTAssertFalse(EgakiumSidebarGesturePolicy.shouldClose(
    ...
    func testLeadingAssistantAndAgentRowsUseTheFullAvailableWidth() {
        XCTAssertEqual(
            EgakiumThreadBubbleWidthPolicy.resolve(
                isTrailing: false,
    ...
    func testUserMessagesDoNotRepeatAnIdentityHeader() {
        XCTAssertFalse(EgakiumMessageHeaderPolicy.showsIdentity(for: .user))
        XCTAssertTrue(EgakiumMessageHeaderPolicy.showsIdentity(for: .assistant))
        XCTAssertTrue(EgakiumMessageHeaderPolicy.showsIdentity(for: .agent))
        XCTAssertTrue(EgakiumMessageHeaderPolicy.showsIdentity(for: .system))
    }
    ...
    func testOnlyUserConversationRowsUseNativeLiquidGlassBubble() throws {
        ...
        XCTAssertTrue(sharedMessageRow.contains(
            ".egakiumLiquidGlass(cornerRadius: 10)"))
        XCTAssertFalse(sharedMessageRow.contains(".egakiumContentSurface("))
        ...
        let macChatSource = try String(
            contentsOf: repositoryRoot
                .appendingPathComponent(
                    "Apps/EgakiumMac/Sources/EgakiumChatScreen.swift"),
            encoding: .utf8)
        let macMessageBubble = try sourceSlice(
            macChatSource,
            from: "struct EgakiumMessageBubble: View",
            to: "struct EgakiumComposer: View")
        ...
        XCTAssertFalse(macMessageBubble.contains(".egakiumContentSurface("))
        ...
        XCTAssertFalse(codeBubbleContent.contains(".egakiumContentSurface("))
    ...
    func testThreadErrorsCollectEveryConversationSourceAndLeaveTranscriptClean() throws {
        ...
        let errors = EgakiumThreadErrorPresentation.errors(
            items: items,
            errorTexts: [
                "Voice input failed.",
        ...
        let transcript = EgakiumThreadErrorPresentation.transcriptItems(items)
    ...
    func testThreadErrorCardIsAbsentWhenEveryErrorSourceIsEmpty() {
        ...
        XCTAssertTrue(EgakiumThreadErrorPresentation.errors(
            items: items,
            errorTexts: ["  ", "\n"]).isEmpty)
        XCTAssertEqual(
            EgakiumThreadErrorPresentation.transcriptItems(items),
            items)
        ...
        XCTAssertTrue(EgakiumThreadErrorPresentation.errors(
            ...
            errorTexts: []).isEmpty)
        XCTAssertEqual(
            EgakiumThreadErrorPresentation
    ...
    func testWorkspaceShellsUseOneConditionalRightRailErrorList() throws {
        ...
        XCTAssertTrue(codeSource.contains("EgakiumThreadErrorList("))
        ...
        XCTAssertTrue(coworkSource.contains("EgakiumThreadErrorList("))
    ...
    func testWorkspaceInspectorUsesOnlyTheStableOuterWidth() {
        let hidden = EgakiumWorkspaceInspectorLayoutPolicy.resolve(
            availableWidth: 979,
            isRequested: true,
            activationWidth: 980,
        ...
        XCTAssertEqual(
            hidden,
            EgakiumWorkspaceInspectorLayout(
                isVisible: false,
                threadWidth: 979,
                inspectorWidth: 0))

        let visible = EgakiumWorkspaceInspectorLayoutPolicy.resolve(
            availableWidth: 980,
            isRequested: true,
            activationWidth: 980,
            minimumThreadWidth: 620,
        ...
        let integratedCowork = EgakiumWorkspaceInspectorLayoutPolicy.resolve(
            availableWidth: 980,
            isRequested: true,
            activationWidth: 980,
            minimumThreadWidth: 620,
        ...
        for _ in 0..<10_000 {
            XCTAssertEqual(
                EgakiumWorkspaceInspectorLayoutPolicy.resolve(
                    availableWidth: 980,
                    isRequested: true,
                    activationWidth: 980,
                    minimumThreadWidth: 620,
    ...
    func testCoworkStatusRailAndConversationWidthStayFixedAcrossContentStates() {
        XCTAssertEqual(EgakiumCoworkStatusRailLayoutPolicy.railWidth, 348)
        XCTAssertEqual(EgakiumCoworkStatusRailLayoutPolicy.cardWidth, 318)
        XCTAssertEqual(EgakiumCoworkStatusRailLayoutPolicy.cardSpacing, 18)

        for availableWidth in [CGFloat(980), 1_100, 1_372, 1_800] {
            let emptyConversation =
                EgakiumCoworkStatusRailLayoutPolicy.resolve(
    ...
    func testCoworkStatusRailRenderSnapshotExcludesConversationSelection() {
        ...
        let changedErrorState = CoworkStatusRailRenderSnapshot(
            ...
            goal: nil,
            workTasks: CoworkWorkTaskSummary(),
            errors: EgakiumThreadErrorPresentation.errors(
                items: [],
    ...
    func testWorkspaceInspectorNeverProducesInvalidGeometry() {
        for width in stride(from: CGFloat(-100), through: 2_000, by: 0.5) {
            let layout = EgakiumWorkspaceInspectorLayoutPolicy.resolve(
    ...
    func testWorkspaceThreadsDoNotVendWindowToolbarOrNestedInspectorPreferences() throws {
        ...
        XCTAssertFalse(
            coworkSource.contains(
                "EgakiumGlassEffectGroup(\n                spacing: EgakiumCoworkStatusRailLayoutPolicy"))
        ...
        XCTAssertTrue(surfaceSource.contains(
            "private struct EgakiumClearLiquidGlassBackdrop"))
        XCTAssertTrue(surfaceSource.contains(
        ...
        let appSource = try String(
            contentsOf: repositoryRoot
                .appendingPathComponent("Apps/EgakiumMac/Sources/EgakiumMacApp.swift"),
            encoding: .utf8)
    ...
    func testCanvasHostUsesOnlyTheOfficialCEFView() throws {
        let testFile = URL(fileURLWithPath: #filePath)
        let packageRoot = testFile
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        ...
        XCTAssertTrue(source.contains("EgakiumCEFView"))
        XCTAssertTrue(source.contains("loadCanvasIndexURL"))
        XCTAssertTrue(source.contains("cefView.reloadCanvas()"))
        XCTAssertTrue(source.contains("cefView.closeBrowser()"))
        XCTAssertFalse(source.contains("import WebKit"))
        XCTAssertFalse(source.contains("WKWebView"))
        XCTAssertFalse(source.contains("WKContentWorld"))
        XCTAssertFalse(source.contains("WKNavigationDelegate"))
        XCTAssertFalse(source.contains("SessionCanvasRuntime"))
        XCTAssertFalse(source.contains("directorySignature"))
        XCTAssertFalse(source.contains("Open Canvas"))

        let bridge = try String(
            contentsOf: repositoryRoot.appendingPathComponent(
                "Apps/EgakiumMac/CEF/EgakiumCEFBridge.mm"),
            encoding: .utf8)
        XCTAssertTrue(bridge.contains("CefBrowserHost::CreateBrowser"))
        XCTAssertTrue(bridge.contains("window_info.SetAsChild"))
        XCTAssertTrue(bridge.contains("CefScopedLibraryLoader"))
        XCTAssertTrue(bridge.contains("settings.external_message_pump = true"))
        XCTAssertTrue(bridge.contains("settings.no_sandbox = false"))
        XCTAssertTrue(bridge.contains("egakium://canvas/"))
        XCTAssertTrue(bridge.contains("method_exchangeImplementations"))
        XCTAssertTrue(bridge.contains("bool DoClose"))
        XCTAssertTrue(bridge.contains("[browser_view removeFromSuperview]"))
        XCTAssertFalse(bridge.contains("WKWebView"))
        XCTAssertFalse(bridge.contains("loadFileURL"))
        XCTAssertFalse(bridge.contains("settings.no_sandbox = true"))

        let project = try String(
            contentsOf: repositoryRoot.appendingPathComponent("project.yml"),
            encoding: .utf8)
        XCTAssertTrue(project.contains("Prepare Official CEF Runtime"))
        XCTAssertTrue(project.contains("Embed Official CEF Runtime"))
        XCTAssertTrue(project.contains(
            "SWIFT_OBJC_BRIDGING_HEADER: Apps/EgakiumMac/CEF/EgakiumMac-Bridging-Header.h"))
        XCTAssertTrue(project.contains("libEgakiumCEFHost.a"))
        XCTAssertTrue(project.contains("ARCHS: arm64"))
        XCTAssertFalse(project.contains("INFOPLIST_KEY_NSPrincipalClass"))
    }
