...
private func egakiumNormalizedStatus(_ status: String) -> String {
    status
...
private func egakiumLocalizedRawStatus(_ status: String) -> String {
    switch egakiumNormalizedStatus(status) {
    case "idle": return EgakiumLocalization.string("idle")
    case "active": return EgakiumLocalization.string("active")
    case "running": return EgakiumLocalization.string("running")
    case "thinking": return EgakiumLocalization.string("thinking")
    case "tool": return EgakiumLocalization.string("tool")
    case "assigned": return EgakiumLocalization.string("assigned")
    case "paused": return EgakiumLocalization.string("paused")
    case "pending": return EgakiumLocalization.string("pending")
    case "queued": return EgakiumLocalization.string("queued")
    case "ready": return EgakiumLocalization.string("ready")
    case "mailbox": return EgakiumLocalization.string("mailbox")
    case "completed", "complete", "done":
        return EgakiumLocalization.string("completed")
    case "failed": return EgakiumLocalization.string("failed")
    case "error": return EgakiumLocalization.string("error")
    case "rejected": return EgakiumLocalization.string("rejected")
    case "blocked": return EgakiumLocalization.string("blocked")
    case "budget_limited": return EgakiumLocalization.string("budget limited")
    case "usage_limited": return EgakiumLocalization.string("usage limited")
    case "in_progress", "inprogress":
        return EgakiumLocalization.string("in progress")
    case "cancelled", "canceled":
        return EgakiumLocalization.string("cancelled")
    case "detached", "removed", "cleaned":
        return EgakiumLocalization.string("detached")
    default: return status
    }
}

private func egakiumLocalizedDisplayStatus(_ status: String) -> String {
    switch egakiumNormalizedStatus(status) {
    case "idle": return EgakiumLocalization.string("Idle")
    case "active": return EgakiumLocalization.string("Active")
    case "running": return EgakiumLocalization.string("Running")
    ...
    default:
        let display = status
            ...
            .capitalized
        return EgakiumLocalization.string(display)
    }
}

private func egakiumLocalizedAgentRole(_ role: String) -> String {
    switch role.lowercased() {
    case "main": return EgakiumLocalization.string("main")
    case "coordinator": return EgakiumLocalization.string("coordinator")
    case "worker": return EgakiumLocalization.string("worker")
    case "reviewer": return EgakiumLocalization.string("reviewer")
    default: return role
    }
}
...
public struct CoworkAgentInfo: Identifiable, Equatable, Sendable {
    ...
    public var statusLine: String {
        ...
        if queued > 0 {
            return EgakiumLocalization.format(
                "%@ · %lld queued",
                egakiumLocalizedRawStatus(status),
                Int64(queued))
        }
        if completedTasks > 0 {
            return EgakiumLocalization.format(
        ...
        return egakiumLocalizedRawStatus(status)
    }
    ...
    public var inferenceDisplayLabel: String? {
        ...
        case .unresolved:
            return EgakiumLocalization.string("Inference unavailable")
        case .incompatible:
            return EgakiumLocalization.string("Inference incompatible")
        ...
        if !components.isEmpty {
            ...
            return inferenceResolution == .legacy
                ? EgakiumLocalization.format("Legacy · %@", label)
                : label
        }
        return inferenceResolution == .resolved
            ? EgakiumLocalization.string("Inference resolved")
            : EgakiumLocalization.string("Legacy inference")
    }
...
private enum CoworkInspectorTab: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .agents: return EgakiumLocalization.string("Agents")
        case .tasks: return EgakiumLocalization.string("Tasks")
        case .context: return EgakiumLocalization.string("Context")
        }
    }
}
...
public struct CoworkProjectInfo: Equatable, Sendable {
    ...
    public init(sessionID: String = "",
                ...
                defaultModel: String = EgakiumLocalization.string("current model"),
                defaultPermission: String = EgakiumLocalization.string("reviewed"),
    ...
    func body(content: Content) -> some View {
        content.accessibilityValue(
            selection.selectedAgentID == agentID
                ? EgakiumLocalization.string("Selected")
                : EgakiumLocalization.string("Not selected"))
    }
}
...
public struct CoworkShell: View {
    ...
    public init(threadPage: CoworkAgentThreadPage,
                ...
                sessionTitle: String = EgakiumLocalization.string("Cowork"),
    ...
    private var threadErrors: [EgakiumThreadErrorEntry] {
        EgakiumThreadErrorPresentation.errors(
    ...
    private func content(outerSize: CGSize) -> some View {
        ...
        let inspectorLayout = EgakiumCoworkStatusRailLayoutPolicy.resolve(
        ...
        let primaryScrollerClearance: CGFloat = inspectorLayout.isVisible
            ? EgakiumCoworkStatusRailLayoutPolicy.scrollerClearance
            : 0
            ...
            .overlay(alignment: .leading) {
                threadColumn(
                    ...
                    inspectorIsAvailable: rawWidth
                        >= EgakiumCoworkStatusRailLayoutPolicy.activationWidth,
    ...
    private func threadColumn(layout: EgakiumThreadContentLayout,
    ...
    private func resolvedHeaderActions(
        ...
        if showsCompactActions {
            if let onShowProjectSettings {
                actions.append(EgakiumThreadHeaderAction(
                    title: EgakiumLocalization.string("Project"),
            ...
            if let onAddAgent {
                actions.append(EgakiumThreadHeaderAction(
                    title: EgakiumLocalization.string("Attach"),
        ...
        let inspectorTitle = permissionPinsInspector
            ? EgakiumLocalization.string("Permission Review")
            : inspectorIsVisible
    ...
    private var inspectorColumn: some View {
        ScrollView {
            ...
            VStack(
                alignment: .leading,
                spacing: EgakiumCoworkStatusRailLayoutPolicy.cardSpacing
            ) {
            ...
            .frame(width: EgakiumCoworkStatusRailLayoutPolicy.cardWidth)
            .padding(
                .leading,
                EgakiumCoworkStatusRailLayoutPolicy.leadingInset)
            .padding(
                .trailing,
                EgakiumCoworkStatusRailLayoutPolicy.trailingInset)
            .padding(.vertical, 16)
        }
    ...
    @ViewBuilder private var permissionStatusSection: some View {
        if let pending {
            PermissionCard(
                ...
                onResolve: onResolve)
                .frame(
                    width: EgakiumCoworkStatusRailLayoutPolicy.cardWidth,
                    alignment: .leading)
                .egakiumClearLiquidGlass(cornerRadius: 22)
    ...
    private func agentStatusRowContent(_ agent: CoworkAgentInfo) -> some View {
            ...
            if agent.isAttached && agent.inferenceResolution.requiresAttention {
                Image(systemName: "exclamationmark.triangle.fill")
                    ...
                    .accessibilityLabel(agent.inferenceDisplayLabel
                        ?? EgakiumLocalization.string("Inference unavailable"))
            }
        }
        ...
        .help(["\(agent.name): \(egakiumLocalizedRawStatus(agent.status))", agent.inferenceDisplayLabel]
            .compactMap { $0 }
            .joined(separator: " · "))
    }
            ...
            TimelineView(.periodic(from: .now, by: 1)) { timeline in
                VStack(alignment: .leading, spacing: 9) {
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        goalMetric(
                            EgakiumLocalization.string("Tokens"),
                            value: tokenSummary(for: goal))
                        goalMetric(
                            EgakiumLocalization.string("Run"),
                            value: goal.currentRunOrdinal.map { "#\($0)" }
                                ?? EgakiumLocalization.string("Not started"))
                    }
    ...
    private func goalMetric(_ title: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(EgakiumLocalization.string(title).uppercased())
                .font(.caption.bold())
                .foregroundStyle(threadStyle.tertiaryText)
            Text(value)
                .font(.caption.monospacedDigit())
    ...
    @ViewBuilder private func goalActions(_ goal: CoworkGoalCardInfo) -> some View {
        if goal.canPause || goal.canResume || goal.canEdit || goal.canClear {
            ...
            EgakiumGlassEffectGroup(spacing: 10) {
                HStack(spacing: 10) {
                    if goal.canPause {
    ...
    private func tokenSummary(for goal: CoworkGoalCardInfo) -> String {
        ...
        guard let tokenBudget = goal.tokenBudget else {
            return EgakiumLocalization.format("%@ · no budget", used)
        }
    ...
    private func auditSummary(for goal: CoworkGoalCardInfo) -> String? {
        ...
        if let proven = goal.auditProvenCount, let required = goal.auditRequirementCount {
            parts.append(EgakiumLocalization.format(
    ...
    private func displayStatus(_ status: String) -> String {
        egakiumLocalizedDisplayStatus(status)
    }
    ...
    private func rightRailSection<Content: View>(
        ...
        VStack(alignment: .leading, spacing: 14) {
            Label(EgakiumLocalization.string(title), systemImage: systemImage)
    ...
    @ViewBuilder private var contextInspector: some View {
        ...
        if let latestTurnStats {
            inspectorSection("Last Turn") {
                EgakiumTurnStatsSummaryView(stats: latestTurnStats, style: threadStyle)
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 7) {
            inspectorRow(
                ...
                value: project.sessionID.isEmpty
                    ? EgakiumLocalization.string("current")
                    : project.sessionID)
            ...
            ForEach(summary.failedTasks) { task in
                taskCompactRow(
                    ...
                    actionTitle: onRetryTask == nil
                        ? nil
                        : EgakiumLocalization.string("Retry"),
    ...
    private func statusStrip(layout: EgakiumThreadContentLayout) -> some View {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 5) {
                        Text("@\(agent.name)")
                            ...
                            .foregroundStyle(threadStyle.primaryText)
                            .lineLimit(1)
                        Text(egakiumLocalizedAgentRole(agent.role))
                            .font(.caption2.bold())
                            .foregroundStyle(threadStyle.secondaryText)
                            .lineLimit(1)
                    }
        ...
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text("@\(agent.name)")
                    ...
                    .lineLimit(1)
                Spacer(minLength: 8)
                Text(egakiumLocalizedRawStatus(agent.status))
                    .font(.caption2.bold())
                    .foregroundStyle(statusColor(for: agent.status))
                    .lineLimit(1)
            ...
            Text(agent.workspace)
                ...
                .textSelection(.enabled)
            Divider().opacity(0.35)
            agentDetailRow("Role", value: egakiumLocalizedAgentRole(agent.role))
            ...
            agentDetailRow(
                "Queued",
                value: EgakiumLocalization.format(
    ...
    private func taskCompactRow(task: CoworkTaskLine,
        ...
        VStack(alignment: .leading, spacing: 4) {
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                ...
                Text(egakiumLocalizedDisplayStatus(task.status))
                    .font(.caption2.bold())
    ...
    private func agentRoster(layout: EgakiumThreadContentLayout) -> some View {
        VStack(alignment: .leading, spacing: 10) {
                    ...
                    LazyHStack(spacing: 8) {
                        if agents.isEmpty {
                            Text("No agents in this session")
                                ...
                                .padding(.horizontal, 12)
                                .padding(.vertical, 9)
                                .egakiumContentSurface(cornerRadius: 18)
                        }
                        ForEach(agents) { agent in
                            agentPill(agent)
                        }
    ...
    private func agentPill(_ agent: CoworkAgentInfo) -> some View {
        ...
        return Button {
            ...
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
            .egakiumLiquidGlass(cornerRadius: 18, interactive: true)
        }
        .buttonStyle(.plain)
        .disabled(!agent.isConversationSelectable)
    }
    ...
    @ViewBuilder private func taskHighlights(layout: EgakiumThreadContentLayout) -> some View {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if showsVisibleThinkingIndicator {
                            EgakiumThreadThinkingRow(
                                ...
                                .id("egakium-cowork-thinking-\(thinkingPhaseID)")
                        }
                        Color.clear
                            .frame(height: 1)
                            .padding(.bottom, 16)
    ...
    private func resolvedViewportAdmission(
        for pageScope: EgakiumThreadPresentationScope
    ) -> EgakiumMessageViewportAdmission {
    ...
    private var threadHistoryWindow: EgakiumThreadHistoryWindow<CodeItem> {
        EgakiumThreadHistoryWindow(
            items: EgakiumThreadErrorPresentation.transcriptItems(
                threadPage.items),
    ...
    private var threadPresentationScope: EgakiumThreadPresentationScope {
        EgakiumThreadPresentationScope(
    ...
    private func composerArea(layout: EgakiumThreadContentLayout) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            EgakiumThreadComposer(
                placeholder: EgakiumLocalization.string("Give Main a project task..."),
                input: $input,
...
private struct CoworkWorkTaskRow: View {
    ...
    private var displayStatus: String {
        egakiumLocalizedDisplayStatus(task.status)
    }
