...
struct EgakiumLatestOnlySubmissionID: Hashable, Comparable, RawRepresentable, Sendable {
...
struct EgakiumLatestOnlyPermit<Key: Hashable & Sendable>: Sendable {
  public let key: Key
  public let submissionID: EgakiumLatestOnlySubmissionID

  fileprivate let schedulerID: UUID
}

/// The caller-reported reason a permit's real work ended.
enum EgakiumLatestOnlyFinishOutcome: Sendable {
  case success
  case cancelled
  case failed
}
...
struct EgakiumLatestOnlyPermitSchedulerSnapshot<Key: Hashable & Sendable>: Sendable {
...
enum EgakiumLatestOnlyPermitSchedulerConfigurationError: Error, Equatable, Sendable {
...
actor EgakiumLatestOnlyPermitScheduler<Key: Hashable & Sendable> {
  public typealias Permit = EgakiumLatestOnlyPermit<Key>
  public typealias Snapshot = EgakiumLatestOnlyPermitSchedulerSnapshot<Key>

  private struct PendingAcquire: Sendable {
    let submissionID: EgakiumLatestOnlySubmissionID
  ...
  private var latestSubmissionByKey: [Key: EgakiumLatestOnlySubmissionID] = [:]
  ...
  public init(
    ...
    guard maxConcurrentPermits > 0 else {
      throw EgakiumLatestOnlyPermitSchedulerConfigurationError.nonPositiveConcurrentMaximum(
        maxConcurrentPermits
      )
    }
    guard maxPendingAcquires > 0 else {
      throw EgakiumLatestOnlyPermitSchedulerConfigurationError.nonPositivePendingMaximum(
        maxPendingAcquires
      )
    }
  ...
  public func finish(
    _ permit: Permit,
    outcome: EgakiumLatestOnlyFinishOutcome = .success
  ) -> Bool {
    finishInternal(
      permit,
      outcome: outcome,
  ...
  private func makeSubmissionID() -> EgakiumLatestOnlySubmissionID {
    precondition(
      nextSubmissionRawValue < UInt64.max,
      "EgakiumLatestOnlySubmissionID space exhausted"
    )
    nextSubmissionRawValue += 1
    return EgakiumLatestOnlySubmissionID(rawValue: nextSubmissionRawValue)
  }

  private func suspendAcquire(
    for key: Key,
    submissionID: EgakiumLatestOnlySubmissionID
  ) async -> Permit? {
    await withCheckedContinuation { continuation in
      registerAcquire(
        for: key,
  ...
  private func finishInternal(
    _ permit: Permit,
    outcome: EgakiumLatestOnlyFinishOutcome,
