...
public struct EgakiumThreadStyle {
    ...
    public static func standard(_: ColorScheme) -> EgakiumThreadStyle {
        let stroke = egakiumPlatformSeparator
        return EgakiumThreadStyle(
            primaryText: .primary,
...
public enum EgakiumMessageHeaderPolicy {
...
struct EgakiumThreadErrorEntry: Identifiable, Equatable, Sendable {
    let id: String
    var title: String?
    var details: [String]
...
enum EgakiumThreadErrorPresentation {
    private struct Candidate {
        let fingerprint: String
        let entry: EgakiumThreadErrorEntry
    }

    static func errors(
        items: [CodeItem],
        errorTexts: [String]
    ) -> [EgakiumThreadErrorEntry] {
        let latestSubmissionID = items.reversed().first(where: {
            $0.kind == .user && $0.submissionID != nil
        })?.submissionID
        var orderedFingerprints: [String] = []
        var entriesByFingerprint: [String: EgakiumThreadErrorEntry] = [:]
        ...
        if item.kind == .user,
           item.submissionStatus == .failed || item.submissionFailure != nil {
            ...
            return makeCandidate(
                ...
                title: EgakiumLocalization.string("Needs attention"),
                primaryDetail: failure?.message
                    ?? advice?.detail
                    ?? EgakiumLocalization.string("Needs attention"),
...
struct EgakiumThreadErrorList: View {
    ...
    private func errorRow(_ error: EgakiumThreadErrorEntry) -> some View {
...
public enum EgakiumMessageTimestampPresentation {
...
public struct EgakiumThreadPresentationScope: Hashable, Sendable {
...
public struct EgakiumThreadBottomAnchorID: Hashable, Sendable {
    public let scope: EgakiumThreadPresentationScope

    public init(scope: EgakiumThreadPresentationScope) {
        self.scope = scope
    }
}

enum EgakiumThreadScrollReason: Equatable, Sendable {
    case initialRestore
    case liveUpdate
    case completion
    ...
    var diagnosticReason: EgakiumScrollDiagnosticReason {
        switch self {
        case .initialRestore:
...
struct EgakiumThreadScrollRequest: Equatable, Sendable {
    let scope: EgakiumThreadPresentationScope
    let generation: UInt64
    let reason: EgakiumThreadScrollReason
    let animated: Bool
    let wasBottomFollowing: Bool
}

struct EgakiumThreadScrollSignature: Equatable, Sendable {
...
struct EgakiumThreadScrollGeometry: Equatable, Sendable {
    let isAtBottom: Bool
    ...
    static func == (
        lhs: EgakiumThreadScrollGeometry,
        rhs: EgakiumThreadScrollGeometry
    ) -> Bool {
...
enum EgakiumThreadFollowState: Equatable, Sendable {
...
public enum EgakiumMessageViewportAdmission: Equatable, Sendable {
    case immediate
...
private struct EgakiumMessageViewportAdmissionEnvironmentKey: EnvironmentKey {
    static let defaultValue = EgakiumMessageViewportAdmission.immediate
}

extension EnvironmentValues {
    public var egakiumMessageViewportAdmission:
        EgakiumMessageViewportAdmission {
        get { self[EgakiumMessageViewportAdmissionEnvironmentKey.self] }
        set { self[EgakiumMessageViewportAdmissionEnvironmentKey.self] = newValue }
    }
}

enum EgakiumThreadRichSettleToken: Hashable, Sendable {
    case finalDocument(
        messageID: String,
...
struct EgakiumThreadRichSettleSource: Equatable, Sendable {
    let scope: EgakiumThreadPresentationScope
    let activationGeneration: UInt64
}

private struct EgakiumThreadScrollCoordinatorEnvironmentKey: EnvironmentKey {
    static let defaultValue: EgakiumThreadScrollCoordinator? = nil
}

private struct EgakiumThreadRichSettleSourceEnvironmentKey: EnvironmentKey {
    static let defaultValue: EgakiumThreadRichSettleSource? = nil
}

extension EnvironmentValues {
    var egakiumThreadScrollCoordinator: EgakiumThreadScrollCoordinator? {
        get { self[EgakiumThreadScrollCoordinatorEnvironmentKey.self] }
        set { self[EgakiumThreadScrollCoordinatorEnvironmentKey.self] = newValue }
    }

    var egakiumThreadRichSettleSource: EgakiumThreadRichSettleSource? {
        get { self[EgakiumThreadRichSettleSourceEnvironmentKey.self] }
        set { self[EgakiumThreadRichSettleSourceEnvironmentKey.self] = newValue }
    }
}
...
struct EgakiumThreadScrollCadenceModel: Sendable {
    ...
    struct Fire: Equatable, Sendable {
        let request: EgakiumThreadScrollRequest
        let nextWake: Wake?
    }
    ...
    private(set) var executorRequest: EgakiumThreadScrollRequest?
    /// The only pending request. New live updates replace this exact slot.
    private(set) var pendingRequest: EgakiumThreadScrollRequest?
    ...
    var outstandingRequests: [EgakiumThreadScrollRequest] {
        [executorRequest, pendingRequest].compactMap { $0 }
    }

    mutating func submit(
        _ request: EgakiumThreadScrollRequest,
...
struct EgakiumThreadRichSettleModel: Sendable {
    ...
    struct Epoch: Equatable, Sendable {
        let token: EgakiumThreadRichSettleToken
    ...
    enum Check: Equatable, Sendable {
        ...
        case settled(token: EgakiumThreadRichSettleToken)
    }

    private(set) var epoch: Epoch?
    private(set) var lastOpenedToken: EgakiumThreadRichSettleToken?
...
@MainActor
final class EgakiumThreadScrollCoordinator: ObservableObject {
    ...
    private(set) var activeScope: EgakiumThreadPresentationScope?
    ...
    @Published private(set) var followState: EgakiumThreadFollowState =
        .followingBottom
    @Published private(set) var viewportAdmission:
        EgakiumMessageViewportAdmission = .suspended(generation: 0)
    @Published private(set) var richSettleSource:
        EgakiumThreadRichSettleSource? = nil

    private var pendingTask: Task<Void, Never>?
    private var pendingWakeDeadlineNanoseconds: UInt64?
    private var pendingWakeToken: UInt64 = 0
    private var cadence = EgakiumThreadScrollCadenceModel()
    ...
    private var richSettle = EgakiumThreadRichSettleModel()
    private var richSettleTask: Task<Void, Never>?
    private var richSettleTaskToken: UInt64 = 0
    private let performanceDiagnostics: EgakiumPerformanceDiagnostics

    init(
        performanceDiagnostics: EgakiumPerformanceDiagnostics = .shared
    ) {
    ...
    func effectiveViewportAdmission(
        for scope: EgakiumThreadPresentationScope,
    ...
    func activate(
        ...
        richSettleSource = EgakiumThreadRichSettleSource(
            scope: scope,
    ...
    func deactivate(scope: EgakiumThreadPresentationScope) {
        guard activeScope == scope else { return }
        invalidatePending()
        invalidateGeometryObservation()
        invalidateBottomAnchorVisibility()
    ...
    func installScrollExecutor(
        scope: EgakiumThreadPresentationScope,
        perform: @escaping @MainActor (EgakiumThreadScrollRequest) -> Void
    ) {
        guard activeScope == scope else { return }
    ...
    func userInteractionDidBegin(scope: EgakiumThreadPresentationScope) {
        guard activeScope == scope else { return }
    ...
    func userInteractionDidEnd(scope: EgakiumThreadPresentationScope) {
        guard activeScope == scope,
    ...
    func jumpToLatest(
        ...
        guard activeScope == scope else {
            ...
            performanceDiagnostics.recordScrollRequest(
                reason: EgakiumThreadScrollReason.jumpToLatest.diagnosticReason,
                outcome: .stale,
    ...
    func richDocumentDidCommit(
        token: EgakiumThreadRichSettleToken,
        source: EgakiumThreadRichSettleSource
    ) {
    ...
    private func synchronizeRichSettleWake(
        _ check: EgakiumThreadRichSettleModel.Check
    ) {
        switch check {
        case .inactive:
            return
...
public struct EgakiumJumpToLatestButton: View {
...
public enum EgakiumThreadHistoryWindowPolicy {
    public static let capacity = 16
}

enum EgakiumThreadRichEntryPolicy {
...
struct EgakiumThreadHistorySelection: Equatable {
    let scope: EgakiumThreadPresentationScope
    let requestedUpperBound: Int?

    func upperBound(
        for currentScope: EgakiumThreadPresentationScope
    ) -> Int? {
        scope == currentScope ? requestedUpperBound : nil
    }
}

public struct EgakiumThreadHistoryWindow<Element> {
    ...
    public static func resolve(
        allItems: [Element],
        requestedUpperBound: Int?,
        capacity: Int = EgakiumThreadHistoryWindowPolicy.capacity
    ) -> Self {
...
public struct EgakiumThreadHistoryPager: View {
        ...
        HStack(spacing: 8) {
            if hasEarlier {
                Button(action: onEarlier) {
                    Label(
                        EgakiumLocalization.string("Earlier"),
            ...
            if hasLater {
                Button(action: onNewer) {
                    Label(
                        EgakiumLocalization.string("Newer"),
                ...
                Button(action: onLatest) {
                    Label(
                        EgakiumLocalization.string("Latest"),
                        systemImage: "arrow.down.to.line")
                }
...
enum EgakiumThreadStackLayoutMode: Equatable {
    case eager
    case lazy
...
public struct EgakiumAdaptiveThreadStack<Content: View>: View {
    ...
    @ViewBuilder public var body: some View {
        switch EgakiumThreadStackLayoutMode.resolve(
...
private var egakiumPlatformSeparator: Color {
    #if canImport(AppKit)
...
private struct EgakiumContentSurfaceModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay {
                shape.stroke(egakiumPlatformSeparator, lineWidth: 1)
            }
    }
}

private struct EgakiumSubtleContentSurfaceModifier: ViewModifier {
    let cornerRadius: CGFloat
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay {
                shape.stroke(
                    egakiumPlatformSeparator.opacity(0.55),
                    lineWidth: 0.5)
            }
    }
}
...
private struct EgakiumClearLiquidGlassBackdrop: View, Equatable {
    let cornerRadius: CGFloat
    ...
    static func == (
        lhs: EgakiumClearLiquidGlassBackdrop,
        rhs: EgakiumClearLiquidGlassBackdrop
    ) -> Bool {
    ...
    private var stableOutline: some View {
        RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
            .strokeBorder(
                egakiumPlatformSeparator.opacity(0.42),
...
private struct EgakiumLiquidGlassModifier: ViewModifier {
    ...
    @ViewBuilder func body(content: Content) -> some View {
        if usesClearVariant {
            content.background {
                EgakiumClearLiquidGlassBackdrop(
                    cornerRadius: cornerRadius,
...
private struct EgakiumGlassButtonModifier: ViewModifier {
...
public extension View {
    ...
    func egakiumContentSurface(cornerRadius: CGFloat = 16) -> some View {
        modifier(EgakiumContentSurfaceModifier(cornerRadius: cornerRadius))
    }
    ...
    func egakiumSubtleContentSurface(cornerRadius: CGFloat = 12) -> some View {
        modifier(EgakiumSubtleContentSurfaceModifier(cornerRadius: cornerRadius))
    }

    /// Native Liquid Glass on current systems, with semantic Material fallback.
    func egakiumLiquidGlass(cornerRadius: CGFloat = 16,
                            interactive: Bool = false) -> some View {
        modifier(EgakiumLiquidGlassModifier(
            cornerRadius: cornerRadius,
    ...
    func egakiumClearLiquidGlass(cornerRadius: CGFloat = 16,
                                 interactive: Bool = false) -> some View {
        modifier(EgakiumLiquidGlassModifier(
            cornerRadius: cornerRadius,
            isInteractive: interactive,
    ...
    func egakiumGlassButton(prominent: Bool = false) -> some View {
        modifier(EgakiumGlassButtonModifier(isProminent: prominent))
    }
    ...
    func egakiumCompactIconButton(prominent: Bool = false) -> some View {
        labelStyle(.iconOnly)
            .controlSize(.regular)
            .buttonBorderShape(.circle)
            .egakiumGlassButton(prominent: prominent)
    }
    ...
    @ViewBuilder func egakiumComposerIconButton(
        prominent: Bool = false
    ) -> some View {
        #if os(iOS)
        labelStyle(.iconOnly)
            .controlSize(EgakiumComposerControlMetrics.iconControlSize(
                for: .iOS))
            .buttonBorderShape(.circle)
            .egakiumGlassButton(prominent: prominent)
            .frame(
                width: EgakiumComposerControlMetrics.controlHeight,
                height: EgakiumComposerControlMetrics.controlHeight)
            .contentShape(Circle())
        #else
        egakiumCompactIconButton(prominent: prominent)
            .frame(
                width: EgakiumComposerControlMetrics.controlHeight,
                height: EgakiumComposerControlMetrics.controlHeight)
            .contentShape(Circle())
    ...
    func egakiumComposerSelectionMenu() -> some View {
        buttonStyle(.plain)
    }

    /// Shared 40-point capsule geometry for the model/profile Menu label.
    func egakiumComposerSelectionLabel() -> some View {
        padding(
            .horizontal,
            EgakiumComposerControlMetrics.selectionHorizontalPadding)
            .frame(
                height: EgakiumComposerControlMetrics.controlHeight,
                alignment: .leading)
            .frame(
                ...
                alignment: .leading)
            .egakiumLiquidGlass(
                cornerRadius: EgakiumComposerControlMetrics.controlHeight / 2,
                interactive: true)
    }
...
public struct EgakiumGlassEffectGroup<Content: View>: View {
    private let spacing: CGFloat?
    private let content: Content
...
public struct EgakiumTurnStatsSummaryView: View {
    private let stats: TurnStatsSnapshot
    private let style: EgakiumThreadStyle

    public init(stats: TurnStatsSnapshot, style: EgakiumThreadStyle) {
        self.stats = stats
        self.style = style
    }
    ...
    public var body: some View {
        ...
        .padding(.vertical, 6)
        .egakiumContentSurface(cornerRadius: 14)
        .help(summary)
    }
    ...
    private var parts: [String] {
        ...
        if let ttftMillis = stats.ttftMillis {
            values.append(EgakiumLocalization.format(
                "ttft %@",
    ...
    private var tokenPart: String? {
        if let totalTokens = stats.totalTokens {
            let total = EgakiumLocalization.format(
                "%@ tok",
        ...
        if let promptTokens = stats.promptTokens,
           let cachedPromptTokens = stats.cachedPromptTokens {
            let uncachedPromptTokens = max(promptTokens - cachedPromptTokens, 0)
            pieces.append(EgakiumLocalization.format(
                "%@ input",
                formatNumber(uncachedPromptTokens)))
            pieces.append(EgakiumLocalization.format(
                "%@ cached",
...
public struct EgakiumComposerUsageStrip: View {
    private let stats: TurnStatsSnapshot?
    private let style: EgakiumThreadStyle
                ...
                HStack(spacing: 18) {
                    ...
                    if stats.promptTokens == nil,
                       ...
                       let totalTokens = stats.totalTokens {
                        metric(
                            EgakiumLocalization.string("Total"),
                            value: formatNumber(totalTokens))
                    }
                    if let totalMillis = stats.totalMillis {
                        metric(
                            EgakiumLocalization.string("Time"),
...
public struct EgakiumSessionHistoryItem: Identifiable, Hashable {
...
public struct EgakiumSessionHistoryList: View {
    private let title: String
    private let newTitle: String
    private let emptyTitle: String
    private let items: [EgakiumSessionHistoryItem]
    private let style: EgakiumThreadStyle
    ...
    public init(title: String,
                newTitle: String,
                emptyTitle: String,
                items: [EgakiumSessionHistoryItem],
                    ...
                    LazyVStack(spacing: 5) {
                        ForEach(items) { item in
                            Button {
                                ...
                                EgakiumSessionHistoryRow(item: item, style: style)
                            }
                            .buttonStyle(.plain)
                            .accessibilityIdentifier(
...
private struct EgakiumSessionHistoryRow: View {
    let item: EgakiumSessionHistoryItem
    let style: EgakiumThreadStyle
...
public struct EgakiumRecoveryAdviceView: View {
...
public struct EgakiumThreadComposerSecondaryAction {
    public var systemImage: String
...
public enum EgakiumComposerControlMetrics {
    ...
    static func glassEffectSpacing(
        for platform: EgakiumComposerGlassPlatform
    ) -> CGFloat {
        switch platform {
        case .iOS:
            return 0
...
enum EgakiumComposerGlassPlatform: Sendable {
    case iOS
    case macOS
}

public enum EgakiumSidebarGesturePolicy {
...
public struct EgakiumThreadComposer: View {
    @Binding private var input: String
    private let placeholder: String
    private let canSend: Bool
    private let isInputDisabled: Bool
    private let style: EgakiumThreadStyle
    private let secondaryAction: EgakiumThreadComposerSecondaryAction?
    private let trailingAction: EgakiumThreadComposerSecondaryAction?
    private let stopAction: EgakiumThreadComposerSecondaryAction?
    ...
    private var inputPointSize: CGFloat = EgakiumTypography.spec(for: .chat).nominalPointSize

    private var inputFont: Font {
        EgakiumTypography.chat(inputPointSize)
    }
    ...
    public init(placeholder: String,
                ...
                style: EgakiumThreadStyle,
                secondaryAction: EgakiumThreadComposerSecondaryAction? = nil,
                leadingAccessory: AnyView? = nil,
                inputLeadingAccessory: AnyView? = nil,
                trailingAction: EgakiumThreadComposerSecondaryAction? = nil,
                stopAction: EgakiumThreadComposerSecondaryAction? = nil,
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            EgakiumGlassEffectGroup(spacing: composerGlassEffectSpacing) {
                composerControls
            }
        }
    }
    ...
    private var composerGlassEffectSpacing: CGFloat {
        #if os(iOS)
        EgakiumComposerControlMetrics.glassEffectSpacing(for: .iOS)
        #else
        EgakiumComposerControlMetrics.glassEffectSpacing(for: .macOS)
        #endif
    }
    ...
    private var inputControl: some View {
        TextField(placeholder, text: $input, axis: .vertical)
            ...
            .padding(.horizontal, EgakiumComposerControlMetrics.inputHorizontalPadding)
            .padding(.vertical, EgakiumComposerControlMetrics.inputVerticalPadding)
            .frame(
                minHeight: EgakiumComposerControlMetrics.controlHeight,
                alignment: .center)
            .egakiumLiquidGlass(
                cornerRadius: EgakiumComposerControlMetrics.inputCornerRadius,
                interactive: true)
    }

    private func compactActionButton(
        _ action: EgakiumThreadComposerSecondaryAction
    ) -> some View {
    ...
    private var sendButton: some View {
        Button(action: onSend) {
            Label(EgakiumLocalization.string("Send"), systemImage: "arrow.up")
                .egakiumComposerIconLabel()
        }
        .egakiumComposerIconButton(prominent: true)
        .disabled(!canSend)
        .accessibilityLabel(EgakiumLocalization.string("Send"))
    ...
    private func stopButton(
        ...
        Button(role: .destructive, action: action.action) {
            Label(
                EgakiumLocalization.string("Stop"),
        ...
        .help(action.help)
        .disabled(action.isDisabled)
        .accessibilityLabel(EgakiumLocalization.string("Stop"))
        .accessibilityIdentifier("thread.composer.stop")
    }

}

public struct EgakiumThreadContentLayout {
...
enum EgakiumThreadBubbleWidthPolicy: Equatable {
...
public struct EgakiumThreadBubbleRow<Content: View>: View {
    ...
    @ViewBuilder private var row: some View {
        switch EgakiumThreadBubbleWidthPolicy.resolve(
            isTrailing: isTrailing,
...
public struct EgakiumThreadThinkingRow: View {
    private let layout: EgakiumThreadContentLayout
    private let style: EgakiumThreadStyle
    private let label: String
    private let phaseID: String

    public init(layout: EgakiumThreadContentLayout,
                style: EgakiumThreadStyle,
                label: String = EgakiumLocalization.string("Thinking…"),
...
public struct EgakiumThinkingElapsedLabel: View {
    private let label: String
    private let phaseID: String
    ...
    public var body: some View {
        TimelineView(.periodic(from: startedAt, by: 1)) { context in
            ...
            let text = EgakiumLocalization.format(
                "%llds %@",
                Int64(elapsed),
                label)
            Text(text)
...
public enum EgakiumThreadHeaderActionPresentation: Sendable {
    case glass
    case compactSystemIcon
}

public struct EgakiumThreadHeaderAction {
    let title: String
    let systemImage: String
    let isDisabled: Bool
    let isIconOnly: Bool
    let presentation: EgakiumThreadHeaderActionPresentation
    let help: String
    ...
    public init(title: String,
                ...
                presentation: EgakiumThreadHeaderActionPresentation = .glass,
                help: String? = nil,
...
struct EgakiumWorkspaceThreadHeader: View {
    let title: String
    let subtitle: String?
    let style: EgakiumThreadStyle
    let actions: [EgakiumThreadHeaderAction]
    ...
    private var titleBlock: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(EgakiumTypography.largeTitle())
                .foregroundStyle(style.primaryText)
            if let subtitle, !subtitle.isEmpty {
                Text(subtitle)
                    .font(EgakiumTypography.caption(13, .medium))
                    .foregroundStyle(style.secondaryText)
                    .lineLimit(2)
            }
        }
    ...
    @ViewBuilder private var actionRow: some View {
        if !actions.isEmpty {
            EgakiumGlassEffectGroup(spacing: 8) {
                HStack(spacing: 8) {
    ...
    @ViewBuilder private func actionButton(
        _ action: EgakiumThreadHeaderAction
    ) -> some View {
