...
struct ThreadView: View {
    ...
    private static let bottomAnchorID = "egakium-shared-chat-thread-bottom"

    var body: some View {
        VStack(spacing: 0) {
            ScrollViewReader { proxy in
                ScrollView {
                    EgakiumAdaptiveThreadStack(
            ...
            if !placesTurnStatsInComposer,
               let latestTurnStats = model.latestTurnStats {
                EgakiumTurnStatsSummaryView(stats: latestTurnStats, style: .standard(scheme))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.horizontal)
                    .padding(.vertical, 4)
            }
...
struct MessageRow: View {
    ...
    private var chatTextSize: CGFloat = EgakiumTypography.spec(for: .chat).nominalPointSize
    ...
    private var style: EgakiumThreadStyle {
        .standard(scheme)
    }
        ...
        VStack(alignment: .leading, spacing: 4) {
                    ...
                    if (message.role == .assistant || message.role == .agent),
                       let timestamp = message.timestamp {
                        Text(EgakiumMessageTimestampPresentation.string(for: timestamp))
                            .font(EgakiumTypography.metadata(metadataSize))
                            .monospacedDigit()
            ...
            if message.role == .assistant || message.role == .agent {
                ...
                if !message.citations.isEmpty {
                    EgakiumMessageCitationsView(citations: message.citations)
                }
            } else {
                if !displayText.isEmpty {
                    Text(displayText)
                        .font(EgakiumTypography.chat(chatTextSize))
                ...
                if message.role == .user, !message.attachments.isEmpty {
                    Label(
                        ...
                        .font(EgakiumTypography.caption(captionSize))
                        .foregroundStyle(.secondary)
                }
            }
            if let advice = message.recoveryAdvice {
                EgakiumRecoveryAdviceView(advice: advice, tint: .red, style: style)
            }
        }
    }
    ...
    private var roleLabel: String? {
        ...
        case .assistant: return EgakiumLocalization.string("Assistant")
        case .agent:
            return message.agent?.rawValue ?? EgakiumLocalization.string("Agent")
...
public struct EgakiumMessageCitationsView: View {
...
struct ComposerView: View {
    ...
    var body: some View {
        EgakiumThreadComposer(
            ...
            accessory: placesTurnStatsInComposer
                ? AnyView(EgakiumComposerUsageStrip(
    ...
    private var voiceAction: EgakiumThreadComposerSecondaryAction? {
        #if canImport(AVFoundation)
        let voice = model.voiceInput
        return EgakiumThreadComposerSecondaryAction(
    ...
    private var inputLeadingAccessory: AnyView? {
        #if os(iOS)
        let label = EgakiumLocalization.string("Attachments and chat tools")
        return AnyView(
            Menu {
                Button {
                    model.generateImage()
                    ...
                    Label(
                        EgakiumLocalization.string("Generate image from prompt"),
        ...
        List {
            Section("Status") {
                ...
                LabeledContent(
                    "Streaming",
                    value: isStreaming
                        ? EgakiumLocalization.string("Yes")
                        : EgakiumLocalization.string("No"))
                LabeledContent(
                    "Image job",
                    value: isGeneratingArtifact
                        ? EgakiumLocalization.string("Running")
                        : EgakiumLocalization.string("Idle"))
                LabeledContent(
                    "Artifact progress",
                    value: artifactProgress.isEmpty
                        ? EgakiumLocalization.string("None")
                        : EgakiumLocalization.format("%lld active", Int64(artifactProgress.count)))
