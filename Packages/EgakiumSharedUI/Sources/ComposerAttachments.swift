...
public struct EgakiumComposerDraftAttachment: Identifiable, Equatable, Sendable {
...
public struct EgakiumComposerAttachmentFile: Equatable, Sendable {
    public var name: String
...
public enum EgakiumComposerAttachmentFileReader {
    public static func read(_ url: URL) throws -> EgakiumComposerAttachmentFile {
        ...
        #endif
        return EgakiumComposerAttachmentFile(
...
public enum EgakiumComposerAttachmentResolutionError: LocalizedError, Sendable {
...
public struct EgakiumComposerAttachmentStore: Sendable {
    ...
    public func preserve(
        _ file: EgakiumComposerAttachmentFile
    ) async throws -> EgakiumComposerDraftAttachment {
        ...
        guard verifiedRef == ref,
            ...
            throw EgakiumError.io("attachment read-back verification failed")
        }
        try Task.checkCancellation()
        return EgakiumComposerDraftAttachment(
            id: ref.id,
            name: file.name,
            mime: file.mime)
    }
    ...
    public func imageAttachments(
        ...
        for id in ids {
            ...
            guard let ref = await store.ref(for: id) else {
                throw EgakiumComposerAttachmentResolutionError.missing(id)
            }
            guard ref.mime.hasPrefix("image/") else {
                throw EgakiumComposerAttachmentResolutionError.unsupported(
                    id,
                    mime: ref.mime,
                    surface: surface)
            }
            ...
            do {
                ...
                throw EgakiumComposerAttachmentResolutionError.unreadable(
                    id,
