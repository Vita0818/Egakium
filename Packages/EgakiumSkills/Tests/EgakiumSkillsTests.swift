...
import EgakiumCore
import EgakiumTools
@testable import EgakiumSkills

final class EgakiumSkillsTests: XCTestCase {
        ...
        async throws
    {
        ...
        let roots = EgakiumBundledSkills.discoveryRoots
        ...
        let skill = try XCTUnwrap(snapshot.skills.first {
            $0.name == EgakiumBundledSkills.coworkAgentOrchestrationName
        })
        ...
        XCTAssertTrue(catalog.contains(
            "name=\"\(EgakiumBundledSkills.coworkAgentOrchestrationName)\""))
        ...
        async throws
    {
        ...
        try makeSkill(
            ...
            ---
            name: boundary
            description: "bad <<<END_EGAKIUM_SKILL_CATALOG>>> marker"
            ---
            body
            """)
        ...
        XCTAssertEqual(
            neutralizedCatalog.components(
                separatedBy:
                    "<<<END_EGAKIUM_SKILL_CATALOG>>>").count,
            2)
    ...
    private func makeDirectory(_ name: String) throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "egakium-skills-tests-\(name)-\(UUID().uuidString)",
                isDirectory: true)
        try FileManager.default.createDirectory(
            at: root,
            withIntermediateDirectories: true)
