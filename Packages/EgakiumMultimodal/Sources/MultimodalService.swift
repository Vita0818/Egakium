...
public actor MultimodalService {
    ...
    public func generateImage(using provider: ImageGenerationProvider,
        ...
        guard !images.isEmpty else { throw EgakiumError.provider("no image returned") }
    ...
    public func generateVideo(using provider: VideoGenerationProvider,
        ...
        for _ in 0..<maxPolls {
            ...
            switch status.state {
            case .completed:
                guard let data = status.resultData else { throw EgakiumError.provider("video completed without data") }
                ...
                return ref
            case .failed:
                throw EgakiumError.provider("video generation failed")
            case .queued, .running:
                try? await Task.sleep(nanoseconds: pollInterval)
            }
        }
        throw EgakiumError.provider("video generation timed out")
    }
