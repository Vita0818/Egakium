import XCTest
import EgakiumCore
@testable import EgakiumProviders
...
private final class CapturingDataClient: HTTPDataClient, @unchecked Sendable {
    private let queue = DispatchQueue(label: "egakium.tests.image-edit-http")
...
private final class SequencedDataClient: HTTPDataClient, @unchecked Sendable {
    private let queue = DispatchQueue(label: "egakium.tests.sequenced-data")
    private var index = 0
...
private let nonHTTPBaseEndpoint = ProviderEndpoint(id: "bad-base",
                                                   baseURL: URL(fileURLWithPath: "/tmp/egakium-provider"),
                                                   apiKeyRef: KeychainRef(service: "s", account: "a"),
                                                   wire: .openai)

final class EgakiumProvidersMultimodalTests: XCTestCase {
    ...
    func testImageEditSendsMultipartReferenceAndParsesBase64() async throws {
        ...
        XCTAssertTrue(request.value(forHTTPHeaderField: "Content-Type")?
            .hasPrefix("multipart/form-data; boundary=egakium-image-edit-") == true)
    ...
    func testCompatibleTranscriptionBuildsDiskBackedMultipartWAVRequest()
        ...
        XCTAssertTrue(
            request.value(forHTTPHeaderField: "Content-Type")?
                .hasPrefix("multipart/form-data; boundary=EgakiumBoundary-")
                == true)
