...
enum ProviderErrorFormatting {
    ...
    static func httpStatus(_ status: Int,
        ...
        return EgakiumError.provider(parts.joined(separator: " "))
    }
    ...
    static func transport(_ error: Error) -> Error {
        ...
        if let egakium = error as? EgakiumError {
            return sanitized(egakium)
        }
        if let urlError = error as? URLError {
            return EgakiumError.provider(transportMessage(urlError))
        }
        return EgakiumError.provider(clean(error.localizedDescription))
    }
    ...
    static func invalidStreamPayload(_ payload: String, underlying: Error) -> EgakiumError {
    ...
    static func incompleteStream(operation: String) -> EgakiumError {
        .decoding(
            "\(operation) ended before a completion marker. " +
            "Check endpoint compatibility, proxy buffering, or provider stability.")
    }

    static func invalidToolCallStream(_ message: String) -> EgakiumError {
        .decoding(
    ...
    static func invalidResponsePayload(_ data: Data,
                                       operation: String,
                                       expected: String,
                                       underlying: Error? = nil) -> EgakiumError {
        var parts = [
    ...
    private static func sanitized(_ error: EgakiumError) -> EgakiumError {
        switch error {
