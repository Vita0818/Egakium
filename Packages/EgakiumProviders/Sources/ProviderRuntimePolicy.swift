...
enum ProviderRuntime {
    ...
    static func exhausted(_ error: Error, attempts: Int, operation: String) -> Error {
        ...
        if let egakium = normalized as? EgakiumError {
            switch egakium {
            case .provider(let message):
                return EgakiumError.provider(message + suffix)
            case .config(let message):
                return EgakiumError.config(message + suffix)
            case .decoding(let message):
                return EgakiumError.decoding(message + suffix)
            case .io(let message):
                return EgakiumError.io(message + suffix)
            case .notFound(let message):
                return EgakiumError.notFound(message + suffix)
            case .permissionDenied(let message):
                return EgakiumError.permissionDenied(message + suffix)
            case .cancelled:
                return egakium
            }
        }
        return EgakiumError.provider("\(operation) failed. \(normalized.localizedDescription)\(suffix)")
    }
        ...
        return try await withThrowingTaskGroup(of: T.self) { group in
            ...
            do {
                guard let value = try await group.next() else {
                    throw EgakiumError.provider("\(operation) did not produce a result.")
                }
                group.cancelAll()
                return value
            } catch {
