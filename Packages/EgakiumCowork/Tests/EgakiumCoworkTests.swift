...
private func tempLog() throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("egakium-cowork-\(UUID().uuidString)", isDirectory: true)
...
final class EgakiumCoworkTests: XCTestCase {
    ...
    func testMainCanSpawnWorkerButSpawnedWorkerHasNoCoordinatorTools() async throws {
        ...
        let sharedCanvas = try SessionCanvasStore.ensureCanvas(
            in: wsMain,
            sessionID: SessionID(rawValue: "cw"))
        let sharedCanvasBeforeSpawn = try Data(contentsOf: sharedCanvas.indexURL)
        defer {
            try? FileManager.default.removeItem(at: wsMain)
        }
        let mainProvider = ScriptedProvider([
        ...
        let eventsAfterSpawn = await log.replay()
        let spawnedPayload = try XCTUnwrap(eventsAfterSpawn.compactMap {
            envelope -> AgentSpawnedPayload? in
            guard case .agentSpawned(let payload) = envelope.event,
                  payload.agent == worker else { return nil }
            return payload
        }.last)
        let element = try XCTUnwrap(spawnedPayload.provisionedCanvasElement)
        let requestedElement = try XCTUnwrap(eventsAfterSpawn.compactMap {
            envelope -> SessionCanvasElementDescriptor? in
            guard case .agentSpawnRequested(let payload) = envelope.event,
                  payload.agent == worker else { return nil }
            return payload.provisionedCanvasElement
        }.last)
        XCTAssertEqual(requestedElement, element)
        XCTAssertEqual(element.templateVersion, 1)
        XCTAssertTrue(element.elementID.rawValue.hasPrefix("element_"))
        XCTAssertEqual(
            element.relativeIndexPath,
            ".egakium/canvas/cw/elements/\(element.elementID.rawValue)/index.html")
        let elementURL = wsWorker.appendingPathComponent(
            element.relativeIndexPath)
        XCTAssertEqual(
            try String(contentsOf: elementURL, encoding: .utf8),
            SessionCanvasElementTemplate.html)
        XCTAssertEqual(
            try Data(contentsOf: sharedCanvas.indexURL),
            sharedCanvasBeforeSpawn,
            "spawn must not patch @main's shared Canvas index")
        let mainFollowup = try XCTUnwrap(mainProvider.requests.last)
        let spawnToolResult = try XCTUnwrap(mainFollowup.messages.first {
            $0.role == .tool && $0.toolCallId == "spawn"
        }?.content)
        XCTAssertTrue(spawnToolResult.contains(
            "canvas_element_id=\(element.elementID.rawValue)"))
        XCTAssertTrue(spawnToolResult.contains(
            "canvas_element_path=\(element.relativeIndexPath)"))
        ...
        let systemPrompt = try XCTUnwrap(request.messages.first?.content)
        XCTAssertTrue(systemPrompt.contains("running inside Egakium"))
        ...
        XCTAssertTrue(systemPrompt.contains(
            "`\(element.elementID.rawValue)`"))
        XCTAssertTrue(systemPrompt.contains(
            "`\(element.relativeIndexPath)`"))
        XCTAssertTrue(systemPrompt.contains(
            "This is a workspace file, not a prompt-only example"))
        XCTAssertFalse(systemPrompt.contains(
            "EGAKIUM_GENERIC_ELEMENT_TEMPLATE"))
        XCTAssertFalse(systemPrompt.contains("data-element-id"))
    ...
    func testMainCanExplicitlySpawnCoordinatorSubAgent() async throws {
        ...
        XCTAssertTrue(mainSystemPrompt.contains("running inside Egakium"))
    ...
    func testSpawnAgentDescriptorIsNotReadOnly() {
        ...
        XCTAssertTrue(
            SpawnAgentTool.descriptor.description
                .contains("host-provisions one fresh generic Canvas element"))
        XCTAssertTrue(
    ...
    func testSpawnAgentIntentIsControlPlaneAndDefaultsToReadOnly() throws {
        ...
        XCTAssertEqual(intent.controlEffects, [
            ...
            .attachWorkspace,
            .grantCapability,
            .provisionCanvasElement,
        ])
        XCTAssertEqual(intent.metadata["requestedAccess"], .string(WorkspaceAccess.readOnly.rawValue))
        XCTAssertEqual(intent.metadata["provisionsGenericCanvasElement"], .bool(true))
        XCTAssertEqual(intent.metadata["canvasElementPathScope"],
                       .string("target_workspace_session_namespace"))
        XCTAssertEqual(intent.metadata["canvasElementTemplateVersion"], .number(1))
        XCTAssertTrue(intent.risks.contains(.workspaceMutation))
        XCTAssertTrue(
            intent.isReadOnlyWorkspaceCompatible,
            "host provisioning must not grant the acting agent general write authority")
