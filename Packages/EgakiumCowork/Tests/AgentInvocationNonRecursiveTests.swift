...
private func nonRecursiveLog(_ suffix: String = UUID().uuidString) throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("egakium-nonrecursive-\(suffix)", isDirectory: true)
...
final class AgentInvocationNonRecursiveTests: XCTestCase {
    ...
    func testToolSpawnedWorkerReportsStructuredResultAndIsAutoDetachedWhenIdle() async throws {
        ...
        let provisioned = try XCTUnwrap(events.compactMap {
        ...
        let survivingElement = wsWorker.appendingPathComponent(
            provisioned.relativeIndexPath)
        XCTAssertEqual(
            try String(contentsOf: survivingElement, encoding: .utf8),
            SessionCanvasElementTemplate.html,
            "agent recycling must not delete its independently reassignable element")
    }
