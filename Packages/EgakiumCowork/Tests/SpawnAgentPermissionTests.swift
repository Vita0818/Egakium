...
final class SpawnAgentPermissionTests: XCTestCase {
    private func tempLog() throws -> EventLog {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-spawn-\(UUID().uuidString)", isDirectory: true)
    ...
    private func tempWorkspace(name: String = "ws") throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-\(name)-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSpawnAdmissionFailureRollsBackFreshCanvasElementBeforeReturningError() async throws {
        let log = try tempLog()
        let mainWorkspace = try tempWorkspace(name: "main-rollback")
        let childWorkspace = try tempWorkspace(name: "child-rollback")
        defer {
            try? FileManager.default.removeItem(at: mainWorkspace)
            try? FileManager.default.removeItem(at: childWorkspace)
        }
        ...
        await orch.setAdmissionEventsAppender { _ in
            throw NSError(
                domain: "SpawnAgentPermissionTests",
                code: 91,
                userInfo: [NSLocalizedDescriptionKey: "injected batch failure"])
        }
        ...
        XCTAssertTrue(message.contains("spawn admission could not be persisted"))
        XCTAssertTrue(message.contains("injected batch failure"))
        XCTAssertFalse(message.contains("could not be safely rolled back"))
        let after = await log.replay()
        let names = await orch.agentNames()
        XCTAssertEqual(after, before)
        XCTAssertFalse(names.contains(AgentID(rawValue: "rollback-worker")))
        let elementsRelative = try SessionCanvasStore.relativeElementsDirectoryPath(
            for: await log.sessionID)
        let elementsURL = childWorkspace.appendingPathComponent(
            elementsRelative,
            isDirectory: true)
        XCTAssertTrue(FileManager.default.fileExists(atPath: elementsURL.path))
        XCTAssertTrue(try FileManager.default.contentsOfDirectory(
            at: elementsURL,
            includingPropertiesForKeys: nil).isEmpty)
    }

    func testSpawnLostAcknowledgmentRecoversCommittedBatchWithoutDeletingElement() async throws {
        let log = try tempLog()
        let workspace = try tempWorkspace(name: "lost-ack-element")
        ...
        await orch.setAdmissionEventsAppender { events in
            ...
            throw NSError(
                domain: "SpawnAgentPermissionTests",
                code: 92,
                userInfo: [NSLocalizedDescriptionKey: "injected lost acknowledgment"])
        }

        let message = await orch.spawnFromTool(
            requestedBy: Orchestrator.mainAgentID,
            name: "lost-ack-worker",
            path: workspace.path,
            requestedAccess: .readWrite)

        XCTAssertTrue(message.hasPrefix("spawned @lost-ack-worker"), message)
        XCTAssertTrue(message.contains(
            "admission_recovered_after_lost_ack=true"), message)
        let agents = await orch.agentNames()
        XCTAssertTrue(
            agents.contains(AgentID(rawValue: "lost-ack-worker")),
            message)
        let events = await log.replay()
        let descriptor = try XCTUnwrap(events.compactMap {
            envelope -> SessionCanvasElementDescriptor? in
            guard case .agentSpawned(let payload) = envelope.event,
                  payload.agent == AgentID(rawValue: "lost-ack-worker") else {
                return nil
            }
    ...
    func testRestoreRecoversProvisionedElementForAgentListing() async throws {
        let log = try tempLog()
        let workspace = try tempWorkspace(name: "restore-element")
        defer { try? FileManager.default.removeItem(at: workspace) }
        let first = orchestrator(log: log, decision: .allow)
        let mainAttached = await first.attach(Agent(
            name: Orchestrator.mainAgentID,
            workspaceRoot: workspace,
            model: ModelID(rawValue: "m"),
            profile: .reviewed,
            coordinationDepth: Agent.defaultCoordinationDepth))
        XCTAssertTrue(mainAttached)
        let result = await first.spawnFromTool(
            requestedBy: Orchestrator.mainAgentID,
            name: "restored-worker",
            path: workspace.path,
            requestedAccess: .readWrite)
        XCTAssertTrue(result.hasPrefix("spawned @restored-worker"))
        let events = await log.replay()
        let descriptor = try XCTUnwrap(events.compactMap {
            envelope -> SessionCanvasElementDescriptor? in
            guard case .agentSpawned(let payload) = envelope.event,
                  payload.agent == AgentID(rawValue: "restored-worker") else {
                return nil
            }
            return payload.provisionedCanvasElement
        }.last)

        let restored = orchestrator(log: log, decision: .allow)
        await restored.restore(from: CoworkProjection.build(from: events))
        let listing = await restored.listForTool()

        XCTAssertTrue(listing.contains(
            "canvas_element=\(descriptor.elementID.rawValue)"))
        XCTAssertTrue(listing.contains(
            "path=\(descriptor.relativeIndexPath)"))
        XCTAssertEqual(
            try String(
                contentsOf: workspace.appendingPathComponent(
                    descriptor.relativeIndexPath),
