...
private func e2eLog() throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("egakium-cowork-e2e-\(UUID().uuidString)", isDirectory: true)
...
private func makeCounterFixture() throws -> CounterFixture {
    ...
    let root = fm.temporaryDirectory
        .appendingPathComponent("egakium-cowork-count-fixture-\(UUID().uuidString)", isDirectory: true)
    let macSources = root
        .appendingPathComponent("Apps", isDirectory: true)
        .appendingPathComponent("EgakiumMac", isDirectory: true)
        .appendingPathComponent("Sources", isDirectory: true)
    let iosSources = root
        .appendingPathComponent("Apps", isDirectory: true)
        .appendingPathComponent("EgakiumiOS", isDirectory: true)
        .appendingPathComponent("Sources", isDirectory: true)
    ...
    return CounterFixture(
        root: root,
        macOSRoot: root.appendingPathComponent("Apps/EgakiumMac", isDirectory: true),
        iOSRoot: root.appendingPathComponent("Apps/EgakiumiOS", isDirectory: true))
}
...
final class CoworkEndToEndTests: XCTestCase {
    ...
    func testMacOSIOSSwiftFileCountScenarioClosesThroughSchedulerAndProjection() async throws {
        ...
        let log = try e2eLog()
        let macOSProvider = E2EProvider(text: "macOS Swift count: 2\nApps/EgakiumMac/Sources/A.swift\nApps/EgakiumMac/Sources/B.swift")
        let iOSProvider = E2EProvider(text: "iOS Swift count: 3\nApps/EgakiumiOS/Sources/C.swift\nApps/EgakiumiOS/Sources/D.swift\nApps/EgakiumiOS/Sources/E.swift")
        ...
        let macOSObjective = "Recursively count Swift files in Apps/EgakiumMac only. Do not count iOS files."
        let iOSObjective = "Recursively count Swift files in Apps/EgakiumiOS only. Do not count macOS files."
        ...
        XCTAssertTrue(macOSContract.objective.contains("EgakiumMac"))
        XCTAssertFalse(macOSContract.objective.contains("EgakiumiOS"))
        XCTAssertTrue(iOSContract.objective.contains("EgakiumiOS"))
        XCTAssertFalse(iOSContract.objective.contains("EgakiumMac only"))
