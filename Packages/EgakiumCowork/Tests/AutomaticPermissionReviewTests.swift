...
private func autoReviewTempLog() throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("egakium-auto-review-\(UUID().uuidString)", isDirectory: true)
...
final class AutomaticPermissionReviewTests: XCTestCase {
        ...
        for variant in variants {
            let directory = FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "egakium-bootstrap-\(variant.suffix)-\(UUID().uuidString)",
                    isDirectory: true)
    ...
    func testSpawnToolUsesOneAutomaticReviewAndCommitsCoordinatorAdmissionAtomically() async throws {
        ...
        XCTAssertEqual(
            spawnReview.intent?.metadata["provisionsGenericCanvasElement"],
            .bool(true))
        XCTAssertTrue(spawnReview.intent?.risks.contains(.workspaceMutation) == true)
        ...
        XCTAssertTrue(events.contains {
            ...
            return payload.agent == childID
                && payload.requestedBy == main
                && payload.provisionedCanvasElement != nil
        })
    }
