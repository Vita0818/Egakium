...
public struct SpawnAgentTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        ...
        description: "Create a new sub-agent bound to a folder so you can delegate work to it. "
            ...
            + "Every successful spawn also host-provisions one fresh generic Canvas element "
            + "document in the new agent's exact workspace and returns its ID and relative path. "
    ...
    public func touchedPaths(_ args: ToolArgs) -> [String] {
        // No model-authored file path exists: the host chooses the exact
        // ElementID only at spawn commit. The authorization intent carries the
        // canonical target workspace plus the explicit provisioning effect;
        // the resulting exact relative path is persisted in the spawn events.
        []
    }
    ...
    public func permissionIntent(_ args: ToolArgs, workspaceRoot: URL) -> PermissionIntent {
        ...
        return PermissionIntent(
            ...
            metadata: [
                ...
                "provisionsGenericCanvasElement": .bool(true),
                "canvasElementPathScope": .string(
                    "target_workspace_session_namespace"),
                "canvasElementTemplateVersion": .number(
                    Double(SessionCanvasElementTemplate.templateVersion)),
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard let manager = context.agentManager else {
            throw EgakiumError.io("agent management is not available in this session")
        }
        ...
        guard trimmed.hasPrefix("spawned @") else {
            let message = trimmed.lowercased().hasPrefix("error:")
                ...
                : trimmed
            throw EgakiumError.io(message.isEmpty ? "agent spawn did not complete" : message)
        }
...
public struct ListAgentsTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        name: "list_agents",
        description: "List active agents with name, model, coordinator/worker lease role, provisioned Canvas element, compact task state, and folder.",
        sideEffect: .readOnly,
        parameters: .object([
            "type": .string("object"),
            "properties": .object([:]),
...
public struct RemoveAgentTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        if result.lowercased().hasPrefix("error:") {
            throw EgakiumError.io(String(result.dropFirst("error:".count))
                .trimmingCharacters(in: .whitespacesAndNewlines))
        }
