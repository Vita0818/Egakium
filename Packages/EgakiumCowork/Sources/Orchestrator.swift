...
public actor Orchestrator {
    ...
    /// Fresh generic element resources created as part of successful spawn
    /// admissions. This is provenance for active agents, never an ownership or
    /// lease table.
    private var provisionedCanvasElements:
        [AgentID: SessionCanvasElementDescriptor]
    ...
    /// Optional host-owned internal tools. EgakiumCowork remains independent
    /// of every concrete implementation (including EgakiumKnowledge).
    ...
    init(log: EventLog,
        ...
        self.provisionedCanvasElements = [:]
    ...
    public func attach(_ agent: Agent,
        ...
        let attachGate = PermissionReviewGateSnapshot(
            ...
            policyVersion: "egakium.workspace-admission.v1")
        let attachAuthorization = ResolvedToolAuthorization(
            authorizationID: IDGen.random(prefix: "tool-authorization"),
            registryVersion: "egakium.cowork.admission.v1",
            concreteToolID: "egakium.cowork.admission.v1/agent.attach",
    ...
    public func detach(_ name: AgentID, reason: String = "agent detached") async -> Bool {
        ...
        // The document itself intentionally survives agent recycling so @main
        // can keep displaying or reassigning it. Only the active-agent prompt
        // association ends here.
        provisionedCanvasElements.removeValue(forKey: name)
        if let capabilityLeaseID {
    ...
    public func restore(from _: CoworkProjection) async {
        ...
        provisionedCanvasElements.removeAll()
        ...
        provisionedCanvasElements = projection.provisionedCanvasElements.filter {
            registry.agent($0.key) != nil
        }
    ...
    private func requireValidWorkTaskGraph() throws {
        if let workTaskRecoveryFailure {
            throw EgakiumError.config(workTaskRecoveryFailure)
        }
    }
    ...
    private static func provenWorkTaskCreatePreflightRejection(
        ...
        let code: String
        let recovery: String
        if let egakiumError = error as? EgakiumError {
            switch egakiumError {
    ...
    func createWorkTask(canManage: Bool,
        ...
        do {
            guard canManage else {
                throw EgakiumError.permissionDenied("the current capability lease cannot create WorkTasks")
            }
            let title = request.title.trimmingCharacters(in: .whitespacesAndNewlines)
            let description = request.description.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !title.isEmpty, !description.isEmpty else {
                throw EgakiumError.decoding("WorkTask title and description must be non-empty")
            }
    ...
    func updateWorkTask(currentWorkTaskID: WorkTaskID?,
        ...
        do {
            ...
            guard let current = workTaskGraph.task(request.taskID) else {
                throw EgakiumError.notFound("WorkTask \(request.taskID.rawValue)")
            }
            ...
            guard hasMutation else {
                throw EgakiumError.decoding("task_update requires at least one mutable field")
            }
        ...
        guard let updated = preparedGraph.task(updatedTaskID) else {
            throw EgakiumError.notFound("WorkTask \(updatedTaskID.rawValue)")
        }
    ...
    func getWorkTask(currentWorkTaskID: WorkTaskID?,
        ...
        guard let task = workTaskGraph.task(taskID) else {
            throw EgakiumError.notFound("WorkTask \(taskID.rawValue)")
        }
        guard canReadWorkTask(
            task,
            currentWorkTaskID: currentWorkTaskID,
            canManage: canManage) else {
            throw EgakiumError.permissionDenied("WorkTask is outside the caller's readable scope")
        }
    ...
    func createGoal(request: GoalCreateRequest,
        ...
        guard !objective.isEmpty else {
            throw EgakiumError.decoding("Goal objective must be non-empty")
        }
        if let tokenBudget = request.tokenBudget, tokenBudget <= 0 {
            throw EgakiumError.decoding("Goal token budget must be greater than zero")
        }
        let sessionID = await log.sessionID
    ...
    func editGoal(request: GoalEditRequest,
        ...
        guard hostAuthorized else {
            throw EgakiumError.permissionDenied("Goal edits are reserved for the user/host control plane")
        }
        let objective = request.objective.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !objective.isEmpty else {
            throw EgakiumError.decoding("Goal objective must be non-empty")
        ...
        guard projection.currentGoalID == request.goalID,
            ...
            throw EgakiumError.notFound("current Goal \(request.goalID.rawValue)")
        }
        let edited: Goal
    ...
    func transitionGoal(_ goalID: GoalID,
        ...
        guard projection.currentGoalID == goalID,
            ...
            throw EgakiumError.notFound("current Goal \(goalID.rawValue)")
        }
        if status == .blocked {
    ...
    func settleGoalRunAudit(_ audit: GoalAuditSummary,
        ...
        guard projection.currentGoalID == goalID,
            ...
            throw EgakiumError.notFound("active current Goal \(goalID.rawValue)")
        }
    ...
    func clearGoal(_ goalID: GoalID,
        ...
        guard hostAuthorized else {
            throw EgakiumError.permissionDenied("clearing a Goal is reserved for the user/host control plane")
        }
        await acquireAdmissionLock()
        defer { releaseAdmissionLock() }
        let projection = CoworkProjection.build(from: await log.replay())
    ...
    func spawnFromTool(requestedBy: AgentID,
        ...
        let canvasSessionID = await log.sessionID
        let provisionedElement: SessionCanvasElementDocument
        do {
            provisionedElement = try SessionCanvasStore.provisionElementTemplate(
                in: canonical,
                sessionID: canvasSessionID,
                elementID: CanvasElementID.new())
        } catch {
            return "error: generic Canvas element could not be provisioned: \(error.localizedDescription)"
        }
        let provisionedElementDescriptor = provisionedElement.descriptor
        let spawnAdmissionEvents: [Event] = [
            .agentSpawnRequested(AgentSpawnRequestedPayload(
                ...
                model: proposedAgent.model,
                agentInferenceBinding: proposedAgent.agentInferenceBinding,
                provisionedCanvasElement: provisionedElementDescriptor,
        ...
        var recoveredCommittedAdmission = false
        do {
            try await appendAdmissionEvents(spawnAdmissionEvents)
        } catch {
            ...
            do {
                ...
                return "error: spawn admission outcome could not be proven after a persistence failure; the Canvas element was preserved at \(provisionedElementDescriptor.relativeIndexPath) for recovery: \(persistenceError); replay: \(error.localizedDescription)"
            }
            guard replay.hasCompleteKnownHistory else {
                return "error: spawn admission outcome could not be proven because the EventLog contains unsupported events; the Canvas element was preserved at \(provisionedElementDescriptor.relativeIndexPath) for recovery: \(persistenceError)"
            }
            let durableEvents = replay.envelopes.map(\.event)
            let canonicalExpectedEvents: [Event]
            do {
                let encoder = Envelope.makeEncoder()
                let decoder = Envelope.makeDecoder()
                canonicalExpectedEvents = try spawnAdmissionEvents.enumerated().map {
                    offset, event in
                    ...
                    return try decoder.decode(
                        Envelope.self,
                        from: encoder.encode(envelope)).event
                }
            } catch {
                return "error: spawn admission outcome could not be canonicalized after a persistence failure; the Canvas element was preserved at \(provisionedElementDescriptor.relativeIndexPath) for recovery: \(persistenceError); canonicalization: \(error.localizedDescription)"
            }
            let containsExactBatch: Bool
            if durableEvents.count >= canonicalExpectedEvents.count {
                containsExactBatch = (0...(durableEvents.count - canonicalExpectedEvents.count))
                    .contains { offset in
                        Array(durableEvents[
                            offset..<(offset + canonicalExpectedEvents.count)
                        ]) == canonicalExpectedEvents
                    }
            } else {
                containsExactBatch = false
            }
            if containsExactBatch {
                // The EventLog WAL/replay boundary proved that the exact batch
                // committed despite the original lost acknowledgment. Keep the
                // matching element and finish the in-memory admission.
                recoveredCommittedAdmission = true
            } else {
                let containsDescriptorFact = durableEvents.contains { event in
                    switch event {
                    case .agentSpawnRequested(let payload):
                        return payload.agent == id
                            && payload.provisionedCanvasElement
                                == provisionedElementDescriptor
                ...
                guard !containsDescriptorFact else {
                    return "error: spawn admission history is incomplete or conflicting; the Canvas element was preserved at \(provisionedElementDescriptor.relativeIndexPath) for recovery: \(persistenceError)"
                }
                do {
                    try SessionCanvasStore.rollbackProvisionedElement(
                        provisionedElement)
                    return "error: spawn admission could not be persisted: \(persistenceError)"
                } catch {
                    return "error: spawn admission could not be persisted and the uncommitted Canvas element could not be safely rolled back at \(provisionedElementDescriptor.relativeIndexPath): \(persistenceError); rollback: \(error.localizedDescription)"
                }
            }
        }
        if recoveredCommittedAdmission {
            // Normal append success refreshes the rebuildable session
            // projection in appendAdmissionEvents. A recovered lost
            // acknowledgment crossed that branch, so repair the cache here.
        ...
        provisionedCanvasElements[id] = provisionedElementDescriptor
        ...
        let recovery = recoveredCommittedAdmission
            ? " · admission_recovered_after_lost_ack=true"
            : ""
        return "spawned @\(name) · \(inference) · \(canCoordinate ? "coordinator" : "worker") · \(requestedAccess.rawValue) · \(canonical.path) · canvas_element_id=\(provisionedElementDescriptor.elementID.rawValue) · canvas_element_path=\(provisionedElementDescriptor.relativeIndexPath) · canvas_template_version=\(provisionedElementDescriptor.templateVersion)\(recovery)"
    }
    ...
    func listForTool() -> String {
        ...
        return all.map {
            ...
            let canvasElement = provisionedCanvasElements[$0.name].map {
                "canvas_element=\($0.elementID.rawValue) path=\($0.relativeIndexPath) template=\($0.templateVersion)"
            } ?? "canvas_element=none"
            return [
                "@\($0.name.rawValue)",
                $0.model.rawValue,
    ...
    private func run(_ agent: Agent,
        ...
        let canvasSessionID = await log.sessionID
        let editableCanvasSessionID: SessionID? =
            agent.name == Self.mainAgentID
                && taskContract?.kind == .root
            ? canvasSessionID
            : nil
        let provisionedCanvasElement: SessionCanvasElementDescriptor?
        if let durableDescriptor = provisionedCanvasElements[agent.name] {
            do {
                guard let existing = try SessionCanvasStore.existingElement(
                    in: agent.workspaceRoot,
                    sessionID: canvasSessionID,
                    descriptor: durableDescriptor) else {
                    throw CoworkTaskExecutionError.invalidLease(
                        "the provisioned Canvas element is missing")
                }
                provisionedCanvasElement = existing.descriptor
            } catch let error as CoworkTaskExecutionError {
                throw error
            } catch {
                throw CoworkTaskExecutionError.invalidLease(
                    "the provisioned Canvas element is unsafe or invalid")
            }
        } else {
            provisionedCanvasElement = nil
        }
        ...
        let systemPrompt = ContextBuilder.coworkSystemPrompt(
            ...
            canCoordinate: canCoordinate,
            editableCanvasSessionID: editableCanvasSessionID,
            provisionedCanvasElement: provisionedCanvasElement,
            includesGenericCanvasElementTemplate:
                agent.name != Self.mainAgentID
                    && provisionedCanvasElement == nil)
    ...
    private func prepareAuthorization(
        ...
        guard let requesterID = request.invocation.agent,
            ...
            throw EgakiumError.permissionDenied("delegating agent is not attached")
        }
        ...
        if let suppliedTarget {
            ...
            guard candidate != requesterID,
                ...
                throw EgakiumError.permissionDenied("requested delegation target is reserved")
            }
            guard let existing = registry.agent(candidate) else {
                throw EgakiumError.notFound(
        ...
        if let failure = delegationFailure(
            ...
            throw EgakiumError.permissionDenied(failure)
        }
    ...
    private func prepareSpawnAuthorization(
        ...
        guard let requesterID = request.invocation.agent,
              let requester = registry.agent(requesterID) else {
            throw EgakiumError.permissionDenied("spawning agent is not attached")
        }
        let arguments = try JSONDecoder().decode(
        ...
        -> ToolRegistry
    {
        ...
        return ToolRegistry(
            ...
            registryVersion: "egakium.cowork.v4")
    }
