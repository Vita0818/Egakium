...
import XCTest
import EgakiumCore
@testable import EgakiumProtocol
...
final class InferenceProfileProtocolTests: XCTestCase {
    ...
    func testAgentLifecyclePayloadsCarryBindingAndLegacyPayloadsDecodeAsUnresolved() throws {
        ...
        let element = SessionCanvasElementDescriptor(
            elementID: CanvasElementID(rawValue: "element_worker_001"),
            relativeIndexPath:
                ".egakium/canvas/sess_test/elements/element_worker_001/index.html")
        ...
        let spawnRequested = AgentSpawnRequestedPayload(
            ...
            model: exactBinding.modelID,
            agentInferenceBinding: exactBinding,
            provisionedCanvasElement: element)
        let decodedSpawnRequested = try decoder.decode(
            AgentSpawnRequestedPayload.self,
            from: encoder.encode(spawnRequested))
        XCTAssertEqual(decodedSpawnRequested.agentInferenceBinding, exactBinding)
        XCTAssertEqual(decodedSpawnRequested.provisionedCanvasElement, element)
        ...
        let spawned = AgentSpawnedPayload(
            ...
            agentInferenceBinding: exactBinding,
            provisionedCanvasElement: element)
        let decodedSpawned = try decoder.decode(
            AgentSpawnedPayload.self,
            from: encoder.encode(spawned))
        XCTAssertEqual(decodedSpawned.agentInferenceBinding, exactBinding)
        XCTAssertEqual(decodedSpawned.provisionedCanvasElement, element)
        ...
        let decodedLegacySpawnRequested = try decoder.decode(
            AgentSpawnRequestedPayload.self,
            from: Data(legacySpawn.utf8))
        XCTAssertNil(decodedLegacySpawnRequested.agentInferenceBinding)
        XCTAssertNil(decodedLegacySpawnRequested.provisionedCanvasElement)
        let decodedLegacySpawn = try decoder.decode(
            AgentSpawnedPayload.self,
            from: Data(legacySpawn.utf8))
        XCTAssertNil(decodedLegacySpawn.agentInferenceBinding)
        XCTAssertNil(decodedLegacySpawn.provisionedCanvasElement)
    }
