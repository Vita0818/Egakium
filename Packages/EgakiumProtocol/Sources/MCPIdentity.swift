...
/// Egakium request. This type is intentionally not interchangeable with a
...
/// Host identity for a task that Egakium owns on behalf of a server callback
/// request. Keeping this distinct prevents accidental routing into the remote
/// task state machine or the Egakium/Cowork task graph.
