...
final class DocumentToolsIntegrationTests: XCTestCase {
    ...
    func testInstalledDocumentRuntimeEPUBWriteWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_DOCUMENT_RUNTIME_SMOKE"] == "1" else {
            throw XCTSkip(
                "set EGAKIUM_REAL_DOCUMENT_RUNTIME_SMOKE=1 to run the installed document runtime smoke")
        }
        guard let runtime = egakiumDocumentRuntimeRoot(),
              fileManager.isExecutableFile(
                  atPath: runtime.appendingPathComponent("bin/egakium-rbook-helper").path),
              fileManager.isExecutableFile(
                  atPath: runtime.appendingPathComponent("bin/egakium-epubcheck").path) else {
        ...
        let observation = try await DocumentWriteTool().execute(
            ToolArgs(raw: #"""
            {
              ...
              "operations":[
                {"kind":"metadata.set","parameters":{"field":"identifier","value":"urn:egakium:runtime-smoke"}},
                {"kind":"metadata.set","parameters":{"field":"title","value":"Egakium Runtime Smoke"}},
    ...
    func testInstalledDocumentRuntimeCoreToolChainWhenEnabled() async throws {
        ...
        guard let runtime = egakiumDocumentRuntimeRoot(),
              let libreOffice = egakiumLibreOfficeRuntimeAppURL()?
        ...
        let context = ToolContext(
            workspaceRoot: workspace,
            documentBackend: DocumentBackendProcessRunner(timeoutSeconds: 300))
        let marker = "EGAKIUM DOCUMENT RUNTIME 73921"

        let write = try await DocumentWriteTool().execute(
            ToolArgs(raw: #"""
            {"format":"docx","mode":"create","output_path":"runtime-smoke.docx",
             "operations":[{"kind":"paragraph.add","parameters":{"text":"EGAKIUM DOCUMENT RUNTIME 73921"}}]}
            """#),
            in: context)
        ...
        let htmlMarker = "EGAKIUM HTML RUNTIME 18426"
        ...
        let slideMarker = "EGAKIUM PRESENTATION RUNTIME 48216"
        ...
        let xlsxWrite = try await DocumentWriteTool().execute(
            ...
            {"format":"xlsx","mode":"create","output_path":"runtime-smoke.xlsx",
             "operations":[
              ...
              {"kind":"cell.set","parameters":{"sheet":"Sheet","cell":"E3","value":"EGAKIUM SHEET RUNTIME 93715"}}
             ]}
            """#),
            in: context)
        XCTAssertTrue(
        ...
        let xlsxRead = try await ReadXLSXTool().execute(
            ...
            in: context)
        XCTAssertTrue(xlsxRead.text.contains("EGAKIUM SHEET RUNTIME 93715"), xlsxRead.text)
    ...
    func testInstalledDocumentRuntimeReadsUserCorpusWhenConfigured() async throws {
        guard let rawRoot = ProcessInfo.processInfo.environment[
            "EGAKIUM_REAL_DOCUMENT_CORPUS_ROOT"],
              !rawRoot.isEmpty else {
            throw XCTSkip(
                "set EGAKIUM_REAL_DOCUMENT_CORPUS_ROOT to run the external document corpus smoke")
        }
        guard let runtime = egakiumDocumentRuntimeRoot(),
              fileManager.isExecutableFile(
    ...
    func testInstalledDocumentRuntimePDFCPUAndOCRWhenEnabled() async throws {
        ...
        let stage = workspace.appendingPathComponent(
            ".egakium-document-stage-runtime-smoke",
            isDirectory: true)
        ...
        let digest = try DocumentInputFile.freeze(
            path: ".egakium-document-stage-runtime-smoke/ocr-source.pdf",
            expectedFormat: .pdf,
            workspace: workspace).identity.sha256
        let observation = try await DocumentOCRTool().execute(
            ToolArgs(raw: """
            {"input_path":".egakium-document-stage-runtime-smoke/ocr-source.pdf",
        ...
        XCTAssertTrue(normalized.contains("EGAKIUMOCR48291"), observation.text)
        #else
    ...
    func testStandardRegistryExposesSplitDocumentReaders() throws {
        ...
        XCTAssertEqual(registry.registryVersion, "egakium.standard.v4")
    }
    ...
    func testPDFRenderCommitsCompleteBundleWithoutLeakingStageDirectory() async throws {
        ...
        XCTAssertFalse(rootChildren.contains { $0.hasPrefix(".egakium-document-stage-") })
        #else
        throw XCTSkip("PDFKit integration requires Apple PDF frameworks")
        #endif
    }
    ...
    func testDocumentProcessLeaseNarrowsBroadWorkspaceAuthorityToReviewedInputAndStage() throws {
        ...
        let narrowed = try documentProcessLease(
            ...
            internalWritablePaths: ["out/.egakium-document-stage-test"])

        XCTAssertEqual(narrowed.access, .readWrite)
        XCTAssertEqual(
            Set(narrowed.allowedPathRules.map(\.pattern)),
            ["source.docx", "out/.egakium-document-stage-test"])
        ...
        XCTAssertTrue(profile.contains("egakium-document-stage-test"))
    ...
    func testLibreOfficeSandboxAllowsOnlyInvocationPrivateUnixSocket() throws {
        ...
        defer { try? fileManager.removeItem(at: workspace) }
        let socketRoot = URL(
            fileURLWithPath: "/private/tmp/egakium-lo-0123456789ab",
            isDirectory: true)
        ...
        XCTAssertTrue(profile.contains(#"(subpath "/private/tmp/egakium-lo-0123456789ab")"#))
    ...
    private func makeWorkspace() throws -> URL {
        let url = fileManager.temporaryDirectory.appendingPathComponent(
            "egakium-document-tool-integration-\(UUID().uuidString)",
            isDirectory: true)
