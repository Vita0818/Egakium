...
private actor ShellResultQueue {
    ...
    func next() throws -> ShellResult {
        guard results.isEmpty == false else {
            throw EgakiumError.io("no fake shell results remain")
        }
...
private struct FakeGit: GitService {
    ...
    var commitsText: String = "abc123\tEgakium\t2026-01-01\tInitial"
...
final class EgakiumToolsTests: XCTestCase {
    func testCheckedInternalToolCloseFailsAndRemainsSingleFlight() async {
        ...
        } catch let error as EgakiumError {
    ...
    private func tempWorkspace() throws -> URL {
        let ws = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-ws-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeBlankPDF(pageCount: Int, at url: URL) throws {
        ...
        for index in 0..<pageCount {
            ...
            guard let page = PDFPage(image: image) else {
                throw EgakiumError.io("could not create test PDF page \(index)")
            }
            document.insert(page, at: index)
        }
        guard document.write(to: url) else {
            throw EgakiumError.io("could not write test PDF")
        }
    }
    #endif
    ...
    private func freeLoopbackPort() throws -> Int {
        ...
        guard descriptor >= 0 else {
            throw EgakiumError.io("could not create loopback socket")
        }
        ...
        guard bindResult == 0 else {
            throw EgakiumError.io("could not bind loopback socket")
        }
        ...
        guard nameResult == 0 else {
            throw EgakiumError.io("could not inspect loopback socket")
        }
    ...
    private func startStaticHTTPServer(directory: URL, port: Int) throws -> Process {
        guard let python = python3Executable() else {
            throw EgakiumError.io("python3 is required for the real browser persistence smoke")
        }
    ...
    private func waitForHTTPServer(port: Int, path: String = "/login.html") async throws {
        ...
        throw EgakiumError.io("local browser smoke server did not start")
    }
    #endif
    ...
    func testOrdinaryFileToolsCannotBypassManagedKnowledgePublication()
        ...
        try FileManager.default.createDirectory(
            at: snapshots,
            withIntermediateDirectories: true)
        let pointer = store.appendingPathComponent(".egakium-rag-store.json")
        ...
        do {
            _ = try await ReadFileTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.egakium-rag-store.json"}"#),
                in: context)
        ...
        do {
            _ = try await ListFilesTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.egakium-rag-snapshots"}"#),
                in: context)
        ...
        do {
            _ = try await SearchTextTool().execute(
                ToolArgs(raw: #"{"query":"snapshot","path":"knowledge/.egakium-rag-snapshots"}"#),
                in: context)
        ...
        do {
            _ = try await WriteFileTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.egakium-rag-store.json","content":"changed"}"#),
                in: context)
        ...
        let diff = [
            "--- a/knowledge/.egakium-rag-snapshots/profile.json",
            "+++ b/knowledge/.egakium-rag-snapshots/profile.json",
            "@@ -1 +1 @@",
            "-snapshot-original",
            "+changed",
        ].joined(separator: "\n")
    ...
    func testStructuredProcessShellRunnerUsesSanitizedEnvironmentAndWorkspaceLease() async throws {
        ...
        setenv("EGAKIUM_HOST_SECRET_MARKER", "host-secret-marker", 1)
        defer { unsetenv("EGAKIUM_HOST_SECRET_MARKER") }
        let runner = StructuredProcessShellRunner(timeoutSeconds: 5)

        let ordinary = try await runner.run("/bin/cat ordinary.txt; printf '|%s' \"$EGAKIUM_HOST_SECRET_MARKER\"", cwd: ws)
    ...
    func testBrowserBackendProcessRunnerUsesSanitizedEnvironment() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        setenv("EGAKIUM_BROWSER_HOST_SECRET_MARKER", "host-secret-marker", 1)
        defer { unsetenv("EGAKIUM_BROWSER_HOST_SECRET_MARKER") }

        let invocation = BrowserBackendInvocation(
            javaScript: #"process.stdout.write(`${process.env.EGAKIUM_BROWSER_HOST_SECRET_MARKER || ""}|${process.env.EGAKIUM_BROWSER_PROCESS_BOUNDARY || ""}`);"#,
    ...
    func testProcessShellRunnerDeniesWorkspaceExternalFile() async throws {
        ...
        let outside = ws.deletingLastPathComponent()
            .appendingPathComponent("egakium-outside-\(UUID().uuidString).txt")
        defer {
            try? FileManager.default.removeItem(at: ws)
        ...
        let escapedWrite = ws.deletingLastPathComponent()
            .appendingPathComponent("egakium-escaped-write-\(UUID().uuidString).txt")
    ...
    func testProcessGitServiceRejectsExternalDiffWithoutExecutingIt() async throws {
        ...
        let outsideConfig = ws.deletingLastPathComponent().appendingPathComponent("egakium-included-\(UUID().uuidString).gitconfig")
    ...
    func testProcessGitServiceAuditsWorktreeConfigAndRejectsSymlink() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("egakium-config-worktree-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
        }
    ...
    func testProcessGitServiceHonorsReadOnlyAndDeniedLeaseData() async throws {
        ...
        let setup = try await shell.run(
            "'\(git)' init; '\(git)' config user.email egakium@example.invalid; '\(git)' config user.name Egakium; '\(git)' add ordinary.txt .env; '\(git)' commit -m base",
            cwd: ws)
        XCTAssertEqual(setup.exitCode, 0, setup.stderr)
    ...
    func testProcessGitServiceStagesAndCommitsTempRepoWhenGitAvailable() async throws {
        #if canImport(Darwin)
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_GIT_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_GIT_SMOKE=1 to run the real ProcessGitService smoke")
        }
        ...
        _ = try await shell.run("git config user.email egakium@example.invalid", cwd: ws)
        _ = try await shell.run("git config user.name 'Egakium Test'", cwd: ws)
        ...
        _ = try await shell.run("git config user.email egakium@example.invalid", cwd: patchSource)
        _ = try await shell.run("git config user.name 'Egakium Test'", cwd: patchSource)
        ...
        let worktreeURL = ws.appendingPathComponent(".egakium/git-worktrees/task-1", isDirectory: true)
        ...
        XCTAssertTrue(worktreeInfo.contains("/.egakium/git-worktrees/task-1"), worktreeInfo)
    ...
    func testCompileLatexCannotReadOutsideWorkspace() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("egakium-tex-outside-\(UUID().uuidString).tex")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
        }
    ...
    func testBrowserNavigateUsesPersistentProfileStateAndHistory() async throws {
        ...
        XCTAssertTrue(obs.text.contains("Persistent browser profile: .egakium/browser/profiles/work"))
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/work.json")
        let historyURL = ws.appendingPathComponent(".egakium/browser/history.jsonl")
    ...
    func testBrowserHandoffUsesHeadedPersistentProfileAndRecordsHistory() async throws {
        ...
        XCTAssertTrue(obs.text.contains("Persistent browser profile: .egakium/browser/profiles/login"))
        ...
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/login.json")
    ...
    func testBrowserConcurrentProfilesKeepSeparateStateAndHistoryMetadata() async throws {
        ...
        let stateAURL = ws.appendingPathComponent(".egakium/browser/state/profile-a.json")
        let stateBURL = ws.appendingPathComponent(".egakium/browser/state/profile-b.json")
    ...
    func testRealBrowserBackendSmokeWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser backend smoke")
        }
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let ctx = ToolContext(workspaceRoot: ws)
        ...
        XCTAssertTrue(obs.text.contains("Persistent browser profile: .egakium/browser/profiles/smoke"))
    }

    func testRealCDPBrowserIgnoresStaleDevToolsActivePortWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the stale DevToolsActivePort smoke")
        }
        ...
        let profileDirectory = ws
            .appendingPathComponent(".egakium/browser/profiles/stale-port-smoke", isDirectory: true)
        try FileManager.default.createDirectory(
            at: profileDirectory,
            withIntermediateDirectories: true)
    ...
    func testRealBrowserSearchWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser search smoke")
        }
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let ctx = ToolContext(workspaceRoot: ws)
    ...
    func testRealBrowserHandoffWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_HANDOFF_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_HANDOFF_SMOKE=1 to run the headed browser handoff smoke")
        }
    ...
    func testRealBrowserProfilePersistsCookieLocalStorageAndHistoryWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser profile persistence smoke")
        }
        ...
        let loginHTML = """
        <!doctype html>
        <title>Egakium Login Set</title>
        <body>pending</body>
        <script>
        document.cookie = "egakium_login=present; Max-Age=3600; path=/; SameSite=Lax";
        localStorage.setItem("egakiumLocalLogin", "present");
        document.body.innerText = "login marker set";
        </script>
        """
        let stateHTML = """
        <!doctype html>
        <title>Egakium Login State</title>
        <body>pending</body>
        <script>
        document.body.innerText = "cookie=" + document.cookie + "\\nlocal=" + localStorage.getItem("egakiumLocalLogin");
        </script>
        """
        ...
        XCTAssertTrue(obs.text.contains("cookie=egakium_login=present"), obs.text)
    ...
    func testRealBrowserBackForwardWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser back/forward smoke")
        }
        ...
        let oneHTML = """
        <!doctype html>
        <title>Egakium History One</title>
        <body>history page one marker</body>
        """
        let twoHTML = """
        <!doctype html>
        <title>Egakium History Two</title>
    ...
    func testRealBrowserProfilesRemainIsolatedWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser profile isolation smoke")
        }
        ...
        let profileHTML = """
        <!doctype html>
        <title>Egakium Profile Isolation</title>
        <body>pending</body>
        <script>
        const params = new URLSearchParams(window.location.search);
        const marker = params.get("marker");
        if (marker) {
          document.cookie = "egakium_profile_marker=" + marker + "; Max-Age=3600; path=/; SameSite=Lax";
          localStorage.setItem("egakiumProfileMarker", marker);
        }
        document.body.innerText = "cookie=" + document.cookie + "\\nlocal=" + localStorage.getItem("egakiumProfileMarker");
        </script>
        """
        ...
        XCTAssertTrue(obsA.text.contains("egakium_profile_marker=profile-a-marker"), obsA.text)
        ...
        XCTAssertTrue(obsB.text.contains("egakium_profile_marker=profile-b-marker"), obsB.text)
    ...
    func testRealBrowserDifferentProfilesCanLaunchConcurrentlyWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_CONCURRENCY_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_CONCURRENCY_SMOKE=1 to run the real browser concurrent profile smoke")
        }
        ...
        let pageA = """
        <!doctype html>
        <title>Egakium Concurrent A</title>
        <body>
        ...
        let pageB = """
        <!doctype html>
        <title>Egakium Concurrent B</title>
        <body>
        ...
        let stateAURL = ws.appendingPathComponent(".egakium/browser/state/concurrent-a.json")
        let stateBURL = ws.appendingPathComponent(".egakium/browser/state/concurrent-b.json")
    ...
    func testRealBrowserUploadDownloadWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser upload/download smoke")
        }
        ...
        let formHTML = """
        <!doctype html>
        <title>Egakium Upload Download</title>
        <body>
        ...
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/io-smoke.json")
        try FileManager.default.createDirectory(at: stateURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let statePayload: [String: String] = [
            "profile": "io-smoke",
            "url": pageURL,
            "title": "Egakium Upload Download",
        ...
        XCTAssertTrue(downloadedPath.hasPrefix(".egakium/browser/downloads/io-smoke/"), downloadedPath)
        ...
        XCTAssertTrue(listed.text.contains(".egakium/browser/downloads/io-smoke/"), listed.text)
    ...
    func testRealBrowserSelectAndPressKeyWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser select/press smoke")
        }
        ...
        let formHTML = """
        <!doctype html>
        <title>Egakium Select Press</title>
        <body>
        ...
        try await waitForHTTPServer(port: port, path: "/index.html")
        let pageURL = "http://127.0.0.1:\(port)/index.html"
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/form-smoke.json")
        try FileManager.default.createDirectory(at: stateURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let statePayload: [String: String] = [
            "profile": "form-smoke",
            "url": pageURL,
            "title": "Egakium Select Press",
            "updatedAt": ISO8601DateFormatter().string(from: Date()),
        ]
        let stateData = try JSONSerialization.data(withJSONObject: statePayload, options: [.prettyPrinted, .sortedKeys])
        try stateData.write(to: stateURL, options: .atomic)
    ...
    func testRealBrowserSubmitWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser submit smoke")
        }
        ...
        let formHTML = """
        <!doctype html>
        <title>Egakium Submit Form</title>
        <body>
        ...
        let submittedHTML = """
        <!doctype html>
        <title>Egakium Submit Result</title>
    ...
    func testRealBrowserPopupNewPageWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser popup/new-page smoke")
        }
        ...
        let indexHTML = """
        <!doctype html>
        <title>Egakium Popup Source</title>
        <body>
        ...
        let popupHTML = """
        <!doctype html>
        <title>Egakium Popup Result</title>
    ...
    func testRealBrowserScrollAndWaitWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser scroll/wait smoke")
        }
        ...
        let pageHTML = """
        <!doctype html>
        <title>Egakium Scroll Wait</title>
        <style>
        ...
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/scroll-smoke.json")
        try FileManager.default.createDirectory(at: stateURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let statePayload: [String: String] = [
            "profile": "scroll-smoke",
            "url": pageURL,
            "title": "Egakium Scroll Wait",
            "updatedAt": ISO8601DateFormatter().string(from: Date()),
        ]
        let stateData = try JSONSerialization.data(withJSONObject: statePayload, options: [.prettyPrinted, .sortedKeys])
        try stateData.write(to: stateURL, options: .atomic)
    ...
    func testRealBrowserDynamicFeedAndOnlineTaskWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["EGAKIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set EGAKIUM_REAL_BROWSER_SMOKE=1 to run the real browser dynamic feed/task smoke")
        }
        ...
        let feedHTML = """
        <!doctype html>
        <title>Egakium Social Feed Smoke</title>
        <style>
        ...
        let taskHTML = """
        <!doctype html>
        <title>Egakium Online Task</title>
        <body>
        <h1>Online Task</h1>
        ...
        let stateURL = ws.appendingPathComponent(".egakium/browser/state/feed-task-smoke.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
    ...
    func testBrowserExecutionDeclaresAllManagedPaths() throws {
        ...
        XCTAssertEqual(Set(paths), Set([
            ".egakium/browser/profiles/work",
            ".egakium/browser/downloads/work",
            ".egakium/browser/state/work.json",
    ...
    func testBrowserExecutionRejectsReadOnlyLeaseBeforeCreatingState() async throws {
        ...
        XCTAssertFalse(
            FileManager.default.fileExists(
                atPath: ws.appendingPathComponent(".egakium").path))
    }
    ...
    func testBrowserExecutionRejectsNarrowOrDeniedManagedPathsBeforeCreatingState() async throws {
        ...
        let narrowLease = WorkspaceLease(
            ...
            access: .readWrite,
            allowedPathRules: [
                PathRule(pattern: ".egakium/browser/profiles/**"),
            ],
            deniedPatterns: [])
    ...
    func testBrowserDiagnosticsIgnoresWorkspacePlaywrightModule() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("egakium-node-outside-\(UUID().uuidString).txt")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
            unsetenv("EGAKIUM_NODE_HOST_MARKER")
        }
        try Data("outside-node-marker".utf8).write(to: outside)
        setenv("EGAKIUM_NODE_HOST_MARKER", "host-node-marker", 1)
        ...
        fs.writeFileSync('workspace-playwright-loaded', `${process.env.EGAKIUM_NODE_HOST_MARKER || ''}|${outside}`);
    ...
    func testBrowserProfilesListsMetadataWithoutReadingProfileDatabaseContents() async throws {
        ...
        let cookieFile = ws.appendingPathComponent(".egakium/browser/profiles/work/Default/Cookies")
        ...
        let activeMarkerFile = ws.appendingPathComponent(".egakium/browser/profiles/work/DevToolsActivePort")
        let lockMarkerFile = ws.appendingPathComponent(".egakium/browser/profiles/work/SingletonLock")
        ...
        let downloadURL = ws.appendingPathComponent(".egakium/browser/downloads/work/report.pdf")
        ...
        XCTAssertTrue(obs.text.contains("profile dir: .egakium/browser/profiles/work (present;"))
        XCTAssertTrue(obs.text.contains("state file: .egakium/browser/state/work.json (present)"))
    ...
    func testBrowserProfileDeleteRequiresConfirmationAndOnlyDeletesTargetProfile() async throws {
        ...
        let workCookieFile = ws.appendingPathComponent(".egakium/browser/profiles/work/Default/Cookies")
        let personalCookieFile = ws.appendingPathComponent(".egakium/browser/profiles/personal/Default/Cookies")
        ...
        let workStateURL = ws.appendingPathComponent(".egakium/browser/state/work.json")
        let personalStateURL = ws.appendingPathComponent(".egakium/browser/state/personal.json")
        ...
        let workDownload = ws.appendingPathComponent(".egakium/browser/downloads/work/report.pdf")
        let personalDownload = ws.appendingPathComponent(".egakium/browser/downloads/personal/keep.pdf")
        ...
        XCTAssertTrue(obs.text.contains("removed profile data: yes (.egakium/browser/profiles/work)"))
        XCTAssertTrue(obs.text.contains("removed state file: yes (.egakium/browser/state/work.json)"))
        XCTAssertTrue(obs.text.contains("removed downloads: yes (.egakium/browser/downloads/work)"))
    ...
    func testBrowserSearchReportsResultTextLinksAndHistory() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let stdout = #"{"action":"search","profile":"research","backend":"cdp","backendDetail":"edge","url":"https://duckduckgo.com/?q=Egakium%20agent","title":"Egakium agent at DuckDuckGo","text":"Search results for Egakium agent","links":[{"text":"Egakium project","href":"https://example.com/egakium"}],"elements":[{"role":"textbox","name":"Search","selector":"input[name=q]","tag":"input","type":"search"}]}"#
        let ctx = ToolContext(
            workspaceRoot: ws,
            shell: FakeShell(result: ShellResult(stdout: stdout, stderr: "", exitCode: 0)),
            git: FakeGit(statusText: "", diffText: ""))
        ...
        let obs = try await BrowserSearchTool().execute(
            ToolArgs(raw: #"{"query":"Egakium agent","engine":"duckduckgo","profile":"research","channel":"msedge","waitMillis":100}"#),
            in: ctx)

        XCTAssertTrue(obs.text.contains("browser action: search"), obs.text)
        XCTAssertTrue(obs.text.contains("title: Egakium agent at DuckDuckGo"), obs.text)
        XCTAssertTrue(obs.text.contains("Egakium project - https://example.com/egakium"), obs.text)
        XCTAssertTrue(obs.text.contains(#"textbox "Search" selector=input[name=q] type=search"#), obs.text)
        XCTAssertTrue(obs.text.contains("Search results for Egakium agent"), obs.text)

        let stateURL = ws.appendingPathComponent(".egakium/browser/state/research.json")
        let stateData = try Data(contentsOf: stateURL)
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: stateData) as? [String: Any])
        XCTAssertEqual(state["url"] as? String, "https://duckduckgo.com/?q=Egakium%20agent")
    ...
    func testBrowserDownloadReportsChangedFileAndDownloadsListMetadataOnly() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let stdout = #"{"action":"download","profile":"work","url":"https://example.com/files","title":"Files","text":"Downloaded","links":[],"downloads":[{"filename":"report.pdf","path":".egakium/browser/downloads/work/report.pdf","url":"https://example.com/report.pdf","bytes":7}]}"#
        let ctx = ToolContext(
            workspaceRoot: ws,
            shell: FakeShell(result: ShellResult(stdout: stdout, stderr: "", exitCode: 0)),
            git: FakeGit(statusText: "", diffText: ""))
        ...
        XCTAssertEqual(download.changedFiles, [".egakium/browser/downloads/work/report.pdf"])
        XCTAssertTrue(download.text.contains("downloads:"))
        XCTAssertTrue(download.text.contains("report.pdf -> .egakium/browser/downloads/work/report.pdf"))

        let downloadedFile = ws.appendingPathComponent(".egakium/browser/downloads/work/report.pdf")
        ...
        XCTAssertTrue(listed.text.contains(".egakium/browser/downloads/work/report.pdf"))
    ...
    func testStandardRegistry() {
        ...
        XCTAssertEqual(reg.registryVersion, "egakium.standard.v4")
