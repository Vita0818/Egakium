...
"""#,
    ...
    func testUnexpectedPythonExceptionReturnsOnlyFixedSanitizedSummary() throws {
        ...
        environment["EGAKIUM_DOCUMENT_REQUEST"] = "{not-json-/private/secret.xlsx"
        environment["EGAKIUM_DOCUMENT_OPERATION"] = "read"
    ...
    func testOOXMLCentralDirectoryPreflightRejectsUnsafeMembersBeforeLibraryOpen() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-ooxml-preflight-\(UUID().uuidString)", isDirectory: true)
...
"""#,
    ...
    func testOOXMLPreflightRejectsPackageWhoseContentsDoNotMatchRequestedFormat() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-ooxml-format-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testFixedWritersCreateAndEditSupportedNativeFormats() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-write-tests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testVerifyWriteChecksCompleteXLSXDeclaration() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-verify-xlsx-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testXLSXFormulaVerificationRequiresAndReadsCalculatedCache() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-xlsx-formula-cache-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
...
"""#,
    ...
    func testXLSXEditRejectsPreservationHazardsAndExternalFormulas() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-xlsx-preservation-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
...
"""#,
    ...
    func testVerifyHTMLAcceptsOnlyExplicitAllowlistedLocalAssets() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-verify-html-assets-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testWriteVerifiersHonorFinalStateAfterConsecutiveOverwrites() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-final-state-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testImageVerificationBindsEmbeddedContentDigest() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-image-digest-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testPPTXChartVerificationBindsSeriesDataCategoriesAndTitle() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-pptx-chart-verification-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testWriteRouteRejectsExecutionControlFields() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-write-rejection-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testVerifyWriteRejectsExtraPayloadFields() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-document-verify-rejection-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testPrepareHTMLRenderInlinesAllowlistedImagesAndRejectsResponsiveSources() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-html-render-prepare-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testReadDOCXReturnsBoundedMarkdownAndRejectsProjectionControls() throws {
        let runtime = try requireFixedRuntime()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-docx-read-flags-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: false)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testHTMLRendererRendersSanitizedInputInsideStageReadRoot() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-html-render-positive-\(UUID().uuidString)", isDirectory: true)
    ...
    func testHTMLRendererRejectsInputOutsideItsStageReadRoot() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-html-render-read-root-\(UUID().uuidString)", isDirectory: true)
        let stage = root.appendingPathComponent("stage", isDirectory: true)
        try FileManager.default.createDirectory(at: stage, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    private func requireFixedRuntime() throws -> URL {
        guard let root = egakiumDocumentRuntimeRoot() else {
