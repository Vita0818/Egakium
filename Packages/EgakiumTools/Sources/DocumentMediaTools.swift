...
public struct CompileLaTeXTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard inputURL.pathExtension.lowercased() == "tex" else {
            throw EgakiumError.decoding("compile_latex inputPath must point to a .tex file")
        }
        ...
        guard result.exitCode == 0 else {
            throw EgakiumError.io("LaTeX compile failed. \(transcript)")
        }
        guard FileManager.default.fileExists(atPath: outputPDF.path) else {
            throw EgakiumError.io("LaTeX compile finished but did not create \(outputPDF.lastPathComponent). \(transcript)")
        }
...
public struct GenerateImageTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard let generator = context.imageGenerator else {
            throw EgakiumError.config("generate_image is not configured; attach an image provider or local image backend before using this tool")
        }
...
public struct EditImageTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard let generator = context.imageGenerator else {
            throw EgakiumError.config("edit_image is not configured; attach an image provider or local image backend before using this tool")
        }
