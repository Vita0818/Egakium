...
import Crypto
#else
#error("EgakiumTools requires CryptoKit or swift-crypto")
#endif
import EgakiumCore
...
public struct ToolArgs: Sendable {
    ...
    public func decode<T: Decodable>(_ type: T.Type) throws -> T {
        guard let data = raw.data(using: .utf8) else {
            throw EgakiumError.decoding("tool args are not valid UTF-8")
        }
        do {
            return try JSONDecoder().decode(T.self, from: data)
        } catch {
            throw EgakiumError.decoding("tool args: \(error.localizedDescription)")
        }
    }
}
...
/// prevents clients outside EgakiumTools from manufacturing a process launch.
...
/// implementations resolve the executable from a fixed Egakium runtime and
...
/// callers cannot turn this into a general shell: only Egakium browser tools
...
struct BrowserBackendInvocation: Sendable {
    ...
    var injectedShellCommand: String {
        """
        set -e
        command -v node >/dev/null 2>&1 || { echo "node is not installed; install Node.js to use Egakium browser tools" >&2; exit 127; }
        EGAKIUM_BROWSER_ARGS='\(encodedArguments)' node <<'EGAKIUM_BROWSER_INJECTED_NODE'
        \(javaScript)
        EGAKIUM_BROWSER_INJECTED_NODE
        """
    }
}
...
public struct ToolContext: Sendable {
    ...
    /// Dedicated backend for Egakium-generated Playwright/CDP browser
...
public struct ToolRegistry: Sendable {
    ...
    public init(_ tools: [any Tool], registryVersion: String = "egakium.standard.v1") {
        self.init(
    ...
    public static func standard(
        ...
        return ToolRegistry(
            registrations: registrations,
            registryVersion: "egakium.standard.v4")
    }
}
