...
public actor ProcessTerminalSessionManager: TerminalSessionManaging {
    ...
    public func execute(_ request: TerminalExecRequest,
        ...
        guard !isShutDown else {
            throw EgakiumError.config("the terminal manager is shut down")
        }
        pruneCompletionTombstones()
        guard sessions.count < maximumSessions else {
            throw EgakiumError.io("the terminal session limit (\(maximumSessions)) has been reached")
        }
        guard request.command.isEmpty == false else {
            throw EgakiumError.decoding("exec_command requires a non-empty command")
        }
        ...
        guard (250...30_000).contains(request.yieldMilliseconds) else {
            throw EgakiumError.decoding("yield_time_ms must be between 250 and 30000")
        }
        guard (1_000...1_800_000).contains(request.timeoutMilliseconds) else {
            throw EgakiumError.decoding("timeout_ms must be between 1000 and 1800000")
        }
        guard ["/bin/zsh", "/bin/bash", "/bin/sh"].contains(request.shellPath),
            ...
            throw EgakiumError.permissionDenied("the requested shell is not available")
        }
        ...
        guard lease.rootIdentity == owner.workspaceRootIdentity else {
            throw EgakiumError.permissionDenied("terminal owner does not match the workspace lease")
        }
        ...
        guard FileManager.default.fileExists(
            ...
            throw EgakiumError.notFound("terminal working directory does not exist")
        }

        let runtime = FileManager.default.temporaryDirectory
            .appendingPathComponent("egakium-terminal-\(UUID().uuidString)", isDirectory: true)
        ...
        do {
            ...
            throw EgakiumError.io("could not create the terminal runtime directory")
        }
    ...
    public func interact(sessionID: String,
        ...
        guard (250...30_000).contains(request.yieldMilliseconds) else {
            throw EgakiumError.decoding("yield-time_ms must be between 250 and 30000")
        }
        pruneCompletionTombstones()
        guard var session = sessions[sessionID] else {
            ...
            throw EgakiumError.notFound("terminal session is not active")
        }
        guard session.owner == owner else {
            throw EgakiumError.permissionDenied("terminal session belongs to another agent or task")
        }
        ...
        if let characters = request.characters, characters.isEmpty == false {
            guard characters.utf8.count <= 32 * 1_024 else {
                throw EgakiumError.decoding("terminal input exceeds 32768 UTF-8 bytes")
            }
            ...
            guard !session.inputClosed, let descriptor = session.stdinDescriptor else {
                throw EgakiumError.io("terminal input is already closed")
            }
    ...
    private func launch(_ request: TerminalExecRequest,
        ...
        guard FileManager.default.createFile(atPath: startupMarker.path, contents: Data()) else {
            throw EgakiumError.io("could not create the terminal startup marker")
        }
        ...
        #else
        throw EgakiumError.config("terminal execution is unsupported on this platform")
        #endif
        ...
        if request.usesTTY {
            #if !os(macOS)
            throw EgakiumError.config("PTY mode is currently unavailable on this platform")
            #endif
        } else {
            var pipeDescriptors = [Int32](repeating: -1, count: 2)
            guard systemPipe(&pipeDescriptors) == 0 else {
                throw EgakiumError.io("could not create terminal input pipe")
            }
            ...
            guard systemPipe(&outputPipe) == 0 else {
                throw EgakiumError.io("could not create terminal output pipe")
            }
            ...
            guard systemPipe(&errorPipe) == 0 else {
                throw EgakiumError.io("could not create terminal error pipe")
            }
        ...
        guard let stdoutDrain else {
            killAndReapManagedProcess(pid)
            throw EgakiumError.io("could not start terminal output supervision")
        }
...
private func terminalEnvironment(home: URL, temporary: URL) -> [String: String] {
    ...
    environment["EGAKIUM_MANAGED_TERMINAL"] = "1"
    return environment
}
...
private func duplicateDescriptor(_ descriptor: Int32) throws -> Int32 {
    ...
    guard duplicate >= 0 else {
        throw EgakiumError.io("could not duplicate terminal output descriptor")
    }
    ...
    guard descriptorFlags >= 0,
        ...
        throw EgakiumError.io("could not isolate terminal output descriptor")
    }
    return duplicate
}
...
public struct ExecCommandTool: Tool {
    ...
    public func execute(_ args: ToolArgs,
        ...
        guard let terminal = context.terminal else {
            throw EgakiumError.config("this runtime does not own a terminal manager")
        }
