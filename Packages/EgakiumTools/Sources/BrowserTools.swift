...
private enum BrowserToolConfig {
    ...
    static func normalizedProfile(_ raw: String?) throws -> String {
        ...
        guard profile.count <= 64,
            ...
            throw EgakiumError.decoding("browser profile must use only letters, numbers, '.', '-' or '_'")
        }
        return profile
    }
    ...
    static func browserRoot(in workspace: URL) throws -> URL {
        try PathConfinement.resolve(".egakium/browser", within: workspace)
    }

    static func profileURL(profile: String, workspace: URL) throws -> URL {
        try PathConfinement.resolve(".egakium/browser/profiles/\(profile)", within: workspace)
    }

    static func downloadsURL(profile: String, workspace: URL) throws -> URL {
        try PathConfinement.resolve(".egakium/browser/downloads/\(profile)", within: workspace)
    }

    static func stateURL(profile: String, workspace: URL) throws -> URL {
        try PathConfinement.resolve(".egakium/browser/state/\(profile).json", within: workspace)
    }

    static func historyURL(workspace: URL) throws -> URL {
        try PathConfinement.resolve(".egakium/browser/history.jsonl", within: workspace)
    }
    ...
    static func validatedHTTPURL(_ raw: String) throws -> URL {
        guard let url = URL(string: raw.trimmingCharacters(in: .whitespacesAndNewlines)),
            ...
            throw EgakiumError.config("URL must be http(s) with a host")
        }
        return url
    }
...
private func browserJSONLine(from stdout: String) throws -> String {
    ...
    throw EgakiumError.decoding("browser backend did not return JSON")
}
...
private func browserDownloadsOutput(_ files: [BrowserDownloadFile], profile: String, limit: Int) -> String {
    ...
    lines.append("downloads dir: .egakium/browser/downloads/\(profile)")
    if files.isEmpty {
        lines.append("")
...
private func browserProfileMetadata(profile: String,
    ...
    return BrowserProfileMetadata(
        profile: profile,
        profileDir: ".egakium/browser/profiles/\(profile)",
        downloadsDir: ".egakium/browser/downloads/\(profile)",
        stateFile: ".egakium/browser/state/\(profile).json",
...
private func browserProfilesOutput(_ profiles: [BrowserProfileMetadata],
    ...
    lines.append("limit: \(limit)")
    lines.append("metadata only: reads Egakium profile names, state, history, download metadata, directory stats, and browser runtime marker existence; does not read cookies, localStorage, marker contents, or browser profile databases.")
    if profiles.isEmpty {
        lines.append("")
...
private func browserHistoryNavigationURL(direction: BrowserHistoryDirection, paths: BrowserPaths) throws -> String {
    ...
    guard snapshot.stack.isEmpty == false else {
        throw EgakiumError.config("no current browser history for this profile; call browser_navigate or browser_search first")
    }
    let targetIndex = snapshot.index + direction.offset
    guard snapshot.stack.indices.contains(targetIndex) else {
        throw EgakiumError.config(direction.missingEntryMessage)
    }
...
private func browserProfileDeleteOutput(profile: String,
    ...
    var lines = [
        ...
        "scope: workspace .egakium/browser metadata and profile data only",
    ]
...
private func playwrightCommand(
    ...
    const args = JSON.parse(Buffer.from(process.env.EGAKIUM_BROWSER_ARGS, 'base64').toString('utf8'));
...
private func runPlaywrightUnlocked(arguments: [String: Any],
    ...
    guard shellResult.exitCode == 0 else {
        ...
        throw EgakiumError.io("browser backend failed: \(message)")
    }
    ...
    let profileHint = "\n\nPersistent browser profile: .egakium/browser/profiles/\(paths.profile)\nHistory metadata: .egakium/browser/history.jsonl"
...
private func normalizedBrowserKey(_ raw: String) throws -> String {
    ...
    guard !value.isEmpty, value.count <= 80 else {
        throw EgakiumError.decoding("browser_press_key key must be 1-80 characters")
    }
    guard value.unicodeScalars.allSatisfy({ !CharacterSet.controlCharacters.contains($0) }) else {
        throw EgakiumError.decoding("browser_press_key key must not contain control characters")
    }
    return value
}
...
private func browserScrollDelta(direction: String?, amount: Int?, deltaX: Int?, deltaY: Int?) throws -> (x: Int, y: Int) {
    if deltaX != nil || deltaY != nil {
        ...
        let y = deltaY ?? 0
        guard x != 0 || y != 0 else {
            throw EgakiumError.decoding("browser_scroll requires a non-zero delta")
        }
        return (x, y)
    }
    ...
    case "left":
        return (-pixels, 0)
    default:
        throw EgakiumError.decoding("browser_scroll direction must be down, up, right, or left")
    }
}
...
private func browserHistoryOutput(_ entries: [BrowserHistoryEntry], profile: String?, limit: Int) -> String {
    ...
    lines.append("")
    lines.append("History metadata: .egakium/browser/history.jsonl")
    return lines.joined(separator: "\n")
}
...
public struct WebFetchTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        request.setValue("EgakiumAgent/0.16", forHTTPHeaderField: "User-Agent")
...
public struct BrowserDiagnosticsTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard shellResult.exitCode == 0 else {
            var message = shellResult.stderr.isEmpty ? shellResult.stdout : shellResult.stderr
            if message.count > 10_000 { message = String(message.prefix(10_000)) + "\n[truncated]" }
            throw EgakiumError.io("browser diagnostics failed: \(message)")
        }
        let jsonLine = try browserJSONLine(from: shellResult.stdout)
...
public struct BrowserProfilesTool: Tool {
    ...
    public func touchedPaths(_ args: ToolArgs) -> [String] {
        guard let decoded = try? args.decode(Args.self),
            ...
            return [
                ".egakium/browser/profiles",
                ".egakium/browser/state",
                ".egakium/browser/history.jsonl",
                ".egakium/browser/downloads",
            ]
        }
        return [
            ".egakium/browser/profiles/\(profile)",
...
public struct BrowserProfileDeleteTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        ...
        description: "Delete one workspace-scoped persistent browser profile, including its state, downloads, and Egakium history metadata. Requires confirmProfile to match profile.",
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard confirmed == profile else {
            throw EgakiumError.config("confirmProfile must exactly match profile to delete browser profile data")
        }
...
public struct BrowserHistoryTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        ...
        description: "Read recent Egakium browser history metadata without exposing cookies, local storage, or credential files.",
        sideEffect: .readOnly,
        parameters: Schema.object([
            "profile": Schema.nonEmptyString,
            "limit": Schema.boundedInteger(minimum: 1, maximum: 100),
...
public struct BrowserClickTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard a.selector?.isEmpty == false || a.text?.isEmpty == false || (a.role?.isEmpty == false && a.name?.isEmpty == false) else {
            throw EgakiumError.decoding("browser_click requires selector, text, or role+name")
        }
        let profile = try BrowserToolConfig.normalizedProfile(a.profile)
        let paths = try BrowserToolConfig.prepare(
            profile: profile,
...
public struct BrowserTypeTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        let a = try args.decode(Args.self)
        guard a.selector?.isEmpty == false || a.text?.isEmpty == false || (a.role?.isEmpty == false && a.name?.isEmpty == false) else {
            throw EgakiumError.decoding("browser_type requires selector, text, or role+name")
        }
        if let reason = browserSensitiveTypeTargetReason(selector: a.selector, text: a.text, role: a.role, name: a.name) {
            throw EgakiumError.config("browser_type refuses likely sensitive credential entry target (\(reason)); use browser_handoff for login, password, 2FA, token, or API key entry")
        }
        let profile = try BrowserToolConfig.normalizedProfile(a.profile)
        let paths = try BrowserToolConfig.prepare(
            profile: profile,
...
public struct BrowserSelectOptionTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        let a = try args.decode(Args.self)
        guard a.selector?.isEmpty == false || a.text?.isEmpty == false || (a.role?.isEmpty == false && a.name?.isEmpty == false) else {
            throw EgakiumError.decoding("browser_select_option requires selector, text, or role+name")
        }
        ...
        if let optionIndex = a.optionIndex, optionIndex < 0 || optionIndex > 500 {
            throw EgakiumError.decoding("browser_select_option optionIndex must be between 0 and 500")
        }
        guard optionValue?.isEmpty == false || optionLabel?.isEmpty == false || a.optionIndex != nil else {
            throw EgakiumError.decoding("browser_select_option requires optionValue, optionLabel, or optionIndex")
        }
        let profile = try BrowserToolConfig.normalizedProfile(a.profile)
        let paths = try BrowserToolConfig.prepare(
            profile: profile,
...
public struct BrowserScreenshotTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard outputURL.pathExtension.lowercased() == "png" else {
            throw EgakiumError.decoding("browser_screenshot outputPath must end with .png")
        }
        _ = try validateBrowserWorkspaceAccess(
            readablePaths: [],
...
public struct BrowserUploadFileTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        let a = try args.decode(Args.self)
        guard a.selector?.isEmpty == false || a.text?.isEmpty == false || (a.role?.isEmpty == false && a.name?.isEmpty == false) else {
            throw EgakiumError.decoding("browser_upload_file requires selector, text, or role+name")
        }
        ...
        guard FileManager.default.fileExists(atPath: fileURL.path, isDirectory: &isDirectory), !isDirectory.boolValue else {
            throw EgakiumError.notFound("upload file not found: \(a.filePath)")
        }
...
public struct BrowserDownloadTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        let a = try args.decode(Args.self)
        guard a.selector?.isEmpty == false || a.text?.isEmpty == false || (a.role?.isEmpty == false && a.name?.isEmpty == false) else {
            throw EgakiumError.decoding("browser_download requires selector, text, or role+name")
        }
        let profile = try BrowserToolConfig.normalizedProfile(a.profile)
        let paths = try BrowserToolConfig.prepare(
            profile: profile,
        ...
        var payload: [String: Any] = [
            ...
            "profileDir": paths.profileDir.path,
            "downloadsDir": paths.downloadsDir.path,
            "downloadsRelativeDir": ".egakium/browser/downloads/\(profile)",
            "stateFile": paths.stateFile.path,
        ]
        if let selector = a.selector { payload["selector"] = selector }
        if let text = a.text { payload["text"] = text }
...
public struct BrowserDownloadsTool: Tool {
    ...
    public func touchedPaths(_ args: ToolArgs) -> [String] {
        let profile = (try? BrowserToolConfig.normalizedProfile((try? args.decode(Args.self))?.profile)) ?? BrowserToolConfig.defaultProfile
        return [".egakium/browser/downloads/\(profile)"]
    }
