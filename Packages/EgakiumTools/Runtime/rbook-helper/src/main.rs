...
fn run() -> HelperResult<Value> {
    ...
    let encoded = env::var("EGAKIUM_DOCUMENT_REQUEST").map_err(|_| HelperError::validation())?;
    ...
    let operation_env =
        env::var("EGAKIUM_DOCUMENT_OPERATION").map_err(|_| HelperError::validation())?;
...
fn set_metadata(epub: &mut Epub, field: &str, value: &str) -> HelperResult<()> {
    ...
    if field == "identifier" {
        let mut id = String::from("egakium_unique_identifier");
        let mut suffix = 1_usize;
        while package_id_in_use(epub, &id) {
            id = format!("egakium_unique_identifier_{suffix}");
            suffix += 1;
        }
        entry = entry.id(&id);
...
fn unique_toc_id(epub: &Epub, operation_index: usize) -> String {
    let base = format!("egakium_toc_{operation_index}");
...
mod tests {
    ...
    fn create_payload(directory: &TestDirectory, output: &Path) -> WritePayload {
        ...
        WritePayload {
            ...
            operations: vec![
                metadata("identifier", "urn:egakium:test"),
