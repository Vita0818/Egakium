...
public struct KnowledgeSecureFileSystem: Sendable {
    ...
    public static func canonicalBundleDigest(_ entries: [KnowledgeChecksumEntry]) throws -> String {
        ...
        return try KnowledgeDigest.canonical(
            Projection(version: "egakium-okf-bundle-digest/1", files: selected))
    }
    ...
    public func snapshotSealDigest(root: URL) throws -> String {
        ...
        return try KnowledgeDigest.canonical(Projection(
            version: "egakium-knowledge-snapshot-content-seal/1",
            files: files))
    }
    ...
    private static func role(for path: String) -> String {
        if path == ".egakium-rag/profile.json" { return "profile" }
        if path == ".egakium-rag/chunks.jsonl" { return "chunk_manifest" }
        if path.hasPrefix(".egakium-rag/dense/") { return "dense_index" }
        if path.hasPrefix(".egakium-rag/lexical/") { return "lexical_index" }
        if path.hasPrefix(".egakium-rag/auxiliary/") { return "auxiliary" }
