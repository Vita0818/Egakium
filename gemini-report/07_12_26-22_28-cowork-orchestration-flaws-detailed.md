...
- **涉及文件**：
  - `Packages/EgakiumCowork/Sources/Orchestrator.swift` (包含 `Orchestrator` actor，控制整体生命周期)
  - `Packages/EgakiumCowork/Sources/MessageBus.swift` (跨 Agent 消息投递 actor)
  - `Packages/EgakiumAgentKernel/Sources/AgentRuntime.swift`
- **缺陷表现**：如果在执行某个工具或者沙盒操作（如 `Packages/EgakiumTools/Sources/ShellGit.swift` 里的底层 Process 调用）时发生宿主进程崩溃，所有并行的 Agent 将全部挂掉。系统设计了 `JSONRPC.swift` 的词汇但目前没有实装真正的 Daemon 隔离。

### 2. 协作颗粒度僵化：统一模型与单工作区
- **统一模型缺陷**：
  - **涉及文件**：`Apps/EgakiumMac/Sources/CoworkProjectSettings.swift`
  - **缺陷表现**：该文件管理的是 per-session 的全局项目设置，它仅包含一个统一的默认 `provider/model`，系统无法给不同的 Agent（如主节点和子节点）配置不同的 Provider 和模型。
- **单工作区限制**：
  - **涉及文件**：`Packages/EgakiumTools/Sources/PathConfinement.swift` 和 `WorkspaceLease`。
...
- **涉及文件**：`Packages/EgakiumCowork/Sources/PermissionReviewControlPlane.swift`
- **缺陷表现**：
...
- **Soft Budget 限额风险**：
  - **涉及文件**：`Packages/EgakiumAgentKernel/Sources/AgentExecutionBudget.swift` 以及 `PermissionReviewControlPlane.swift` (第 367-373 行)。
...
- **协作式的 Cancellation**：
  - **涉及文件**：`Packages/EgakiumAgentKernel/Sources/AgentLoop.swift`。
...
- **涉及文件**：
  - `Packages/EgakiumConversation/Sources/EventLog.swift` (JSONL 追加日志)
  - `Packages/EgakiumConversation/Sources/CoworkProjection.swift`
  - `Packages/EgakiumAgentKernel/Sources/ContextProjector.swift`
