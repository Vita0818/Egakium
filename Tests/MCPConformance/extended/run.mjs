...
const client = process.env.EGAKIUM_MCP_CONFORMANCE_CLIENT;
if (!client) throw new Error("EGAKIUM_MCP_CONFORMANCE_CLIENT is required");

const scenarios = [
  "egakium/task-complete",
  "egakium/task-timeout",
  "egakium/task-cancel"
];
...
for (const scenario of scenarios) {
  ...
  try {
    ...
    assertSubsequence(
      methods,
      scenario === "egakium/task-complete"
...
console.log(
  "Egakium MCP extended interoperability complete: task creation, poll, result, timeout, and cancellation"
);
  ...
  switch (method) {
    case "initialize":
      result = {
        ...
        serverInfo: { name: "egakium-task-peer", version: "1.0.0" },
        capabilities: {
          tools: {},
          tasks: {
            cancel: {},
    ...
    case "tasks/get":
      result = taskWire(
        taskID,
        scenario === "egakium/task-complete" ? "completed" : "working",
        now
      );
      break;
    case "tasks/result":
