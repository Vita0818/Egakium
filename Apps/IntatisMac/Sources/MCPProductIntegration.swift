    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            case (.macDeveloperID, .stdio),
                ...
                guidanceStep(
                    1,
                    "Install the MCP server using its trusted publisher's instructions outside Ekagium.")
                guidanceStep(
                    2,
                ...
                guidanceStep(
                    4,
                    "Run Test & Save. Ekagium captures and revalidates the exact launch artifacts before every managed start.")
                Text(
                    "Ekagium does not run an install command, arbitrary shell string, or an unverified executable on behalf of this form.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            case (.linuxCLI, .stdio):
                guidanceStep(
