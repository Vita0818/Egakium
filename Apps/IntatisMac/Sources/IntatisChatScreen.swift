...
struct IntatisMessageBubble: View {
    ...
    private var roleLabel: String? {
        ...
        case .assistant: return "Ekagium"
        case .agent:     return message.agent?.rawValue ?? "Ekagium"
...
struct IntatisComposer: View {
    ...
    var body: some View {
        IntatisThreadComposer(
            placeholder: IntatisLocalization.string("Message Ekagium..."),
            input: $model.input,
            canSend: canSend,
...
struct IntatisSettingsPanel: View {
    ...
    private var openJSONButton: some View {
        Button(action: openJSONConfig) {
            Label("Open Ekagium Config", systemImage: "curlybraces")
                .font(IntatisType.body(14, .semibold))
                .foregroundStyle(.primary)
        }
        .intatisGlassButton()
