...
//  EgakiumMac
//
//  Egakium delegates its palette to macOS. The window canvas, content material,
...
struct EgakiumSystemCanvas: View {
...
private struct EgakiumLegacyWindowBackground: NSViewRepresentable {
...
extension EgakiumThreadStyle {
    static func egakiumMac(_ scheme: ColorScheme) -> EgakiumThreadStyle {
        EgakiumThreadStyle(
            ...
            accent: EgakiumTheme.accent(scheme),
            stroke: EgakiumTheme.separator(scheme),
            cardStroke: EgakiumTheme.separator(scheme),
            error: .red)
    }
}
...
private struct EgakiumCardModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay {
                shape.stroke(EgakiumTheme.separator(scheme), lineWidth: 1)
            }
    }
}

extension View {
    func egakiumCard(cornerRadius: CGFloat = 22) -> some View {
        modifier(EgakiumCardModifier(cornerRadius: cornerRadius))
    }
}
...
struct EgakiumPageHeader: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(EgakiumType.largeTitle(30))
                .foregroundStyle(EgakiumTheme.deepText(scheme))
            if let subtitle {
                Text(subtitle)
                    .font(EgakiumType.caption(13, .medium))
