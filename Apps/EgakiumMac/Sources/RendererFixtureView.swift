#if (DEBUG || EGAKIUM_RENDERER_VALIDATION) && canImport(SwiftUI)
import CryptoKit
import Foundation
...
struct RendererFixtureView: View {
    ...
    private var fixtureHeader: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Picker("Renderer", selection: rendererModeSelection) {
                ...
                Text("Plain safe").tag(EgakiumMessageRendererMode.plainSafe.rawValue)
            }
    ...
    | Egakium facade | Latest-only admission and raw fallback | active |
    ...
    | Egakium facade | Backpressure and safe fallback |
...
private struct RendererFixtureLaunchConfiguration {
    static let stageFlag = "-EgakiumRendererFixtureStage"
    static let incidentFixtureFlag = "-EgakiumRendererIncidentFixture"
    static let autoExitFlag = "-EgakiumRendererFixtureAutoExitSeconds"
    static let resultPathFlag = "-EgakiumRendererFixtureResultPath"
    static let disableSingleDollarMathFlag = "-EgakiumDisableSingleDollarMath"
...
private final class RendererIncidentReplayModel: ObservableObject {
    ...
    func loadIfNeeded() {
        ...
        guard let fixturePath else {
            fail("Missing -EgakiumRendererIncidentFixture PATH")
            return
        }
...
/// `EgakiumThreadScrollCoordinator`, follow/detach state, viewport admission,
    ...
    private func loadFixture() throws -> RendererIncidentFixture {
        guard let fixturePath else {
            throw RendererThreadBurstError.invalidFixture(
                "Missing -EgakiumRendererIncidentFixture PATH")
        }
        let data = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
