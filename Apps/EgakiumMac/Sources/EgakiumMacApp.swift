...
// The shell lives in EgakiumMacRootView; root-owned session state feeds the
...
#if canImport(AppKit)
@MainActor
final class EgakiumApplicationDelegate: NSObject, NSApplicationDelegate {
    private var terminationTask: Task<Void, Never>?
    #if EGAKIUM_RENDERER_VALIDATION
    private var rendererValidationWindowController:
        NSWindowController?
    private var cefCanvasFixtureWorkspace: URL?
    #endif

    func applicationWillFinishLaunching(_ notification: Notification) {
        #if !EGAKIUM_MAC_APP_STORE
        if !EgakiumCEFInitialize() {
            NSLog(
                "Egakium Canvas CEF initialization failed: %@",
                EgakiumCEFInitializationError()
                    ?? "unknown CEF initialization failure")
        }
        #endif
    }

    func applicationDidFinishLaunching(_ notification: Notification) {
        EgakiumMacProcessDiagnostics.shared.start(
            application: NSApplication.shared)
        #if EGAKIUM_RENDERER_VALIDATION
        let arguments = ProcessInfo.processInfo.arguments
        let usesRendererFixture = arguments.contains("-EgakiumRendererFixture")
        let usesCEFCanvasFixture = arguments.contains("-EgakiumCEFCanvasFixture")
        guard (usesRendererFixture || usesCEFCanvasFixture),
              let rawSeconds = Self.argumentValue(
                  after: "-EgakiumRendererFixtureAutoExitSeconds"),
              let seconds = Double(rawSeconds),
              (1...300).contains(seconds)
        else { return }
        let finalizeFixtureResult = usesRendererFixture
            ? RendererFixtureResultLifecycle.configure(arguments: arguments)
            : nil
        _ = NSApplication.shared.setActivationPolicy(
            .regular)
        let fixtureView: AnyView
        if usesCEFCanvasFixture {
            do {
                let workspace = FileManager.default.temporaryDirectory
                    .appendingPathComponent(
                        "egakium-cef-canvas-fixture-\(UUID().uuidString)",
                        isDirectory: true)
                try FileManager.default.createDirectory(
                    at: workspace,
                    withIntermediateDirectories: false)
                cefCanvasFixtureWorkspace = workspace
                let document = try SessionCanvasStore.ensureCanvas(
                    in: workspace,
                    sessionID: SessionID(rawValue: "cef_fixture"))
                fixtureView = AnyView(CoworkCanvasHost(
                    document: document,
                    errorMessage: nil))
            } catch {
                fixtureView = AnyView(ContentUnavailableView {
                    Label(
                        "CEF Canvas Fixture Unavailable",
            ...
            fixtureView = AnyView(RendererFixtureView(arguments: arguments))
        }
        let controller = NSHostingController(
            rootView: fixtureView)
        let window = NSWindow(
            contentRect: NSRect(
                x: 0,
                y: 0,
        ...
        window.title = usesCEFCanvasFixture
            ? "Egakium CEF Canvas Validation"
            : "Egakium Renderer Validation"
        window.center()
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        EgakiumMacProcessDiagnostics.shared.beginTermination()
        #if !EGAKIUM_MAC_APP_STORE
        if EgakiumCEFShutdownIsComplete() {
            return .terminateNow
        }
        #endif
        #if EGAKIUM_RENDERER_VALIDATION
        ...
        if ProcessInfo.processInfo.arguments.contains("-EgakiumRendererFixture") {
        ...
        #if DEBUG
        if let raw = Self.argumentValue(after: "-EgakiumShutdownDeadlineMilliseconds"),
        ...
        terminationTask = Task { @MainActor [weak self] in
            NSLog("Egakium shutdown: draining application session runtimes")
            if manager.state != .stopped {
                _ = await manager.shutdownAll(
                    reason: "Application quit requested",
                    deadline: deadline)
            }
            NSLog("Egakium shutdown: closing CEF browsers")
            #if !EGAKIUM_MAC_APP_STORE
            await withCheckedContinuation { continuation in
                EgakiumCEFShutdown {
                    NSLog("Egakium shutdown: CEF shutdown completed")
                    continuation.resume()
                }
            }
            #endif
    ...
    func applicationWillTerminate(_ notification: Notification) {
        EgakiumMacProcessDiagnostics.shared.beginTermination()
        #if EGAKIUM_RENDERER_VALIDATION
        if let cefCanvasFixtureWorkspace {
            try? FileManager.default.removeItem(at: cefCanvasFixtureWorkspace)
            self.cefCanvasFixtureWorkspace = nil
        }
        #endif
    }
...
@main
struct EgakiumMacApp: App {
    #if canImport(AppKit)
    @NSApplicationDelegateAdaptor(EgakiumApplicationDelegate.self)
    ...
    private var launchAppearance: ColorScheme? {
        ...
        let arguments = ProcessInfo.processInfo.arguments
        if arguments.contains("-EgakiumAppearanceDark") { return .dark }
        if arguments.contains("-EgakiumAppearanceLight") { return .light }
        #endif
        return nil
    }
    ...
    private var launchesCoworkAgentConversationFixture: Bool {
        ProcessInfo.processInfo.arguments.contains(
            "-EgakiumCoworkAgentConversationFixture")
    ...
    var body: some Scene {
        WindowGroup {
            ...
            } else if ProcessInfo.processInfo.arguments.contains("-EgakiumPhaseCPermissionFixture") {
...
@MainActor
private struct EgakiumProductionRootView: View {
...
@main
struct EgakiumMacApp {
    static func main() {
        print("EgakiumMac is a macOS SwiftUI app and only runs on macOS.")
    }
}
#endif
