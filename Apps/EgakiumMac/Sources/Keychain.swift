...
public final class ConfigSecretResolver: SecretResolver, @unchecked Sendable {
    ...
    public static func writableAuthFileURL() -> URL {
        if let override = ProcessInfo.processInfo.environment["EGAKIUM_AUTH_FILE"],
           !override.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return URL(fileURLWithPath: expandedPath(override))
        }
        let home = FileManager.default.homeDirectoryForCurrentUser
        return home.appendingPathComponent(".config/egakium/auth.json")
    }
    ...
    private static func authFileURLs() -> [URL] {
        ...
        var urls: [URL] = [
            writableAuthFileURL(),
            home.appendingPathComponent(".local/share/egakium/auth.json"),
        ]
    ...
    private static func providerConfigURLs() -> [URL] {
        if let configOverride = ProcessInfo.processInfo.environment["EGAKIUM_CONFIG"],
        ...
        let home = FileManager.default.homeDirectoryForCurrentUser
        return [
            home.appendingPathComponent(".config/egakium/egakium.json"),
            home.appendingPathComponent(".config/egakium/egakium.jsonc"),
            home.appendingPathComponent(".config/egakium/config.json"),
            home.appendingPathComponent(".config/egakium/config.jsonc"),
