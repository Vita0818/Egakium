...
private struct EgakiumDiagnosticBundleBuilder {
    ...
    func build() throws -> EgakiumDiagnosticExportResult {
        ...
        let bundleName = "Egakium-Diagnostics-\(Self.fileTimestamp(context.generatedAt))"
    ...
    private func collectHangDiagnostics(
        ...
        guard let root = context.hangDiagnosticsRoot else {
            manifest.collectionErrors.append(.init(
                ...
                message: "The Egakium hang diagnostics directory could not be resolved."))
            return
        }
        let keys: Set<URLResourceKey> = [
            .contentModificationDateKey,
    ...
    private func collectCrashReports(
        ...
        for root in context.crashReportRoots {
            ...
            reports.append(contentsOf: urls.compactMap { url in
                ...
                guard (name.hasPrefix("EgakiumMac") || name.hasPrefix("Egakium")),
    ...
    Egakium local diagnostic bundle / Egakium 本地诊断包

    This ZIP was generated only after the user selected Export Diagnostic Logs.
    It was saved to the selected local destination. Egakium did not upload it.
    ...
    - Egakium process Unified Log entries from the last 24 hours
    - current macOS proxy configuration
    - performance counters and retained hang diagnostics
    - recent Egakium crash reports
