...
typealias CoworkDraftAttachment = EgakiumComposerDraftAttachment
...
final class CoworkViewModel: ObservableObject, PermissionResponder {
    ...
    var mainInferenceDisplayLabel: String {
        ...
        return main.inferenceDisplayLabel ?? EgakiumLocalization.format(
    ...
    private let composerAttachmentStore: EgakiumComposerAttachmentStore
    ...
    init(sessionID: SessionID,
        ...
        self.composerAttachmentStore = EgakiumComposerAttachmentStore(
            store: artifactStore)
    ...
    func start() {
        ...
        do {
            ...
            projectionError = EgakiumLocalization.format(
    ...
    private func commitProjectionSnapshot(
        ...
        let changedThreadAgents = EgakiumExecutionTracePresentation.isEnabled
    ...
    private func restoreSubmittedIntentOutbox() async {
        do {
            ...
            composerError = EgakiumLocalization.format(
    ...
    func agentThreadPage(
        ...
        guard let projectionPump else {
            return CodeProjection().coworkAgentThreadPage(
                ...
                showsExecutionTrace:
                    EgakiumExecutionTracePresentation.isEnabled,
        ...
        let projected = await projectionPump.coworkAgentThreadPage(
            agentID: agentID,
            requestedUpperBound: requestedUpperBound,
            showsExecutionTrace: EgakiumExecutionTracePresentation.isEnabled,
            additionalItems: additionalItems)
        let interruptedFailure = SubmissionFailure(
            code: "interrupted",
            message: EgakiumLocalization.string(
    ...
    private func rebuildOutboxThreadItems(
        ...
        for entry in outboxEntries.values.sorted(by: {
            ...
            let item = CodeItem(
                id: id.rawValue,
                kind: .user,
                title: EgakiumLocalization.string("You"),
    ...
    private static func goalPresentation(
        ...
        if let audit {
            ...
            if let blocker = audit.blocker, !blocker.isEmpty {
                parts.append(EgakiumLocalization.format("Blocker: %@", blocker))
            }
    ...
    private func taskLine(_ task: CoworkTaskView) -> CoworkTaskLine {
        let assignee = task.assignee.map { "@\($0.rawValue)" }
            ?? EgakiumLocalization.string("Unassigned")
    ...
    private func retainWorkspaceAccess(forPath path: String) -> URL? {
        ...
        do {
            ...
            sessionStorageWarning = EgakiumLocalization.format(
    ...
    private func ensureAutomaticPermissionReview(existingProjection projection: CoworkProjection) async {
        guard let orchestrator else {
            setPermissionReviewerStatus(.failed(
                EgakiumLocalization.string("Cowork session is not ready.")))
            return
        }
        setPermissionReviewerStatus(.enabling)
    ...
    private func resumeRuntimeIfReady() async {
        ...
        guard recoverySafe else {
            let message = EgakiumLocalization.string(
    ...
    private func bootstrapMainAgentIfNeeded(existingProjection projection: CoworkProjection,
        ...
        guard let url = retainWorkspaceAccess(forPath: workspace.path) else {
            ...
            composerError = EgakiumLocalization.format(
                ...
                mainID.rawValue)
            setPermissionReviewerStatus(.failed(
                composerError ?? EgakiumLocalization.string("Workspace access unavailable.")))
            return
        }
        if projection.agentRoster[mainID] != nil {
            ensureCanvasDocument(in: url)
            return
        }
        ...
        if attached {
            // The canonical seven-event Cowork bootstrap has completed at this
            // point. Canvas creation is an independent, idempotent host step;
            // it does not append an eighth bootstrap identity/lease event and
            // does not issue a provider request.
            ensureCanvasDocument(in: url)
    ...
    private func ensureCanvasDocument(in workspaceRoot: URL) {
        do {
            canvasDocument = try SessionCanvasStore.ensureCanvas(
                in: workspaceRoot,
                sessionID: sessionID)
            canvasInitializationError = nil
        } catch {
            let message = EgakiumLocalization.format(
                "The Session Canvas could not be initialized: %@",
                error.localizedDescription)
            canvasInitializationError = message
            if sessionStorageWarning?.contains(message) != true {
                sessionStorageWarning = [sessionStorageWarning, message]
                    .compactMap { $0 }
                    .joined(separator: " ")
            }
        }
    ...
    func reauthorizePrimaryWorkspace() {
        guard acceptsNewOperations,
              ...
              let selected = WorkspaceAccess.choose(
                prompt: EgakiumLocalization.string("Reauthorize Primary Workspace")) else {
            return
        }
    ...
    func updateProjectSettings(_ settings: CoworkProjectSettings) async -> Bool {
        guard let operationID = beginDirectOperation() else {
            composerError = EgakiumLocalization.string(
    ...
    func removeAgent(name rawName: String) {
        ...
        let operation = Task { @MainActor [weak self] in
            ...
            await self.synchronizePermissionReviewerHealth(using: orchestrator)
            guard detached else {
                self.composerError = EgakiumLocalization.format(
    ...
    func removeWorkspace(path: String) {
        ...
        let operation = Task { @MainActor [weak self] in
            ...
            guard let configuredWorkspace = self.configuredWorkspace(matching: path) else {
                self.composerError = EgakiumLocalization.string(
    ...
    func addAgent(name rawName: String, workspace authorization: WorkspaceAccessLease) {
        ...
        guard let binding = projectSettings.defaultInferenceProfileBinding else {
            addAgentStatus = .failed(EgakiumLocalization.string(
        ...
        do {
            ...
            addAgentStatus = .failed(EgakiumLocalization.format(
                "Workspace access could not be saved: %@",
                error.localizedDescription))
            authorization.release()
            return
    ...
    func importDraftAttachments(_ urls: [URL]) {
        ...
        let operation = Task { @MainActor [weak self] in
            ...
            for url in urls {
                do {
                    let file = try EgakiumComposerAttachmentFileReader.read(url)
    ...
    private static func presentationMessage(
        ...
        switch error {
        case .empty:
            return EgakiumLocalization.string("Enter a message.")
        case .missingGoal:
            return EgakiumLocalization.string("Enter a goal after /goal.")
        }
    }
    ...
    private static func presentationMessage(
        ...
        case .emptyMessage:
            return EgakiumLocalization.string("Enter a message before sending.")
        case .emptyMention:
            return EgakiumLocalization.string("Type an agent name after @.")
    ...
    private func attachFailureStatus(agentName: String, events: [Envelope]) -> CoworkAddAgentStatus {
        ...
        return .failed(EgakiumLocalization.format(
            "Could not attach @%@.",
            agentName))
    }
    ...
    private static func makeProjectInfo(sessionID: SessionID,
        ...
        return CoworkProjectInfo(
            ...
            tokenBudget: settings.tokenBudget.map {
                EgakiumLocalization.format("%@ tok", formatNumber($0))
            },
    ...
    private static func defaultModelDescription(_ settings: CoworkProjectSettings) -> String {
        ...
        guard let model, !model.isEmpty else {
            return EgakiumLocalization.string("current model")
        }
    ...
    private static func permissionDescription(_ rawValue: String) -> String {
        ...
        case .some(.manual): return EgakiumLocalization.string("manual")
        case .some(.reviewed): return EgakiumLocalization.string("reviewed")
        case .some(.autopilot): return EgakiumLocalization.string("autopilot")
        case .some(.readOnly): return EgakiumLocalization.string("read only")
        case .some(.locked): return EgakiumLocalization.string("locked")
...
enum CoworkAddAgentStatus: Equatable {
    ...
    var message: String? {
        ...
        case .idle:
            return nil
        case .validating:
            return EgakiumLocalization.string("Validating agent…")
        case .attaching(let name):
            return EgakiumLocalization.format("Attaching @%@…", name)
        case .attached(let name):
            return EgakiumLocalization.format("@%@ attached.", name)
        case .denied(let reason):
            return EgakiumLocalization.format("Permission denied: %@", reason)
        case .failed(let message):
            return message
        }
    }
