...
struct EgakiumMacRootView: View {
    // Chat and Code remain compiled product surfaces. Their navigation entries
    // are intentionally hidden while Egakium presents the Cowork-first UI.
    private static let visibleNavigationItems: Set<EgakiumNavItem> = [.cowork]

    @EnvironmentObject var env: AppEnvironment
    @ObservedObject private var runtimeManager: AppSessionRuntimeManager
    @Environment(\.colorScheme) private var scheme
    @State private var selection: EgakiumNavItem = .cowork
    ...
    private var items: [EgakiumNavItem] {
        EgakiumNavItem.allCases.filter { item in
    ...
    var body: some View {
        NavigationSplitView {
            ...
            ZStack {
                EgakiumSystemCanvas().ignoresSafeArea()
                detail
            }
        }
        .navigationTitle("")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Button { isSettings = true } label: {
                EgakiumSidebarSettingsRow(selected: isSettings)
            }
            .buttonStyle(.plain)
            .padding(.horizontal, 12)
            .padding(.top, 12)
...
private struct EgakiumSidebarModeRow: View {
    let title: String
    ...
    private var content: some View {
        HStack(spacing: 10) {
            Image(systemName: systemImage)
                ...
                .frame(width: 22)
            Text(title)
                .font(EgakiumType.body(14, selected ? .semibold : .medium))
                .foregroundStyle(selected
                    ? EgakiumTheme.deepText(scheme)
                    : EgakiumTheme.softText(scheme))
            Spacer(minLength: 0)
        }
        .padding(.horizontal, 12)
...
private struct EgakiumSidebarSettingsRow: View {
    let selected: Bool
    @Environment(\.colorScheme) private var scheme
    ...
    var body: some View {
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .stroke(selected ? EgakiumTheme.selectedStroke(scheme) : EgakiumTheme.separator(scheme),
                        lineWidth: 1)
