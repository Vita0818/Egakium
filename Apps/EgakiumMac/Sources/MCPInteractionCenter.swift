...
private struct MCPOAuthApprovalSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text(
                "Egakium will open the authorization page only after you continue. The callback is bound to a one-shot local loopback address; this does not connect the MCP server.")
                .font(.callout)
                .foregroundStyle(.secondary)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            MCPInteractionHeader(
                ...
                queuedCount: center.queuedCount)
            Text(
                "The server is asking Egakium to run a provider-neutral model request. Approval here does not grant tools or access to the current Agent conversation. You will review the model result separately.")
                .font(.callout)
                .foregroundStyle(.secondary)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            case .url(let value):
                ...
                Text(
                    "Egakium does not prefetch this URL and never receives credentials or other data entered in the external browser.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            HStack {
