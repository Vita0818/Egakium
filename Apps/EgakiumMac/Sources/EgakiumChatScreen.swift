                    ...
                    VStack(spacing: 14) {
                        ...
                        if showsVisibleThinkingIndicator {
                            thinkingRow(layout: layout)
                                .id("egakium-chat-thinking-\(chatThinkingPhaseID)")
                        }
                        Color.clear
                            .frame(height: 1)
                            .padding(.bottom, 16)
...
struct EgakiumMessageBubble: View {
        ...
        VStack(alignment: .leading, spacing: 5) {
            if EgakiumMessageHeaderPolicy.showsIdentity(for: message.role)
                ...
                HStack(spacing: 6) {
                    if let roleLabel {
                        Text(roleLabel)
                            .font(EgakiumType.caption(10, .semibold))
                            .tracking(0.6)
                            .foregroundStyle(EgakiumTheme.tertiaryText(scheme))
                    }
    ...
    private func goalTag(_ tag: String) -> some View {
        Text(tag.uppercased())
            ...
            .padding(.vertical, 2)
            .overlay {
                Capsule().stroke(EgakiumTheme.separator(scheme), lineWidth: 1)
            }
    }
}
...
struct EgakiumSettingsPanel: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                if let settingsError {
                    Text(settingsError)
                        .font(EgakiumType.caption(12, .regular))
                        .foregroundStyle(.red)
                        .fixedSize(horizontal: false, vertical: true)
    ...
    private var openJSONButton: some View {
        Button(action: openJSONConfig) {
            Label("Open Egakium Config", systemImage: "curlybraces")
                .font(EgakiumType.body(14, .semibold))
                .foregroundStyle(.primary)
        }
        .egakiumGlassButton()
        ...
        DisclosureGroup(isExpanded: $isModelManagementExpanded) {
            ...
            HStack {
                ...
                Text("\(catalog.providers[providerIndex].models.count)")
                    .font(EgakiumType.caption(12, .medium))
                    .foregroundStyle(EgakiumTheme.softText(scheme))
            }
        }
    ...
    private func exportDiagnostics() {
        ...
        defer {
            EgakiumMacProcessDiagnostics.shared.setKnownModalPresented(false)
        }
