...
/// Persistent config at `~/.config/egakium/config.json` (all values are strings).
/// `egakium settings` writes it; env vars override it; both override defaults.
enum ConfigFile {
    static var url: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/egakium/config.json")
    }
...
struct CLIConfig {
    ...
    static func load() throws -> CLIConfig {
        ...
        let mode = Mode(rawValue: value("EGAKIUM_MODE", "mode", fallback: "chat")!.lowercased()) ?? .chat
        // Ask the endpoint for token usage (default on). Set EGAKIUM_USAGE=0 if an
        // endpoint rejects the stream_options field.
        let usageStr = value("EGAKIUM_USAGE", "usage", fallback: "1")!.lowercased()
        let includeUsage = !(usageStr == "0" || usageStr == "false" || usageStr == "off")
        // How many tool round-trips one turn may take before giving up. Long
        // agentic tasks need plenty; override with EGAKIUM_MAX_STEPS.
    ...
    static func load(configurationFileURL: URL,
        ...
        let selectedInlineKey = environment["EGAKIUM_API_KEY"]?
        ...
        let rawReasoning = value("EGAKIUM_REASONING", fallback: nil)
        let reasoning = try parsedReasoningEffort(rawReasoning)
        let mode = Mode(rawValue: value("EGAKIUM_MODE", fallback: "chat")!.lowercased()) ?? .chat
        let usageStr = value("EGAKIUM_USAGE", fallback: "1")!.lowercased()
        let includeUsage = !(usageStr == "0" || usageStr == "false" || usageStr == "off")
        let configuredMaxSteps = value(
            "EGAKIUM_MAX_STEPS",
            fallback: nil)
