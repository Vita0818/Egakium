...
struct CLIModernProviderConfig: Sendable {
    ...
    static func existingURL(environment: [String: String]) -> URL? {
        if let override = environment["EGAKIUM_CONFIG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty {
            return URL(fileURLWithPath: expandedPath(override)).standardizedFileURL
        }
        let home = FileManager.default.homeDirectoryForCurrentUser
        let config = home.appendingPathComponent(".config/egakium", isDirectory: true)
        var candidates = [
            config.appendingPathComponent("egakium.json"),
            config.appendingPathComponent("egakium.jsonc"),
        ]
        if let support = FileManager.default.urls(
            for: .applicationSupportDirectory,
            in: .userDomainMask).first {
            let directory = support.appendingPathComponent("Egakium", isDirectory: true)
            candidates.append(directory.appendingPathComponent("egakium.json"))
            candidates.append(directory.appendingPathComponent("egakium.jsonc"))
        }
    ...
    static func load(from url: URL,
        ...
        if permissionReviewerFieldPresent {
            ...
            // configured default, not to EGAKIUM_MODEL or a runtime @main
...
struct CLIExactSecretResolver: SecretResolver {
    ...
    private func readAuthSecret(providerID: String) -> String? {
        ...
        var urls: [URL] = []
        if let override = environment["EGAKIUM_AUTH_FILE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty {
            ...
            let home = FileManager.default.homeDirectoryForCurrentUser
            urls = [
                home.appendingPathComponent(".config/egakium/auth.json"),
