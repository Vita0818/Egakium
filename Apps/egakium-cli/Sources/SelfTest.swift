...
private func tempLog(_ tag: String) throws -> EventLog {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("egakium-\(tag)-\(UUID().uuidString)", isDirectory: true)
...
func runSelfTest() async throws {
    out("\(bold)Egakium self-test\(reset) — offline, no API key, no network.\n")

    // 1) CHAT: a full streamed turn.
    out("\n\(bold)[chat]\(reset)\n› hi")
    let chatLog = try tempLog("chat")
    let chatLoop = ChatLoop(log: chatLog,
                            provider: FakeChat(parts: ["Hello! ", "I am Egakium."]),
    ...
    let okChat = chatMsgs.contains { $0.role == .assistant && $0.text == "Hello! I am Egakium." }
    ...
    let writeArgs = String(decoding: try JSONSerialization.data(
        withJSONObject: ["path": "note.txt", "content": "hello from egakium"]), as: UTF8.self)
    ...
    let okCode = onDisk == "hello from egakium"
    ...
    let profileDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("egakium-profile-selftest-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: profileDirectory, withIntermediateDirectories: true)
    let catalogURL = profileDirectory.appendingPathComponent("catalog.json")
    let configURL = profileDirectory.appendingPathComponent("egakium.json")
    ...
    uniqueUnqualifiedEnvironment["EGAKIUM_MODEL"] = "model-beta"
    uniqueUnqualifiedEnvironment["EGAKIUM_REASONING"] = "high"
    ...
    slashModelEnvironment.removeValue(forKey: "EGAKIUM_REASONING")
    slashModelEnvironment["EGAKIUM_MODEL"] = "anthropic/claude-sonnet"
    ...
    slashTopLevelEnvironment.removeValue(forKey: "EGAKIUM_REASONING")
    ...
    unavailableReasoningEnvironment["EGAKIUM_MODEL"] = "route-a/model-alpha"
    unavailableReasoningEnvironment["EGAKIUM_REASONING"] = "minimal"
    ...
    invalidReasoningEnvironment["EGAKIUM_REASONING"] = "not-a-reasoning-effort"
    ...
    secondEnvironment["EGAKIUM_REASONING"] = "high"
