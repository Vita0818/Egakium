...
/// `egakium settings` — interactive editor for the persistent config file
/// (~/.config/egakium/config.json). Endpoint and API key are the must-haves;
...
func runSettings() throws {
    ...
    while true {
        ...
        \(bold)Egakium settings\(reset)  \(dim)\(ConfigFile.url.path)\(reset)
        ...
        case "s":
            ...
            out("\n\(green)Saved.\(reset) Now just run `egakium` (or `egakium chat`).\n")
            return
        case "q", "":
            out("\nNo changes saved.\n")
            return
