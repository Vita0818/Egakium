// Validation-only macOS process watchdog for the Egakium renderer fixture.
//
// Build (does not launch any app):
//   xcrun swiftc -O -module-cache-path /private/tmp/egakium-renderer-watchdog-module-cache \
//     scripts/RendererValidationWatchdog.swift \
//     -o /private/tmp/egakium-renderer-validation-watchdog
//
// The `run` command intentionally accepts an Egakium validation app plus a
...
private let watchdogLockPath = "/private/tmp/com.Vita0818.Egakium.renderer-validation-watchdog.lock"
private let expectedBundleIdentifier = "com.Vita0818.EgakiumMac"
private let expectedFixtureSHA256 = "fb548849d0b708d31e8c6d055805f29f5c09ee4c8306bf9adc537a48e95707f1"
private let validationExecutableMarker = Data("Egakium renderer fixture".utf8)
...
private enum RendererMode: String, Encodable {
    ...
    var launchArgument: String {
        switch self {
        case .microsoft: "-EgakiumMicrosoftMarkdownMessages"
        case .plainSafe: "-EgakiumPlainSafeMessages"
        }
    }
}
...
private enum MathMode: String, Encodable {
    ...
    var launchArgument: String? {
        switch self {
        case .disabled: "-EgakiumDisableSingleDollarMath"
...
private enum Appearance: String, Encodable {
    ...
    var launchArgument: String {
        switch self {
        case .light: "-EgakiumAppearanceLight"
        case .dark: "-EgakiumAppearanceDark"
        }
    }
}
...
private struct SampleRecord: Encodable {
    let schema = "egakium.renderer-watchdog.sample.v1"
    let sequence: Int
...
private struct EventRecord: Encodable {
    let schema = "egakium.renderer-watchdog.event.v1"
    let sequence: Int
    let elapsedSeconds: Double
    let kind: String
    let detail: String
...
private struct Manifest: Encodable {
    let schema = "egakium.renderer-watchdog.manifest.v4"
...
private struct ResultRecord: Encodable {
    let schema = "egakium.renderer-watchdog.result.v1"
    var outcome: Outcome
    var detail: String
    let elapsedSeconds: Double
    let childPID: Int32
...
private struct RuntimeLogAuditRecord: Encodable {
    let schema = "egakium.renderer-watchdog.runtime-log-audit.v1"
...
private struct FixtureMachineResult: Codable {
    static let currentSchema =
        "egakium.renderer-fixture.result.v1"
...
private func isFamilyMember(_ identity: ProcessIdentity, familyNames: Set<String>) -> Bool {
    ...
    return candidates.contains { candidate in
        familyNames.contains(candidate)
            || candidate.hasPrefix("EgakiumRendererValidation")
            || candidate.hasPrefix("EgakiumRenderer")
    }
}
...
private func validationEnvironment() -> [String: String] {
    ...
    environment["EGAKIUM_RENDERER_WATCHDOG"] = "1"
    return environment
}
...
private func validateApp(_ configuration: RunConfiguration) throws -> (String, Manifest) {
    ...
    guard URL(fileURLWithPath: executablePath).lastPathComponent == "EgakiumMac" else {
    ...
    guard executableData.range(of: validationExecutableMarker) != nil else {
        throw WatchdogFailure.preflight(
            "validation fixture marker is absent; refusing to launch a normal Egakium build")
    }
...
private func runValidation(_ arguments: [String]) throws -> Int32 {
    ...
    var launchArguments = [
        ...
        "-EgakiumRendererIncidentFixture",
        canonicalPath(configuration.fixturePath),
        "-EgakiumRendererFixtureAutoExitSeconds",
        String(format: "%.3f", limits.observationSeconds),
        "-EgakiumRendererFixtureResultPath",
    ...
    var result = try runMonitor(
        specification: MonitorSpecification(
            ...
            familyNames: ["EgakiumMac", "EgakiumRendererValidation"],
            excludedPIDs: [getpid()],
            limits: limits,
...
private struct SelfTestSummary: Encodable {
    let schema = "egakium.renderer-watchdog.self-test.v1"
...
private func validateClosedSetSelfTestContracts() throws {
    ...
    guard isolatedMathStages.allSatisfy({ $0.accepts(.isolation) }),
          ...
          MathMode.disabled.launchArgument == "-EgakiumDisableSingleDollarMath",
...
private func printUsage() {
    ...
    The run command only accepts a hash-pinned Egakium validation build.
