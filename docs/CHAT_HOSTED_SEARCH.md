...
托管搜索时，Egakium 向模型提供厂商对应的搜索能力，并由模型通过 `tool_choice: auto` 自己决定
当前问题是否需要搜索。该 route 不支持、未声明、无法确认或尚未适配时，Egakium 静默发送普通
Chat 请求：不搜索、不提示，也不切换到其他模型或搜索服务。

## 用户可观察行为

| 当前所选 exact Chat route | Egakium 发给模型的能力 | 最终行为 | 用户界面 |
|---|---|---|---|
...
“模型自己选择”只表示当前所选模型决定是否调用已经提供给它的托管搜索能力。Egakium 不按关键词
...
在运行时没有效果。Egakium 新生成的配置不应再主动加入该字段。是否在未来 schema 版本中彻底
...
上表描述的是 Chat 的 provider-hosted 能力，不是 Chat 中的 Egakium Tool。现有
`Capability.toolSearch` 表示 MCP deferred `tool_search` 合同，不得复用为网络搜索能力。
Code/Cowork 另有一个显式 `hosted_web_search` Egakium Tool 包装同一类 provider-hosted wire；它与
...
- Egakium 只拥有运行时结构字段。支持搜索时只注入该 dialect 必需的 tool/endpoint 字段；不支持
...
- provider annotation 若同时暴露 `content`、`start_index`、`end_index`，Egakium 必须和
...
- Chat 不注册或调用 Egakium 搜索工具，也不调用 `hosted_web_search`、`web_fetch`、
...
因此，macOS/iOS Chat 仍是无 Egakium Tools、无 PermissionEngine 的产品面；托管搜索只是在当前
...
- macOS 新生成的 Egakium 配置不再写出 `web_search_model`；读取与内存兼容字段仍保留，更新既有
