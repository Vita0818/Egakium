...
当用户指定、工程已采用或经 `OPEN_SOURCE_REUSE.md` 审查可采用的外部依赖提供相同 capability 时，
架构只能直接集成该依赖的官方 API/runtime。仓内不得建立另一套同等 adapter、shim、compatibility
layer、parallel/shadow backend、preview backend 或 fallback，也不得以原型、测试或迁移阶段为由先
实现替代技术。依赖接入受阻时，架构状态必须是 explicit unavailable/blocked，而不是自动改走本地
实现。允许的 host 代码仅限最薄类型、生命周期、bundle、权限与审计接线；完整规则和例外授权边界见
Vitemis canonical `/Users/vita/Vitemis/docs/DEPENDENCY_POLICY.md` 与项目准入
[`OPEN_SOURCE_REUSE.md`](OPEN_SOURCE_REUSE.md)。

对 Canvas，官方 CEF 是唯一接受的网页 renderer。当前结构已经是 `CoworkCanvasHost →
EgakiumCEFView → CEF AppKit child browser + Chromium Embedded Framework/Helpers`；WKWebView、WebKit
注入和 renderer fallback 已删除。CEF 未链接/加载时 Canvas fail closed。`config/cef.cmake` 固定官方
ARM64 distribution 的 version/archive/hash，build 编译 CEF 官方 wrapper/external pump/sandbox Helpers
并打包 Framework/Resources/notices；本地 Objective-C++ 只负责 lifecycle、child view、request context、
Session-rooted scheme 与关闭接线。

## macOS 发行架构边界

macOS 唯一发行 App 是 Developer ID 签名、公证和直接分发的 `EgakiumMac`。
Mac App Store / App Sandbox 不再是产品架构分支；不得为了它裁剪本地
terminal、Git、global Skills、stdio MCP、浏览器 helper 或其他 Code/Cowork
能力。源码中的 `EgakiumMacAppStore`、`.macAppStore` 与对应 entitlements
...
content-world interaction 已从源码删除；不得重新引入。Canvas ownership 归 Session；组合视图的
...
- 当前入口固定为 primary workspace 下的 `.egakium/canvas/<SessionID>/index.html`。初始化使用同目录
  完整临时文件 + no-replace link 发布，不覆盖既有 `@main` 编辑；旧 Session 走 additive 初始化；
- 宿主只负责首次模板。`Orchestrator` 只给 exact `@main` root 的 `ContextBuilder` prompt 注入精确
  Session 路径；`@main` 通过既有 workspace file/patch、lease、permission 与 durable execution 链
  直接编辑整份页面。ordinary worker 不收到该路径，且不得被安排并发编辑共享入口；
- 当前 renderer：`CoworkCanvasHost` 只创建 `EgakiumCEFView`；App 在 will-finish-launching 使用
...
- provisional DOM v1 把 `#canvas` 的 direct `.egakium-element` 建模为卡片：required safe/unique
  ...
  runtime 曾添加 selection/drag/resize/keyboard，但已删除；当前没有宿主自研交互或布局 override；
- renderer-independent 目标仍要求 source layout 与用户手势分层：`index.html` attributes 是页面
  authority，未来 durable view/layout 另走 versioned projection。CEF profile/Web storage 不得冒充
  EventLog/layout truth；
- `SessionCanvasElementTemplate` v1 是唯一 host-authored generic child-document seed。它固定 responsive
  `#element`/`#content` boundaries、system colors、`connect-src 'none'`/`frame-src 'none'`，且没有
  AgentID/ElementID/SessionID/path/card-layout。`SessionCanvasStore.defaultElementHTML()` 只返回 exact
  bytes，不创建文件或取得 workspace authority；
- ordinary `spawn_agent` 在既有 schema/lease/permission/durable tool ticket 通过后，由 Orchestrator 于
  admission lock 内生成 fresh `CanvasElementID`，在 child canonical workspace 下 no-overwrite 发布
  `.egakium/canvas/<SessionID>/elements/<ElementID>/index.html`，再以原子 EventLog batch 同时登记
  identity/workspace/capability 与相同的 `SessionCanvasElementDescriptor`。append 报错后先调用
  complete-known projection replay：若 WAL/replay 证明 exact contiguous batch 已提交，则保留文件并
  完成 live admission；只有完整历史证明 descriptor fact 不存在时，Store 才在 exact template bytes/
  目录 shape 未变时补偿删除。unknown/incomplete/conflict 时保留并 fail closed；
- 成功 ToolResult 与 `list_agents` 返回 ElementID/path/template version；replay projection 恢复 active
  descriptor，worker dispatch 前必须重新验证 canonical descriptor、no-symlink directory 与 regular
  file，再由 `ContextBuilder` 注入真实文件路径。manual/legacy attach worker 仍可获得 identity-free
  prompt copy fallback；ordinary worker 从不获得 shared `index.html` 路径。`@main` 不预造 ID/path，
  必须等待 ToolResult 后才创建 card 或 later delegate。read-only child 有文件但不能编辑，agent recycle
  只解除 active prompt association、不删除 element file；
- 正式 durable 元素模型仍以内容与布局分离为方向：sub-agent 编辑一个指定元素的独立
  HTML/CSS/JS/assets，exact `@main` 协调 x/y/size/z-order/group/relation；当前 provisional ID/frame
...
- 当前没有专用 Canvas mutation tool；直编复用现有 ToolRegistry、CapabilityLease、WorkspaceLease、
  PermissionEngine 和 durable execution。未来 Canvas/element mutation tool 仍须经过同一链路与
  EventLog；CEF/JavaScript 不得获得隐藏原生执行后门；
- CEF dependency closure 固定为 macOS ARM64
  `151.3.17+gf059e67+chromium-151.0.7922.138`。XcodeGen prebuild 校验 archive/hash 并编译官方
  `libcef_dll_wrapper`、external pump 和五 Helpers；postbuild 按 CEF versioned framework layout 嵌入
  Framework/Resources/Helpers/LICENSE/CREDITS。缺失或 arch/hash 不符即构建失败，不存在替代 renderer；
- 当前目标仅适用于 macOS；iOS 继续是 Chat-only 真子集。

Canvas 工作副本路径与 provisional direct-card source shape 已固定；CEF target/Helpers 和
`egakium://canvas` origin 已实现。Canvas durable event、Element revision、layout projection、
ArtifactStore 关系、用户交互 persistence 和 native bridge schema 尚未冻结。旧 DOM local events/API
已删除，不是恢复自研适配层的依据。
完整已确认合同、当前/待实现边界、实现顺序与验收方向见
[`EGAKIUM_CANVAS_COWORK.md`](EGAKIUM_CANVAS_COWORK.md)。
...
发布布局使用 `.egakium-rag-store.json`、`.egakium-rag-snapshots/` 和 `.egakium-rag-host/`。三者是
...
`EgakiumKnowledge` 的薄 Profile/build adapter、同 target 内不调用模型的
...
知识正文仍是普通 OKF Markdown/YAML；`.egakium-rag/profile.json`、`chunks.jsonl`、
dense/lexical index、checksums 和 store pointer 都是 Egakium 派生合同。Profile 冻结完整
...
`Float32` exact KNN JSON。lexical route 使用 Egakium 多语言/代码 tokenizer 与 BM25，融合使用
...
`EgakiumKnowledge`。截至 08-09，CLI call site 仍不构造 adapter，也没有 mount command；这些
...
`__egakium_authorization_context`。宿主仅在 deterministic gate 实际进入 Cowork automatic ask 时消费并验证这条
...
acting model request (once)
  ...
  + per-call __egakium_authorization_context
...
pre-turn / mid-turn 压缩，同时保留 Egakium 的 EventLog-first 持久化：
...
`EgakiumSkills` 把 Skill 建模为有界上下文能力，不是新的执行权限。生产链路按
Codex 的 catalog → activation → progressive resources 三层组织，但由 Egakium
独立实现并继续服从现有安全边界：
...
`EgakiumSkills`。Skill tool 不增加 `ToolCapability`，不授予 filesystem、
...
App/CLI/EventLog consumer，renderer 也保持 Egakium 的公平截断实现，并非 Codex
逐字节同构。
...
不携带 endpoint、command、header、credential 或 query。Egakium 当前不实现
...
Egakium 对目录 symlink 采取比 Codex 更严格的 fail-closed 策略；不能把当前
实现描述为全部 Codex feature parity。

`EgakiumSkills` 另通过 SwiftPM resource bundle 发布 host-authored
...
Egakium 只实现连接外部 MCP Server 的客户端角色；没有 Egakium MCP
...
不构成 MCP Server。开发期 `EgakiumMCPConformanceClient` 也只是 official
...
|---|---|---|---|---|
| `EgakiumMac` | `EgakiumMCP` | `EgakiumCurlTransport` | `EgakiumMCPStdio` + `EgakiumMCPStdioGuard` | Code/Cowork 设置、会话内容与审批 |
| `egakium` CLI | `EgakiumMCP` | 原生 HTTP/OAuth | `EgakiumMCPStdio` + guard | 管理命令、Code/Cowork exact-session owner |
| `EgakiumiOS` | 不链接 | 不链接 | 不链接 | 无 MCP runtime、transport 或产品 UI |

`EgakiumMacAppStore` 的 HTTP-only linkage 仍可能存在于当前源码图，但它是
legacy/non-shipping target，不属于上表的产品架构或验收矩阵。

`EgakiumProtocol` 中 SDK-independent、可旧日志解码的 MCP payload 仍可由共享
...
| Window presentation | 每个 `EgakiumMacRootView` | mode、当前 session、inspector 显隐 | 只影响本窗口 |
| Session presentation | `EgakiumThreadPresentationScope` + 该窗口 thread 的 `@StateObject` | ScrollView、bottom-follow、scroll generation、临时 geometry | scope 改变即销毁/重建 |

`EgakiumThreadPresentationScope` 不写入 EventLog，也不是 runtime key。相同 session 在两个窗口中可共享 runtime，但每个窗口构造独立的 scroll coordinator；任一窗口的 cancel/deactivate 不会取消另一窗口的展示任务。Code / Cowork 详情和 thread subtree 以 scope 建立 SwiftUI identity，bottom sentinel 使用 `EgakiumThreadBottomAnchorID(scope:)`，因此旧 session 的 anchor 或 task 无法命中新 session。
...
Egakium 是 Apple-first、Swift-native 优先的本地 AI 工作区，三个产品面：Chat（普通多模态对话）/ Code（单 agent 本地工作区）/ Cowork（多 agent 本地工作区协作）。项目允许按 `docs/OPEN_SOURCE_REUSE.md` 选择性复用兼容许可证的公开源码，但 Egakium 继续拥有自己的产品身份、权限/lease/EventLog 控制面和 Apple 平台边界。Apple 应用与 SwiftPM 声明的最低系统为 macOS 26 / iOS 26。默认 `EgakiumMac` 是 DeveloperID/non-sandbox 本地 workbench，提供全量 macOS 产品面；iOS 仍是 chat 子集。
...
```text
            ...
            │                      │                      │
   ┌────────▼─────────┐  ┌────────▼─────────┐  ┌─────────▼──────────┐
   │  EgakiumMac       │  │  EgakiumiOS       │  │  egakium-cli        │
...
model tool_call exec_command / write_stdin
  ...
  -> ProcessTerminalSessionManager
     ...
     -> tty=true: EgakiumPTYLauncher forkpty -> controlling terminal
...
EventLog raw message text + Envelope.ts (single source of truth)
  ...
  -> role policy + EgakiumMessageRendererMode
     ...
     -> assistant / agent + microsoft: EgakiumMessageContentView(.richText)
...
Code/Cowork 的 presentation gate 只控制哪些 `CodeItem` 进入 SwiftUI 会话树、自动滚动签名和 Code inspector。默认隐藏 `.toolCall` / `.toolResult` / `.patch` / `.note`，避免把大型 JSON 风格工具过程当成长文本气泡排版；`.agentToAgent` 保持可见，因此媒介化的通用 agent 消息以及 `information_requested` / `information_replied` 会进入默认会话。`CodeProjection` 另以 agent single-flight 的 `task_started` 边界关联该 `{TaskID, attempt}` 最后一个完整 `message_completed`：只有同一 TaskID、同一 attempt、同一 agent 且正文与 `task_completed.result` 完全相同，才给后者附加非 wire 的 `.executionTrace` 展示来源，默认隐藏这个 scheduler lifecycle 镜像；没有匹配 message、attempt 不一致、正文不同、retry 未生成对应 message 或跨任务同文时，task result 继续作为 conversation fallback 显示。迟到旧 attempt 的 terminal 只清理自己的配对，不会清掉当前新 attempt。user、真实 agent message、媒介化 agent-to-agent communication 与 `.error` 仍显示，权限卡片/提交状态等专用结构化 UI 不受影响。完整 `task_completed` 事件、tool observation、AgentInvocation candidate result、agent 上下文、恢复与审计仍由 EventLog / projection 持有。后台启动参数 `-EgakiumShowExecutionTrace` 或环境变量 `EGAKIUM_SHOW_EXECUTION_TRACE=1` 会恢复此前完整视图；该开关默认关闭、没有 UI 或 UserDefaults、进程启动时解析。

每个可见窗口独立持有 `EgakiumThreadScrollCoordinator`。geometry callback
...
界面文案本地化同样停在 presentation boundary：两个 Apple App 的主 bundle 共用 English-source `Localizable.xcstrings`，显式提供 `en` / `zh-Hans`；静态 SwiftUI 文案由系统解析，先构造成普通 `String` 的动态标签通过 `EgakiumLocalization` 从 `Bundle.main` 查表并以原 English key 回退。系统 Preferred Languages / App Language 负责进程启动时的 bundle localization，不在根 View 注入 locale，也不另存应用内语言状态。用户/模型正文、session 与 agent identity、provider/model ID、路径、EventLog、协议 token、工具参数和 model-facing prompt 不进入该层；否则同一 durable 数据会随界面语言被改写，破坏回放和安全边界。

macOS Chat/Code/Cowork 的顶层消息容器采用固定 history window，而不是消息粒度
virtualization：`EgakiumThreadHistoryWindow` 每页最多 16 个 row，页内使用 eager
`VStack`，更多历史由 `EgakiumThreadHistoryPager` 显式 Earlier/Newer/Latest。
...
当前 Egakium dependency 声明已删除旧 MarkdownUI/NetworkImage/swift-cmark0.5/HighlightSwift 和 vendor highlight 资源；完整可构建 Microsoft derivative vendored 于 `Vendor/SwiftStreamingMarkdown`，根 `Package.swift` 使用仓内相对路径。派生包 exact-pins `swift-markdown` 0.8（传递 `swift-cmark` 0.8）以及只在 iOS/macOS 链接的 iosMath 2.5.0 commit `838cddc01fdd67efd530f8bb67959ad2715f9b06`；根与 vendor 两份 `Package.resolved` 已匹配。Microsoft MIT `LICENSE` 与永久 patch ledger 和源码一起由 Egakium 根 Git revision 固定；iosMath MIT、八套 unmodified math font 的 GUST/LPPL 或 OFL 条款、资源清单与用户批准另由 `ThirdPartyNotices/MathRendering.md` 固定。iosMath 自身无传递 package dependency；其 `fonts/` payload 是 8 OTF、8 math-table plist、5 license、4 README 与一个 conversion script，共 26 files / 7,234,424 bytes，完整 built `iosMath_iosMath.bundle` 加生成的根 `Info.plist` 后为 27 files。这套公式字体不改变产品 UI 字体选择。三个 exact-pinned remote dependency 在无缓存解析时仍需要网络，完全离线供应链不在本轮范围。
...
EgakiumMac Debug build、真实 `cowork_tf2lkjbh` entry/scroll/A→B→A 和
55-message Earlier/Latest 通过；旧 lazy soak 只保留为历史证据，不验证当前
容器的长时表现。

### Provider 模型请求扩展

macOS 的 Egakium-owned OpenCode-compatible 配置把 `provider.<id>.models.<model>.options` 视为开放的 model-scoped JSON 请求扩展点。`AppConfig` 不解释厂商字段，而是保真为 `[String: JSONValue]`，按 model ID 挂到 `ProviderEndpoint.modelRequestOptions`；`provider.<id>.npm` 与 model object 内的 `provider.npm` 也分别保留为 provider adapter 与 model override。当前 OpenAI wire runtime 在构造 Chat/Code request body 时先加载这些扩展，再由 exact package adapter 解释已知选项，最后写入运行时字段。因此 routing、sampling、reasoning、response format 或未来厂商自定义嵌套对象无需为每个 provider 增加 Swift 配置字段。此处“开放 JSON”只描述兼容 `ProviderEndpoint` 路径；Cowork durable catalog 有独立的显式 allowlisted schema，不能用这一段推导 unknown option 可被持久化。
...
开放配置与最终 wire 是两个边界：配置文件、immutable profile 和 UI 投影继续保留原始 key/value；最终 Chat Completions builder 按冻结的 `ProviderRequestAdapter` 执行 package-specific lowering，而不是应用跨 provider 的全局 camel/snake/nested 优先级。Egakium-owned custom provider 的选择顺序是显式 model `provider.npm` → provider `npm` → `@ai-sdk/openai-compatible`；只有字段真正缺失（`nil`）才进入下一层，显式空串或空白 package identity 必须原样保留并 fail closed。Egakium 不导入 OpenCode 的 models.dev catalog，因此没有其 built-in model metadata fallback。adapter 作为 connection/profile 语义进入 immutable revision；未知 package、以及当前 Chat 路径不支持的 `@ai-sdk/openai` 都在网络前 fail closed，不能按 endpoint 名称猜测或退回 compatible。
...
- macOS LibreOffice fixed runner 为每次调用建立当前用户 `0700` 的短路径
  `/private/tmp/egakium-lo-<12 hex>`，通过 `-env:OSL_SOCKET_PATH=...` 设置 LibreOffice bootstrap
...
- 普通读取语义由 Docling 高层 converter 拥有；Egakium 只做 strict schema、权限/路径校验、输入
  ...
  Egakium 不自行实现 PDF content stream、OOXML/EPUB parser、OCR/layout/formula engine。
...
- `EgakiumTools` 仍只进入 macOS/CLI workspace stack；iOS Chat target 不链接 Tools、Permission、AgentKernel、Cowork 或文档 runtime。
...
- `git_worktree_create`：只在 workspace `.egakium/git-worktrees/<name>` 下创建受管 linked worktree；默认 detached HEAD，可显式指定新 branch。
- `git_worktree_remove`：只移除 `.egakium/git-worktrees/<name>` 下受管 worktree；这是 destructive 工具，要求 `confirmName` 精确匹配。
...
设计取舍：
- 本轮对照 Codex App/Worktrees/CLI 官方文档与 openai/codex 开源实现后，Egakium 采用相同方向：Git 不是开放 shell，而是受限工具能力，围绕 read-only 状态、diff、patch preflight/apply/revert/stage、受管 worktree 和权限审批暴露。当前已落地 Git slice 没有复制 Codex 源码或文案；未来若选择性复用兼容许可证实现，必须按 `docs/OPEN_SOURCE_REUSE.md` 固定来源并适配现有安全边界。
...
- Git repository root 必须与 agent workspace root 一致。普通 repository 的 git metadata 必须在 workspace 内；`.egakium/git-worktrees/<name>` linked worktree 允许 `.git` file 指向 owning workspace repository 的 `worktrees/` metadata，但 workspace path 仍必须在 owner root 之内。
...
- Remote Git 只接受已配置 remote name，不接受 URL remote/refspec；工具输出会遮蔽 URL 凭据与常见 Git hosting token。`git_pull_ff` 与 `git_switch` 要求 clean working tree；`git_push` 不支持 force / force-with-lease。真实 remote auth 由用户本机 Git credential helper/SSH agent 决定，Egakium 不读取、存储或展示凭据。
...
provider tool_call -> AgentLoop schema validation
  ...
  -> browser_*: ToolContext.browserBackend
       ...
       -> BrowserBackendProcessRunner -> fixed Egakium-generated Node driver
       ...
       -> workspace .egakium/browser profile/state/history
  -> ToolObservation(output, changedFiles)
  -> tool_result event -> CodeProjection / CoworkProjection / CLI
```
...
- `browser_profile_delete`：显式删除一个 workspace-scoped persistent browser profile 的 profile data、state file、downloads 目录和 Egakium history metadata；这是 `.destructive` 工具，必须要求 `confirmProfile` 与目标 profile 匹配，并仍通过权限门；若删除前发现 active/lock runtime marker，只输出概括性提示，不列 marker 文件名或内容。
- `browser_history`：只读取 `.egakium/browser/history.jsonl` 中的非 secret 历史 metadata，支持 profile 和条数过滤，不读取 cookie/localStorage/profile 数据库。
- `browser_navigate` / `browser_snapshot` / `browser_handoff` / `browser_reload` / `browser_back` / `browser_forward`：用持久浏览器 profile 打开 URL、读取当前页、打开有界 headed 浏览器窗口供用户手动登录/接管、刷新当前页，或按 Egakium profile 导航栈前进/后退，返回页面 title、URL、可见文本、链接和可定位交互元素摘要。
...
- `browser_download`：点击预期会触发下载的元素，并把下载文件保存到 `.egakium/browser/downloads/<profile>`，通过 `changedFiles` 暴露下载路径；CDP fallback 使用真实鼠标事件触发，避免只靠 DOM `click()` 的非用户手势限制。
- `browser_downloads`：只列出 `.egakium/browser/downloads/<profile>` 下的文件 metadata（路径、大小、修改时间），不读取文件内容。
...
- shipping macOS 浏览器 lane 不接收 raw shell command，也不复用通用
  ...
  Egakium Seatbelt，因为继承的外层 sandbox 会让 Chromium helper 触发
...
- Code 与 Cowork 不维护两套执行内核：二者使用同一个 Swift-native headless `AgentRuntime` 工厂，统一 registry、permission、completion、durable tool ticket 和单次请求参数。`RuntimeEnvironmentManifest` 在每次请求的首个 system message 中稳定声明 Egakium/Code 或 Cowork、外部动作只能通过 API tools、严格 JSON Schema 和 ToolResult 完成语义；动态 agent/workspace/task/lease/event 仍只进入有界 user-role untrusted context。
...
| GUI provider catalog | GUI provider/model/variant 设置与 iOS 显式文件导入 | UserDefaults `egakium.providerCatalog.v1` + secret ref；当前聊天选择 `egakium.providerSelection.v1`；macOS 可由 `EGAKIUM_CONFIG` 显式指定文件、`~/.config/egakium/egakium.json` / `egakium.jsonc`、app support `egakium.json` / `egakium.jsonc` JSON/JSONC 覆盖；iOS 通过系统 Files picker 显式导入后写 app-owned `Egakium/imported-chat-configuration.json` schema-v1 protected snapshot；不自动发现 `opencode.json` 或 OpenCode app 配置；旧 `config.json` / direct `providers` 兜底兼容读取 | provider 持久化 `baseURL` / `chatEndpoint` / secret ref；model 持久化 id / 展示名；macOS variant 只持久化 identity，参数仍来自配置文件；iOS 导入保留 base model raw options/adapter/capabilities，但当前不导入 variants并明确警告；聊天页切换只改当前选择，不改写外部 JSON；明文 API key 不得进 UserDefaults 或 imported snapshot；旧 `egakium.baseURL`/`egakium.model` 仅迁移/兼容 |

## 同步 / 通信机制

- **进程内**：当前 GUI/CLI kernel 仍在单进程内运行；
  `Orchestrator`/`EventLog`/`MessageBus` 均为 `actor`。这是当前事实，不是仅限 v0.1 的规划。
- **JSON-RPC 2.0 词汇**已定义（`JSONRPC.swift`：Command→request、Envelope→event notification），但**尚未挂传输**。未来 `egakium agent --stdio` / `egakium daemon` 是规划中管道。`UNKNOWN` — 当前无 out-of-process 传输实现。
...
- **Code/Cowork 错误 presentation（2026-08-13 当前例外）**：上述通用 structured-content
  描述中的 `error` 与 Code `recent failure` 不再进入 thread/独立 section。`EgakiumMacApp` 将
  Code voice/composer 及 Cowork voice/composer/inference/projection/session-storage 的全部非空错误
  作为数组传入 SharedUI；`EgakiumThreadErrorPresentation` 在 raw bounded page 上继续收集 `.error`、
...
- **Chat 自动命名**：该能力是 Chat 宿主 metadata 流，不是 Tool，也不进入 `ChatLoop`、`AgentLoop`、
  ...
  fail closed。标题 request 只含 Egakium 自有严格 System 指令，以及 user/assistant 正文字段合计最多
...
- **GUI provider catalog**：macOS `AppConfig` 与 iOS `IOSConfig` 使用 UserDefaults 主键 `egakium.providerCatalog.v1` 保存两层 mutable 配置。第一层 provider 存 `id` / 展示名 / `baseURL` / `chatEndpoint` / secret ref 元数据；第二层 model 存模型 id / 展示名。macOS Chat/Code 模型菜单把配置中的 variants 作为同一 model 的独立选择项；切换后只把 provider/model/variant identity 写入 `egakium.providerSelection.v1` 并重建 `ProviderRegistry`，Chat/Code 下一条请求使用新选择的基础 options + variant 覆盖。Cowork composer 复用同一份配置所编译的 secret-free `AppInferenceProfileOption` 列表，以 provider 分组显示“下一次 `@main`”的 exact 暂存项；三个产品面的弹出菜单均保留 provider 分组与 variant detail，但关闭态 label 只读取选中项的 model title。新 options 先交给 Orchestrator 更新 host-approved catalog，再发布到菜单，避免可见项与 admission catalog 竞态。工作中仍可选择，选择本身不 rebind，只有随后按下 Send 才把当时值冻结进该 submission；FIFO 执行边界把仅 `@main` 的 durable rebind 与对应 root queue admission 原子提交。既有 worker、当前/已冻结 task、控制面 agent 与未来新 agent 默认值均不跟随。Project Settings 的独立 exact-profile picker 仍只更新**未来新 agent 默认值**，逐 agent Rebind 继续用于显式修改其他空闲 ordinary agent。iOS 保持 provider/model 两层 Chat 选择。设置页编辑 Base URL 时自动生成 Chat endpoint；编辑 Chat endpoint 时清洗 `/chat/completions` 后缀回填 Base URL。旧 `egakium.baseURL` / `egakium.model` 仍作为迁移来源与兼容镜像。
- **iOS imported Chat config**：iOS 不扫描 `EGAKIUM_CONFIG`、macOS home 或 app-support 候选路径；用户只能从 Chat 左抽屉底部 Settings 进入设置页，再经系统 Files picker 显式选择 JSON/JSONC。共享 `ChatConfigurationImporter` 解析 OpenCode-compatible `provider` map 与 legacy direct `providers`，执行 1 MiB/数量/字符串/HTTP(S) URL 边界检查，并只投影 iOS Chat 所需 provider/model/endpoint/options/adapter/capabilities。成功后不保留 security-scoped 外部 URL，也不监视或重写原文件，而是在 app Application Support 的 `Egakium/imported-chat-configuration.json` 写 schema-v1、complete-file-protection app-owned snapshot。直接 `options.apiKey` 先迁入同样受保护的 `Egakium/auth.json`，snapshot 与 UserDefaults 只保留 secret reference；环境变量/文件引用保持引用但导入结果会提示在 iOS 重新验证或录入。variants 当前被忽略并明确告警；未知 adapter 保留 exact identity并让既有 adapter gate 在网络前 fail closed，绝不静默改成 compatible。iOS root 持有 thread-only Chat 的唯一 `NavigationStack`，顶部 sidebar/session/new、左抽屉、底部两排 composer 和 Settings sheet 属于同一导航层级；model label 在 composer 第一排有界，不能把第二排 controls 推出屏幕。该路径不会扩大 iOS 的 7-product Chat-only linkage。
...
- **macOS advanced config**：macOS GUI 启动时先检查用户显式设置的 `EGAKIUM_CONFIG` 文件；未设置时按顺序只检查 Egakium-owned 路径：`~/.config/egakium/egakium.json` / `egakium.jsonc`、app support 的 `egakium.json` / `egakium.jsonc`，最后才兜底读取旧 Egakium `config.json` / `config.jsonc`。不会自动发现任何名为 `opencode.json` 的文件，也不读取 `~/.config/opencode/` 下的 OpenCode app 配置；OpenCode-compatible 只表示 JSON shape 兼容。找到可解码的 JSON/JSONC 后覆盖 UserDefaults provider catalog；聊天页当前选择覆盖层仍可覆盖 JSON 顶层 `model`，但不能覆盖已解析的 reviewer role。设置页的 Open Egakium Config 按钮会打开当前生效的 Egakium-owned 文件或 `EGAKIUM_CONFIG` 显式指定文件；若只发现旧 `config.json`，会从当前 catalog 生成新的 `~/.config/egakium/egakium.json` 模板并优先打开。保存设置时，用户本次主动输入的 API key 会写入同一个可编辑 provider JSON 的 `provider.<id>.options.apiKey`；写入用户 config 失败时回退到 app support `egakium.json`。创建的模板来自当前 provider catalog，输出 OpenCode-compatible `$schema` / `enabled_providers` / `model` / `provider.<id>.npm` / `name` / `options.baseURL` / `options.apiKey` / `models`，以及 Egakium 顶层 role 字段（包括 `permission_reviewer_model`）；`options.apiKey` 默认是 `{env:...}` 引用而非明文。支持兼容读取旧 direct `providers` 数组，也兼容读取 Egakium 扩展字段 `chatEndpoint` / `apiKeySource`；推荐新配置使用 `provider.<id>.options.baseURL`、`options.apiKey`（OpenCode-style 明文、`{env:NAME}` 或 `{file:path}` 均可由真实请求懒加载）、`models`，以及顶层 `model` 形如 `<provider>/<model-id>`。支持 OpenCode-compatible 的 `enabled_providers` / `disabled_providers` 过滤；`disabled_providers` 优先。省略 `options.apiKey` 时 provider 请求按 provider id 尝试 Egakium auth JSON 与当前 Egakium-owned OpenCode-compatible config，不再回落 OS Keychain。
- **Permission reviewer model route**：macOS 与 modern CLI 只读取 canonical 顶层
  ...
  `EGAKIUM_MODEL`、UserDefaults/UI selection、Cowork session default 或 live/historical `@main`；显式
...
都使工具在 secret、network、bookmark 或 store 副作用前保持不可用。`egakium:siliconflow-v1` 明确选择
OpenAI-compatible embeddings + SiliconFlow v1 rerank dialect；Cohere v2 reranker 必须使用独立
`egakium:cohere-v2` provider，不从 URL 或模型名猜 dialect。role-only provider 的 `models` 可以为空；
...
- `ConfigSecretResolver` 可指向 `environment` / `file` / `authFile`。macOS `authFile` 默认先查 `~/.config/egakium/auth.json`，再兼容查 `~/.local/share/egakium/auth.json` 与 Egakium-owned OpenCode-compatible config 的 `provider.<id>.options.apiKey`；不默认读取 `~/.local/share/opencode/auth.json` 或 `~/.config/opencode/opencode.json`。也可用 `EGAKIUM_AUTH_FILE` 覆盖。iOS 默认落在 app container Application Support 的 `Egakium/auth.json`。UserDefaults catalog 只存 secret ref 元数据，不存 API key；macOS 设置页输入的 API key 写入当前可编辑 provider JSON，iOS 设置页输入的 API key 写入 auth JSON 并尝试设置 `0600`；真实 provider 请求懒加载 secret，并按 source/service/account 做进程内缓存。
...
- `EgakiumMac.DeveloperID.entitlements`：默认 `EgakiumMac` 使用；非 sandbox、Hardened Runtime，本地 Code/Cowork 的 shell/git/browser 能力仍必须经过 `PlatformProfile.macDeveloperID` 与权限门。composer voice 只增加最小 `com.apple.security.device.audio-input=true`，真实录音仍需 `NSMicrophoneUsageDescription` 与用户 TCC 授权；该 entitlement 不代表启用 App Sandbox。
...
- `EgakiumMac.AppStore.entitlements`：当前源码中的未使用 legacy artifact，不是
...
- 仓内根 `ARCHITECTURE.md`（draft-0, 2026-06-11，中文）描述 Egakium 内核/Cowork 设计。本目录 `docs/ARCHITECTURE.md` 据实际源码重写并与之对照。
- Cowork 设计细节见 `docs/COWORK_AGENT_ARCHITECTURE.md` 等 7 个 COWORK_* 文档；原则提炼见 `docs/COWORK_PRINCIPLES.md`。
- 开源源码、公开 prompt、依赖、bundled runtime 与上游升级的准入规则见 `docs/OPEN_SOURCE_REUSE.md`；实际已经采用的上游必须登记在 `NOTICE.md`。截至 2026-07-12，OpenCode 仍为 research-only，尚未把其源码、公开 prompt、UI 资产或 runtime 加入 Egakium。
