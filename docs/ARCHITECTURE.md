...
- 正式元素模型仍以内容与布局分离为方向：sub-agent 编辑一个指定元素的独立 HTML/CSS/JS/assets，
  exact `@main` 协调 x/y/size/z-order/group/relation；原型阶段也允许 `@main` 直接创建嵌套元素或引用
...
- 当前 `WKWebView` 使用 non-persistent website data store、Session Canvas 目录 read access、
  top-level scheme allowlist、Web content-rule network deny 和模板 CSP。它是调试适配层，不是正式 CEF
  BrowserView/Helper/sandbox/签名证明；
- 当前目标仅适用于 macOS；iOS 继续是 Chat-only 真子集。

Canvas 工作副本路径已经由原型固定；Canvas durable event、Element revision、layout projection、
ArtifactStore 关系、CEF target/Helper、内部 scheme/origin/iframe sandbox 和 bridge schema 尚未冻结。
