...
- exact `@main` 在 Canvas 任务中的 `spatial coordinator` 行为来自其追加 prompt/context，负责全局
  元素规划、位置、大小、层级、空间关系和委派；它不是新增的永久 Agent class；
- ordinary sub-agent 默认在一次 TaskContract 中编辑一个指定 ElementID 的独立网页；这只是任务分工
  与 scoped context，不是永久 Element owner、ElementLease、role inheritance 或新的 agent hierarchy；
- 同一 Agent 可以顺序处理多个元素，同一元素也可以在后续 WorkTask/AgentInvocation 中重新委派；
...
因此，“`@main` 管布局、sub-agent 管元素网页”是稳定的产品 prompt 合同，但不构成硬编码角色模型。
