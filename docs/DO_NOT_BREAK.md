...
- 本节继承且不得弱化 `/Users/vita/Vitemis/docs/DEPENDENCY_POLICY.md`；项目依赖准入细节见
  `docs/OPEN_SOURCE_REUSE.md`。
- 已有用户指定、项目已采用或经 `docs/OPEN_SOURCE_REUSE.md` 审查可采用的同能力外部依赖时，必须
  直接集成该依赖；不得新增自研 adapter、shim、compatibility/polyfill、parallel/shadow backend、
  preview backend 或 capability-substitution fallback。
- “临时原型”“测试方便”“先跑起来”“以后替换”都不构成例外。依赖接入受版本、许可证、构建、
  签名、sandbox、安全或供应链阻塞时，必须停止、fail closed 并报告 blocker，不能静默换技术。
- 本地代码只允许作为官方 API 的最薄 binding，负责类型、生命周期、资源、权限和审计接线；不得
  重写依赖的核心算法、协议栈、渲染器或 runtime，也不得保留双实现回退路径。
- 只有用户对 exact 依赖、exact 范围、时限、删除条件和验收作出的新明文授权，才可建立临时例外。
- 验收必须证明 shipping target 实际链接、加载并执行指定依赖，且依赖缺失时不会启动替代后端；
  mock、接口形状、浏览器 fixture 或自研 fallback 通过不能替代该证据。
- 该规则约束“替代外部依赖能力”的 fallback；legacy decode、同一依赖内部的有界 retry 等既有协议
  仍按各自规范执行，但不得被借来规避 dependency-first。

## macOS 分发不变量

- macOS 唯一发行 App 是 Developer ID/direct-distribution `EgakiumMac`；不得把
  Mac App Store App Sandbox 恢复为产品约束、功能裁剪理由、依赖准入条件或
  默认测试/release gate。精确合同见 `docs/MACOS_DISTRIBUTION.md`。
- 源码中的 `EgakiumMacAppStore`、`.macAppStore` 与 App Store entitlements
...
- `.egakium/release-recovery/<run>/state.plist` 是 owner-only 的本地恢复状态（schema
...
- 产品只保留上述一个组合 Cowork 窗口。不得重新增加独立 Canvas `WindowGroup`、window value、
  ...
  入口；这类入口会允许系统打开或恢复只见画布而没有 harness 的错误产品面。
- macOS 主 sidebar 当前只展示 Cowork，Chat/Code 入口只能做 presentation 隐藏。不得删除
  `EgakiumNavItem.chat/code`、detail switch branch、Chat/Code View/ViewModel、session/history/runtime、
...
- provisional element-card v1 是 source contract，不得成为覆盖 `@main` 文件的迁移理由。新模板和
  exact `@main` prompt 使用 `#canvas` direct child `.egakium-element`、safe/unique
  `data-element-id`、source `data-x/y/width/height` 与同 Session relative iframe；旧 Session 只要保留
  `#canvas` 就由 CEF 直接显示；不得原地重写或“升级”磁盘 HTML，也不得重新注入宿主 DOM runtime。
- `SessionCanvasRuntime`、WK content world、`sessionStorage` layout override 和宿主 drag/resize/keyboard
  已删除。不得把这些能力以 CEF adapter 名义重建。source attributes 仍是当前页面布局 authority；
  未来用户交互若落地，必须先选定合规依赖并冻结 durable layout/revision/EventLog 合同。
- child document 只能有一份 host-authored `SessionCanvasElementTemplate` source。模板必须保持
  AgentID/ElementID/SessionID/path/layout/credential-free，以 `#element`/`#content` 为 content-only
  integration boundary，并默认 no-network/no-nested-frame；不得按 agent/model/role 生成不同“模板”，
  也不得把 template copy 写成永久 Agent↔Element owner、ElementLease 或 capability。
- 成功的 ordinary `spawn_agent` 必须由宿主选择 fresh safe `CanvasElementID`，并在 child 的 exact
  workspace root 下以 no-overwrite 方式创建
  `.egakium/canvas/<SessionID>/elements/<ElementID>/index.html`。permission intent 必须显式声明
  `provision_canvas_element` 与 workspace-mutation risk；同一 validated descriptor 必须进入
  `agent_spawn_requested` / `agent_spawned`、成功 ToolResult、`list_agents` 和 child trusted prompt。
  `attach` 不得伪造该 provenance；manual/legacy worker 才可使用 identity-free prompt fallback。
- element file 必须在 spawn admission batch 前完整发布；append 报错后必须先做 complete-known
  EventLog replay。若 exact contiguous batch 已 durable（包括 WAL/lost-ack recovery），必须保留文件并
  完成 live admission；只有完整历史证明 descriptor fact 未提交时，才可删除本次仍为 exact 原始
  template 且目录形状未变的资源。unknown/incomplete/conflict 必须保留并 fail closed，不能误删已被
  durable event 引用或并发修改的文件。admission 成功后 agent recycle/detach 不得删除 element file。
  该跨存储 protocol 不得被写成 filesystem 与 EventLog 的真正原子事务，也不得扩展成永久 owner/lease。
  file publish→EventLog append crash gap 的 unreferenced orphan 当前不得靠目录扫描自动 GC；没有
  versioned recovery/identity proof 时，保留比误删用户或 agent 工作更安全。
- exact `@main` 不再预先选择 spawn ElementID/path；它必须等待成功 ToolResult 返回
  `canvas_element_id` / `canvas_element_path` / `canvas_template_version`，随后才可集成 card 或在后续
  round 以 TaskContract/`delegate_task` 委派。host provisioning 绝不能修改共享 `index.html`。
  read-only child 虽获得真实文件仍不得编辑；需要修改时必须显式 `requestedAccess=read_write` 且不
  超过 issuer ceiling。已有 destination 必须 read/preserve，禁止用 generic template 重置现有工作。
...
- `@main` 与 sub-agent 继续通过既有 spawn/delegate/task/mailbox/scheduler flow 协作；不得同步递归
  `AgentLoop`，也不得因 ElementID 存在而跳过 attach、ToolResult 因果轮次、lease 或 permission。
- Canvas 与 harness 只能通过稳定 SessionID/ElementID 驱动的窄结构化选择、定位、刷新、布局和状态
  事件连接；不得从标题、HTML 正文、当前 UI selection 或 agent display name 反推 authority。
- 当前 Canvas 工作副本固定在 primary workspace 的
  `.egakium/canvas/<SessionID>/index.html`；相同 workspace 中必须按 SessionID 隔离，初始化不得覆盖
  既有编辑。当前 `data-element-id` 和 source frame 只是 provisional DOM contract；durable layout、
  element revision 和 artifact/event schema 在实现前仍须冻结并保持 additive/versioned/
  legacy-decodable。不得让最后一份 HTML、Web session override、CEF
  profile 或 Web storage 覆盖 EventLog 的 Session canonical truth，也不得把 bookmark、credential、
  raw bridge payload 写入普通 projection。
- CEF/HTML/JavaScript 不得直接获得任意文件、Shell、进程、credential、网络或 native bridge 能力。
  内部资源必须有稳定 origin、CSP、路径校验和 document 隔离；model-facing Canvas mutation 仍须经过
  ToolRegistry、CapabilityLease、WorkspaceLease、PermissionEngine 与 durable execution。
- 官方 CEF 是 Canvas 唯一接受 renderer。当前 `CoworkCanvasHost` 只嵌入 `EgakiumCEFView`；不得重新
  增加 WKWebView、WebKit、第二 renderer、compile/runtime switch 或 fallback。CEF 缺失/hash 不符必须
  构建失败，初始化失败必须明确显示 Canvas unavailable。
- CEF native host 只允许官方 API 所需最薄 lifecycle、child browser、request context、Helper、资源、
  scheme、权限和 bundle 接线；不得自建 CEF-equivalent abstraction、Content embedder 或 Chromium/CEF
  核心能力。WebKit-specific content world/navigation/lifecycle 已删除，不得恢复。
- `.deps/cef` 的 exact pinned distribution 和 `build/egakium-cef-runtime` 现由正式 build 消费；`Chromium/` 与
  其他旧 `build/` 内容仍只是保留资产。不得恢复 `chrome --app`、完整 Chromium、
  `content_shell`、自建 Content embedder 或旧独立 CEF App 作为当前产品壳，也不得把旧
  `USE_SANDBOX=OFF` ad-hoc 原型写成发行证据。
- Canvas 当前只属于 macOS 目标；不得把 CEF、Cowork、Tools 或本地 Agent workspace 隐式链接进 iOS。

完整合同见 `docs/EGAKIUM_CANVAS_COWORK.md`。

## OpenSource gitlink 不变量

- 根 `.gitmodules` 与父 index 必须共同描述 26 个 `OpenSource/<project>`：每个 path 只有一个公开
  origin、`shallow = true` 和一个 mode `160000` gitlink。不得把子仓库普通文件重新加入父 index。
- `OpenSource/` 只是研究 checkout；存在 gitlink 不等于 SwiftPM/XcodeGen 依赖、许可证准入、runtime
  可用或发行闭包完成。实际采用仍以 `NOTICE.md`、`ThirdPartyNotices/` 和正式依赖/helper 记录为准。
- 普通父仓库任务不得递归 stage、commit、push、清理或改写子仓库。更新 pointer、origin、shallow
  策略或任一子仓库内容必须由用户明确点名，并重新核对 exact commit、许可证、NOTICE 和供应链边界。
- 本机子仓库工作树和 Git metadata 不得因父仓库提交被删除。父仓库历史中的旧 flattened blob 也不得
  未经用户明确授权通过 rebase/filter/history rewrite 清理。
...
- `ThirdPartyStandards/OpenKnowledgeFormat/0.2/` 的规范、许可证、upstream identity 和
  `SHA256SUMS` 必须一起保持 byte-exact；`.egakium-rag/` profile、chunks、index、checksums、
  receipt 和 store pointer 都是 Egakium 派生状态，不得冒充 OKF 标准或知识真值。
...
- publish 只能从 bounded staging 经同一 Validator、checksum completeness 和 content-seal 复核后
  ...
  写成紧急清除。
- shipping 发布区固定为 `.egakium-rag-store.json`、`.egakium-rag-snapshots/` 与
  `.egakium-rag-host/`。它们必须留在 WorkspaceLease/managed terminal 的不可移除、大小写无关 deny
...
- legacy `snapshots/` 与 current `.egakium-rag-snapshots/` 同时存在必须 fail closed；只读 open 不得
...
- 只允许连接外部 MCP Server 的客户端角色。不得新增 Egakium MCP Server
  ...
  名义引入 server hosting。`EgakiumMCPConformanceClient` 必须保持开发期
  client driver，不能进入发行 bundle。
- 当前产品平台链接必须保持：Developer ID macOS 与 CLI 可链接 stdio + HTTP；
  iOS 不得链接 `EgakiumMCP`、Curl transport、stdio runtime 或 MCP 产品 UI。
  共享 `EgakiumProtocol` payload 不构成平台能力。遗留
  `EgakiumMacAppStore` 的 HTTP-only 图不再是产品不变量或验收门。
...
- 不使用泄露、反编译或绕过访问控制获得的源码/私有 prompt；不复制第三方名称、Logo、图标、截图、UI 资产、商标性外观或品牌文案作为 Egakium 产品身份。
- 公开仓库中许可证兼容的 model-facing prompt 可经审查后派生复用，但必须去除上游品牌/支持链接并重新核对 Egakium 工具名、权限和安全语义；私有或许可证不明 prompt 禁止使用。
- 上游默认权限、工具、文件访问、网络访问或自动执行行为不能直接继承；必须映射到 Egakium 的 PermissionEngine、CapabilityLease、WorkspaceLease、PathConfinement、durable tool ticket 和 EventLog。
...
- 不在 Egakium 内重写 Markdown parser、AST rewriter、代码 lexer/grammar/token classifier、TeX 解析/排版或 Markdown layout。Markdown grammar/AST/原生布局属于经审计的 Microsoft `SwiftStreamingMarkdown` 派生包；Egakium 只维护 admission/backpressure/revision/policy/plain fallback。任何需要改 parser 或 table/paragraph layout 的缺陷，应在仓内 vendored derivative 中以 provenance/tests 约束，并尽量提交 upstream；不得悄悄堆成无来源、无 ledger 的 Egakium 私有 renderer。
...
- **GUI config**：UserDefaults 规范主键为 `egakium.providerCatalog.v1`（mac/iOS 共用），保存 provider/model 两层元数据：provider `baseURL` + `chatEndpoint` + secret ref 元数据、model id + 展示名；当前聊天选择另存 `egakium.providerSelection.v1`，macOS 可保存 provider/model/variant identity，iOS 保存 provider/model identity。Base URL 与 Chat endpoint 需要保持同步：Base URL 自动补 `/chat/completions` 生成 Chat endpoint；Chat endpoint 清洗 `/chat/completions` 后缀回填 Base URL。旧 `egakium.baseURL`、`egakium.model` 仅作为迁移来源/兼容镜像，不得作为唯一新状态。macOS 高级 JSON/JSONC 配置只允许 `EGAKIUM_CONFIG` 显式指定文件、`~/.config/egakium/egakium.json` / `egakium.jsonc`、app support `egakium.json` / `egakium.jsonc`，旧 Egakium `config.json` 仅作兜底兼容读取；不得自动发现名为 `opencode.json` 的文件或读取 OpenCode app 配置。配置内容可采用 OpenCode-compatible 顶层 `$schema` / `enabled_providers` / `model` + `provider` map，但文件所有权、默认路径和生成文件名必须属于 Egakium。聊天页当前选择可覆盖 JSON 顶层 `model` 且不得自动改写外部 JSON；模板不得回退到直接输出内部 `providers` 数组。
...
- **Provider config 路径引用**：`providerConfig` secret ref 只能读取当前 Egakium 自有 `egakium.json/jsonc`、旧 Egakium `config.json/jsonc` 或用户本次显式 `EGAKIUM_CONFIG` 指定文件；历史 UserDefaults 中其他绝对路径必须 fail closed，不能借旧元数据恢复对 `opencode.json` 或第三方配置的隐式读取。
...
- **Agent 网络/浏览器工具**：`web_fetch`、`browser_diagnostics`、
  ...
  `.egakium/browser/`；截图只能写 workspace PNG，上传只能读 workspace 文件，
  download 只能写 profile 专属目录并通过 `changedFiles` 暴露。state 可保存当前
  页面 metadata 和 Egakium navigation stack/index，但不能保存 secret。
...
- **Same-call permission sidecar 输入契约**：Cowork provider-facing business schema 只在 request-owned copy 中增加 required string `__egakium_authorization_context`；对任何 `strict:true` function，decorated copy 必须递归满足 `required == properties.keys` 与 `additionalProperties:false`，否则在发网前 typed fail closed。`tool_search` 本身不得伪装成普通 function；其 request-owned `tool_search_output` 中 deferred function/namespace children 必须装饰，durable output 与远端原业务 schema 不变。原 ToolDescriptor/business required/executor schema 不变，宿主仅在 deterministic gate 实际进入 automatic ask 时消费并验证 sidecar。live reviewer 必须收到 acting model 同一 generation 的 complete canonical sidecar-free safe business arguments、完整 string sidecar 与 mechanical host binding/gate/lease/action facts；不得退回 digest/preview-only reviewer，也不得恢复第二次 Reporter/full-context/PDF/image resend。reviewer provider prompt 不得包含 TaskContract objective/role/deliverable、causal userGoal、raw/current 用户指令、assistant/history、PDF 或图片原文。valid sidecar 只可保留在当前 turn 的 acting-model 内存 conversation；raw sidecar 与 reviewer transient exact-args 副本不得进入 EventLog/permission lifecycle，durable `.tool_call` 与 model history 只保存 stripped business view。sidecar/invocation business digest/count 只绑定该 stripped canonical business view；它必须与自定义 authorization identity digest/count 分域校验，禁止直接要求二者相等。missing/malformed/secret-bearing sidecar 只能产生 failed/runtimeFailed `tool_result`，不得创建 `permission_request` / `permission_resolved`、调用 reviewer或消耗 permission denial fuse；同 business args 修正后必须仍可送审。binding 不一致单列为 authorization snapshot failure并 typed fail closed。manual/nonautomatic call 携带保留字段必须 redacted fail closed，绝不能把它传给业务 tool/MCP/executor。live size ceiling 不能凭空固定，未来只能由 exact route budget 推导并整份拒绝，禁止裁切后继续审查。
...
- **敏感路径**：`~/.ssh`、`~/Library/Keychains`、`~/.local/share/egakium/auth.json`、`~/.local/share/opencode/auth.json`、`~/.config/egakium/egakium.json`、`~/.config/opencode/opencode.json`、secret/token/key 目录 → 硬 deny。
...
- 根 `Egakium.icon` 是 `EgakiumMac` 与 `EgakiumiOS` 的 canonical Apple 图标源；两个
  ...
  不得手工导出另一套 iOS PNG 或让遗留 `EgakiumMacAppStore` 成为图标事实源。
...
- 默认且唯一发行的 `EgakiumMac` 是 Developer ID/direct-distribution
  workbench：`project.yml` 必须指向 `EgakiumMac.DeveloperID.entitlements`，
  ...
  经过 Egakium 权限链；managed terminal/git shell 继续经过 runtime Seatbelt，
  浏览器则遵守上面的专用 broker + WorkspaceLease + Chromium native sandbox
  合同，不能把两类执行器混称为同一个 Seatbelt lane。
- `EgakiumSharedUI` 用 `#if canImport(SwiftUI)`，不得引入 macOS 专属 API 而破坏 Linux/无头构建。
...
- `hosted_web_search` Agent Tool 不得退化成 Chat 隐式开关或 browser/MCP 别名。它必须只绑定同一次
  ...
  不得改走 `browser_search` / `web_fetch` / MCP / shell / 本地浏览器 / 第二模型。Chat 仍为无 Egakium
  ...
  `tool_result`；registry identity 当前为 `egakium.standard.v4` / `egakium.cowork.v4`。
- `Mediator` 默认转发摘要、不转发原始字节：不得退化成透传完整内容。
- `SecretScanner` 标记集不得删减。
- Provider catalog 不存秘密本身：UserDefaults 与 `egakium.providerSelection.v1` 只能保存 provider/model/variant/secret ref 元数据，不得保存 model/variant 任意参数或明文 API key；UserDefaults/docs 不得出现明文 API key。Egakium 生成的 OpenCode-compatible provider 模板默认只能写 `options.apiKey` 的 `{env:NAME}` / `{file:path}` 等引用，不得在模板中写真实 key。macOS 设置页用户主动输入的 API key 必须写入当前可编辑 Egakium-owned OpenCode-compatible provider JSON 的 `provider.<id>.options.apiKey`，iOS 设置页用户主动输入的 API key 仍写入 app container `Egakium/auth.json`（或 `EGAKIUM_AUTH_FILE` 指定文件）并应尝试设置 `0600`；不得写入 OS Keychain。真实 provider 请求可从 Egakium-owned OpenCode-compatible config 的 `provider.<id>.options.apiKey`、auth JSON、env 或 file secret 懒加载 secret；如果 macOS 当前 provider catalog 来自 Egakium-owned OpenCode-compatible config 且该 provider 直接声明了 `options.apiKey`，secret ref 必须绑定到该 provider config 文件本身，不得被同 provider id 的旧 auth JSON 抢先覆盖；默认不得读取 OpenCode app 的 `~/.config/opencode/opencode.json` 或 `~/.local/share/opencode/auth.json`。macOS auth JSON/secret/config 文件可能含密钥，Agent 不得读取、打印、摘要或写入其内容。启动、`needsAPIKey`、设置 UI 占位符和真实 provider 请求均不得调用 OS Keychain 查询；真实 provider 请求读取 secret 后必须复用 resolver 进程内缓存，但 provider registry 刷新时可以清空该缓存以避免旧 key 继续生效；OpenAI-compatible `Authorization` header 必须只发送单层 `Bearer <token>`，并容错剥离用户误存的外层引号或 `Bearer ` 前缀。
...
- 改 Chat 自动命名：至少覆盖 `ChatSessionAutoTitleTests` 与 `ChatAutoTitleViewModelTests`，证明
  ...
  `EgakiumMac` 与 `EgakiumiOS`，复核 iOS target 未链接 Tools/Permission/AgentKernel/Cowork/MCP。
