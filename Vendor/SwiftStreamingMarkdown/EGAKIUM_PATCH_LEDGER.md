# Egakium SwiftStreamingMarkdown Vendored Patch Ledger
...
This ledger describes the Egakium-maintained vendored derivative of Microsoft
SwiftStreamingMarkdown. The vendored snapshot lives at
`Vendor/SwiftStreamingMarkdown` and is versioned by the containing Egakium Git
revision. It is third-party-derived code, not independently authored Egakium
renderer code.
...
- Reserve the public `@concurrent` transfer boundary for Egakium' external
  parse scheduler. Egakium must own the result on `@MainActor` and retain a
...
- Keep any still-compiled legacy image/citation compatibility source out of the
  Egakium production parse profile; it is not a first-release capability.
...
- Track the last valid laid-out paragraph width independently from the
  ...
  contract.
- Remove the Egakium facade's whole-rich-document
...
- Run the full retained Release suite under strict concurrency and warnings as
  errors on macOS.
- Add Egakium-level golden rendering, streaming replacement, selection/copy,
...
- Host accepted formulas through a live TextKit 2
  ...
  Egakium' display revision so an old-size attachment cannot overwrite a
  newer request.
...
- Treat iosMath's eight unmodified OpenType fonts and accompanying math-table,
  ...
  They do not replace or determine Egakium' product-interface font.
...
- The Egakium root and derivative `Package.resolved` files currently agree on
...
- The interactive Computer Use soak recorded 18 AppKit negative-geometry
  issues in the Egakium PID. Each cluster immediately followed system
...
- containing Egakium revision:
  `Packages/EgakiumSharedUI/Tests/MessageRenderingTests.swift`
- containing Egakium revision:
  `Apps/EgakiumMac/Sources/{EgakiumChatScreen,RendererFixtureView}.swift`
- containing Egakium revision:
...
- Egakium SharedUI's syntax-agnostic whole-message rich-admission boundary and
...
- Root `MessageRenderingTests` passed 41/41; `swift build --disable-sandbox
  --target EgakiumSharedUI`, `xcodegen generate`, EgakiumMac macOS Debug, and
  EgakiumiOS generic Simulator Debug all succeeded.
...
`Egakium Renderer Validation` instances were accidentally allowed to coexist.
...
- EgakiumMac Debug/Release and EgakiumiOS generic Simulator Debug/Release
...
The Egakium root revision is the immutable identity for a distributed source
...
notices and provenance, rerun the full Egakium cutover suite, and delete the
