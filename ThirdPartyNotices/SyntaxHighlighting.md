...
derivative used by Egakium. Code blocks are rendered as plain text with native
...
contain no HighlightSwift or highlight.js dependency. The former Egakium
vendored resources are removed from the current working tree and from the
`EgakiumSharedUI` resource list:
...
Egakium previously shipped a selectively vendored highlight.js 11.11.1 engine
