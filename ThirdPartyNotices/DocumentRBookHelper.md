...
- Local component: `Packages/EgakiumTools/Runtime/rbook-helper`
- Reuse class: `dependency` inside an Egakium-owned `external-runtime` helper
...
- Network: denied by the Egakium document process boundary
...
against operation postconditions before Egakium runs EPUBCheck and considers
the staged commit.

## Binary distribution gate

Before a compiled helper is put into a signed Egakium distribution, the
...
typed `backend_missing`; Egakium must not download a helper or switch to a
