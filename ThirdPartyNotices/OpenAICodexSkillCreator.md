...
- Local path: `.agents/skills/egakium-skill-creator/`
...
Every distributed file above is modified by Egakium. The upstream
`agents/openai.yaml`, icons, images, branded assets, and all other Codex
system Skills are not copied.

## Egakium modifications

- Renamed the Skill from `skill-creator` to `egakium-skill-creator` so the
  project copy does not collide with the separately discovered Codex system
  Skill of the same upstream name.
- Reworked the operating instructions for Egakium project roots,
...
- Removed examples that trigger Egakium' credential-shaped-content scanner
...
- Narrowed `agents/openai.yaml` guidance to Egakium' supported MCP dependency
  subset. Interface and policy fields remain optional cross-harness metadata;
  they are not described as Egakium runtime permissions. The helper does not
...
normal Egakium permission and durable-execution path. It adds no network,
...
document all local modifications, and rerun the Egakium Skill validation and
