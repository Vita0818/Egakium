...
authorizes an MCP server product: Egakium excludes every upstream server
...
Tools wire schemas. Egakium moved those five wire-schema files out of the
...
an Egakium MCP client package dependency.
...
Production MCP HTTP/OAuth I/O uses the Egakium-owned
`EgakiumCurlTransport` boundary, not the SDK HTTP transport. macOS links the
