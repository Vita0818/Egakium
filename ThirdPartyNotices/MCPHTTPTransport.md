...
This notice covers the native `EgakiumCurlTransport` dependency used by the
...
`Package.swift` links `EgakiumCurlTransport` with `.linkedLibrary("curl")`.
For the macOS DeveloperID and App Store products this resolves to the libcurl
provided by the Apple SDK/operating system. Egakium does not vendor a Darwin
...
The iOS product does not link `EgakiumMCP` or `EgakiumCurlTransport`.
...
that executable even though Egakium does not redistribute the SDK archives as
standalone files.
...
pkg-config SPDX identifier. Egakium resolves that ambiguity in the
