...
| User bubbles omit `You` | Passed | The real-history Chat capture shows a trailing user bubble whose first visible content is the message; the adjacent assistant keeps `Egakium` and timestamp. |
| Sidebar subtitle is removed | Passed | The current app capture shows only `Egakium` above the mode rows, with history and Settings unchanged. |
...
4. Built and launched a uniquely identified ad-hoc QA copy of the latest EgakiumMac Debug app,
...
- Targeted SwiftPM regression: 32 tests / 0 failures across
  `ExecutionTracePresentationTests`, `EgakiumConversationCodeTests` and `ThreadLayoutTests`.
- Latest `EgakiumMac` macOS Debug Xcode build: `BUILD SUCCEEDED` with local ad-hoc signing.
...
- `EgakiumSharedUI`, `EgakiumMac` Release/Debug and generic Simulator `EgakiumiOS` Debug builds
  succeeded.
- `/Applications/Egakium.app` contains the latest Release build. The host has no valid Developer ID
...
- Keep the accepted top model picker, Egakium drawer, Recents hierarchy, composer and Settings
...
- Read-only PNG conversions: `/private/tmp/egakium-font-reference/IMG_4904.png`,
  `/private/tmp/egakium-font-reference/IMG_4905.png` and
  `/private/tmp/egakium-font-reference/IMG_4906.png`.
- Final implementation captures: `/private/tmp/egakium-ios-serif-home.png`,
  `/private/tmp/egakium-ios-serif-sidebar.png` and
  `/private/tmp/egakium-ios-serif-settings.png`.
- Paired full-screen comparisons: `/private/tmp/egakium-serif-home-comparison.png` and
  `/private/tmp/egakium-serif-sidebar-comparison.png`.
...
| Drawer hierarchy remains correct | Passed | `Egakium` is title2 semibold, `Recents` is headline semibold, and the empty state is subordinate body copy, all in serif. |
...
- Copy and content: `Egakium`, the configured model name, empty-state text and composer prompt are
...
- `EgakiumiOS` generic iOS Simulator Debug unsigned build completed successfully after the final
  source patch.
...
- Compile the latest root `Egakium.icon` into the iOS app and verify its installed appearance.
...
- iOS final default states:
  `/Users/vita/.codex/visualizations/2026/08/01/019fbc6f-219c-7340-a461-92dc6f2794b7/egakium-ios-dark-home.png`
  and
  `/Users/vita/.codex/visualizations/2026/08/01/019fbc6f-219c-7340-a461-92dc6f2794b7/egakium-ios-final-light-home.png`.
- iOS component states:
  `/Users/vita/.codex/visualizations/2026/08/01/019fbc6f-219c-7340-a461-92dc6f2794b7/egakium-ios-light-sidebar.png`
  and
  `/Users/vita/.codex/visualizations/2026/08/01/019fbc6f-219c-7340-a461-92dc6f2794b7/egakium-ios-light-settings.png`.
- Installed icon:
  `/Users/vita/.codex/visualizations/2026/08/01/019fbc6f-219c-7340-a461-92dc6f2794b7/egakium-ios-home-icon.png`.
- Required combined reference/implementation input:
  `/Users/vita/.codex/visualizations/2026/08/01/019fbc6f-219c-7340-a461-92dc6f2794b7/egakium-macos-ios-design-comparison.png`.
...
| Requirement | Result | Evidence |
|---|---|---|
| Typography matches macOS roles | Passed | `Egakium`, session and Settings titles use system serif; body, model label, buttons, menus, forms and input use system sans. The former root `.fontDesign(.serif)` is removed. |
...
| Drawer uses the same information architecture | Passed | Serif `Egakium`, selected Chat glass row, `Recent` + circular New, session list and bottom Settings replace the prior gear/search/large-Chat arrangement. |
...
| Latest app icon is installed | Passed | Built Info.plist declares `CFBundleIconName=Egakium`; iPhone/iPad icon files are present and the simulator home screen shows the new pointer icon. |
...
3. Added the root `Egakium.icon` to the iOS resource graph, regenerated the Xcode project and
...
- XcodeGen and generic `EgakiumiOS` Simulator Debug unsigned build succeeded.
...
`EgakiumThreadViewportFramesPreferenceKey` updated more than once in a frame during window changes.
...
| Window position cannot feed back into thread layout | Passed | GeometryReader frame probes and `EgakiumThreadViewportFramesPreferenceKey` were removed; bottom restoration uses `onScrollVisibilityChange`. The old preference warning count was zero after real full-screen, repeated agent switching and focus return. |
